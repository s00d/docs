...
description: "將 Nuxt I18n Micro 與 Storybook 整合：設定 @nuxtjs/storybook、加入語系控制項，並跨語言預覽元件。"
---
...
將 **Nuxt**、**Storybook** 與 **nuxt-i18n-micro** 整合到專案中，可讓你以元件驅動的方式打造健全的多語系應用。本指南提供逐步操作說明，包含設定、在地化與 Storybook 整合。
...
要讓 Storybook 與 Nuxt、nuxt-i18n-micro 整合，請設定 `.storybook/main.ts`，加入 `mergeTranslations` 與 `staticDirs`。
...
1. **[GitHub 上的 Nuxt + Storybook + i18n 範例](https://github.com/s00d/nuxtjs-storybook-i18n-micro)**
   此 repository 示範如何整合 Nuxt、Storybook 與 nuxt-i18n-micro，包含設定說明與細節。

2. **[Storybook for Nuxt 官方文件](https://storybook.nuxtjs.org/getting-started/setup)**
   Storybook 搭配 Nuxt 的官方設定文件，包含詳細步驟與最佳實務。

3. **[StackBlitz 上的互動範例](https://stackblitz.com/~/github.com/s00d/nuxtjs-storybook-i18n-micro)**
   直接在瀏覽器中體驗 Nuxt、Storybook 與 nuxt-i18n-micro 整合的即時範例。
