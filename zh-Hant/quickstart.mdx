---
title: "快速開始"
description: "安裝並設定 Nuxt I18n Micro。"
---
...
<Tip>
- **根層級檔案**(`locales/en.json` 等)——整個應用共用的翻譯(選單、頁尾、常用 UI)。在 `translationPayloads.mode: 'premerged'`(預設)下,它們會在建置期合併進每個頁面。
- **頁面專屬檔案**(`locales/pages/<route>/<locale>.json`)——特定頁面獨有的翻譯,僅在訪問該頁面時載入。
- **動態路由**——`pages/articles/[id].vue` 對應到 `locales/pages/articles-id/`(方括號被替換為短橫線)。
- **自動產生**——在 `nuxt dev` 期間,所有缺失的翻譯檔會自動建立。
</Tip>
...
- **翻譯**:預設將 `locales/` 保留在**專案根目錄**(建議慣例),而非放在 `app/` 底下
...
可選的同置放置:`i18n: \{ translationDir: 'app/locales' \}`(參見 [fixture `n3`](https://github.com/s00d/nuxt-i18n-micro/tree/main/test/fixtures/n3))。

若要使用可選的 CLI 輔助工具 `text-to-i18n`,請使用 [`nuxt-i18n-micro-cli` ≥ 2.1.1](/guides/cli#-text-to-i18n-command),才能從專案根目錄掃描 `app/pages` 與 `app/components`。
</Info>
...
- **[每個元件的翻譯](/guides/per-component-translations)** —— 認識 `$defineI18nRoute`
- **[自訂語言偵測](/guides/custom-language-detection)** —— 使用 `useI18nLocale()` 以程式方式管理語系
