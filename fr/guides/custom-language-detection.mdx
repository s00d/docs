---
title: "Détection de langue personnalisée dans Nuxt I18n Micro"
description: "Détection de locale personnalisée avec useI18nLocale()."
---
...
Nuxt I18n Micro v3 fournit un composable centralisé — `useI18nLocale()` — pour toute la gestion des locales. Cela remplace les motifs manuels `useState`/`useCookie` de v2. En utilisant `useI18nLocale().setLocale()` dans un plugin serveur, vous pouvez implémenter n'importe quelle logique de détection personnalisée tout en conservant une compatibilité totale avec les systèmes de redirection et de cookie du module.
...
**Principe clé** : Appelez `useI18nLocale().setLocale()` dans un plugin serveur avec `order: -10`. Cela met à jour `useState('i18n-locale')` et le cookie de locale de manière atomique, de sorte que le plugin i18n (`order: -5`) et le plugin de redirection serveur (`order: 10`) voient la locale correcte dès la première requête SSR. Les redirections automatiques côté client s'exécutent dans le middleware de route global lors des navigations suivantes.

## 🛠️ Désactiver la détection automatique intégrée

Si vous voulez un contrôle total sur la détection de locale, désactivez le mécanisme intégré :
...
Il s'agit de l'**approche recommandée** pour toutes les stratégies.
...
2. **`setLocale()` met à jour à la fois** `useState('i18n-locale')` et le cookie — garantit une visibilité immédiate pour tous les plugins suivants
...
5. **Le middleware de route client** (`i18n-redirect`) applique les redirections lors de la navigation SPA lorsque la locale de l'URL ne correspond pas à la préférence

## 🌐 Comportement spécifique aux stratégies
...
|----------|---------------------------|
| `no_prefix` | Rendu en allemand, pas de changement d'URL |
| `prefix` | 302 → `/de/` (redirection côté serveur) |
| `prefix_except_default` | 302 → `/de/` si `de` ≠ défaut ; pas de redirection si `de` = défaut |
| `prefix_and_default` | Pas de redirection (`/` et `/de/` sont valides) |
</Tip>

**Notes importantes :**

- La détection de locale basée sur les cookies est désactivée par défaut (`localeCookie: null`)
- Définissez `localeCookie: 'user-locale'` pour activer la persistance par cookie
- Si la valeur du cookie/état n'est pas dans la liste `locales`, elle retombe sur `defaultLocale`
...
Si les utilisateurs peuvent changer manuellement de langue, respectez leur choix plutôt que l'auto-détection :
...
| `useI18nLocale().getLocale()`          | Renvoie la locale actuelle depuis l'état ou le cookie             |
| `useI18nLocale().getPreferredLocale()` | Renvoie la locale préférée (validée contre la liste `locales`)    |
| Plugin `order: -10`                    | Garantit que votre plugin s'exécute avant l'initialisation d'i18n |
| `enforce: 'pre'`                       | S'exécute dans le groupe de plugins « pre »                       |

**À NE PAS faire :**

- Utiliser directement `useCookie('user-locale')` — `useI18nLocale()` gère les cookies en interne
- Utiliser directement `useState('i18n-locale')` — utilisez plutôt `useI18nLocale().setLocale()`
