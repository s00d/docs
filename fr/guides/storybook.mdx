...
description: "Intégrez Nuxt I18n Micro à Storybook : configurez @nuxtjs/storybook, ajoutez des contrôles de locale et prévisualisez les composants dans toutes les langues."
---
...
Intégrer **Nuxt**, **Storybook** et **nuxt-i18n-micro** dans votre projet vous permet de construire une application localisée robuste avec une approche de développement centrée composants. Ce guide fournit une procédure étape par étape pour mettre en place ces outils ensemble, y compris la configuration, la localisation et l'intégration Storybook.
...
Pour intégrer Storybook avec Nuxt et nuxt-i18n-micro, configurez le fichier `.storybook/main.ts` et ajoutez `mergeTranslations` et `staticDirs`.
...
1. **[Exemple Nuxt + Storybook + i18n sur GitHub](https://github.com/s00d/nuxtjs-storybook-i18n-micro)**  
   Ce dépôt montre comment intégrer Nuxt, Storybook et nuxt-i18n-micro. Il inclut les instructions de mise en place et les détails de configuration.

2. **[Documentation Storybook pour Nuxt](https://storybook.nuxtjs.org/getting-started/setup)**  
   La documentation officielle pour mettre en place Storybook avec Nuxt. Elle fournit des étapes détaillées et des bonnes pratiques.
