...
description: "Index de référence de chaque paquet du workspace @i18n-micro, listant les API et types exportés générés depuis les snapshots source contre lesquels la CI vérifie."
---

# API des paquets

`nuxt-i18n-micro` est construit à partir d'un ensemble de petits paquets, dont la plupart sont utilisables indépendamment : `@i18n-micro/vue` dans une app Vue standard, `@i18n-micro/core` n'importe où.

Chaque page ci-dessous est générée depuis le snapshot d'API contre lequel [`pnpm run api:surface`](https://github.com/s00d/nuxt-i18n-micro/blob/main/docs/guide/maintenance-commands.md#api-surface) vérifie les sources TypeScript, ce qui garantit qu'un export ne peut disparaître sans que cette référence ne change ou que la CI n'échoue.
...
- [Intégrations](/integrations/overview) — guides d'utilisation de ces paquets dans chaque framework
- [Options du module](/api/module-options) — options acceptées par le module Nuxt
- [Méthodes](/api/methods) — les aides runtime disponibles dans les composants
