...
description: "Migrate from @nuxtjs/i18n to Nuxt I18n Micro: map configuration, replace composables and components, and move translation files over."
---
...
If using `no_prefix` strategy, set the locale in a server plugin with `order: -10` using `useI18nLocale().setLocale()` to ensure server and client agree on the locale. See [Custom Language Detection](/guides/custom-language-detection).
