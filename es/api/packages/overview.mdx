...
description: "Índice de referencia para cada paquete del workspace @i18n-micro, listando APIs exportadas y tipos generados a partir de snapshots del código fuente que CI verifica."
---

# APIs de paquetes

`nuxt-i18n-micro` está construido a partir de un conjunto de paquetes pequeños, la mayoría usables por su cuenta. Por ejemplo, `@i18n-micro/vue` en una app Vue simple, `@i18n-micro/core` en cualquier lugar.

Cada página a continuación se genera desde el snapshot de la API que [`pnpm run api:surface`](https://github.com/s00d/nuxt-i18n-micro/blob/main/docs/guide/maintenance-commands.md#api-surface) verifica contra el código fuente TypeScript, por lo que un export no puede desaparecer sin que esta referencia cambie o falle el CI.
...
- [Opciones del módulo](/api/module-options) — opciones aceptadas por el módulo Nuxt.
- [Métodos](/api/methods) — los helpers de runtime disponibles dentro de componentes.
