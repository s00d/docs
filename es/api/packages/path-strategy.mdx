...
description: "API pública del paquete @i18n-micro/path-strategy: resolución de rutas de idioma en runtime, redirecciones y helpers SEO, generada a partir del código fuente."
---

# `@i18n-micro/path-strategy`

38 exports en 6 puntos de entrada.
...
<code>NoPrefixPathStrategy</code> — 36 miembros, idéntico a `NoPrefixPathStrategy` above.
<code>Strategy</code> — 36 miembros, idéntico a `NoPrefixPathStrategy` above.
...
<code>PrefixPathStrategy</code> — 36 miembros, idéntico a `PrefixPathStrategy` above.
<code>Strategy</code> — 36 miembros, idéntico a `PrefixPathStrategy` above.
...
<code>PrefixAndDefaultPathStrategy</code> — 37 miembros, idéntico a `PrefixAndDefaultPathStrategy` above.
<code>Strategy</code> — 37 miembros, idéntico a `PrefixAndDefaultPathStrategy` above.
...
<code>PrefixExceptDefaultPathStrategy</code> — 44 miembros, idéntico a `PrefixExceptDefaultPathStrategy` above.
<code>Strategy</code> — 44 miembros, idéntico a `PrefixExceptDefaultPathStrategy` above.
...
<code>PathStrategy</code> — 22 miembros, idéntico a `PathStrategy` above.
<code>PathStrategyContext</code> — 15 miembros, idéntico a `PathStrategyContext` above.
<code>ResolvedRouteLike</code> — 6 miembros, idéntico a `ResolvedRouteLike` above.
<code>RouteLike</code> — 6 miembros, idéntico a `RouteLike` above.
<code>RouterAdapter</code> — 3 miembros, idéntico a `RouterAdapter` above.
<code>SwitchLocaleOptions</code> — 1 miembros, idéntico a `SwitchLocaleOptions` above.
