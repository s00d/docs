...
description: "Katra opcija, ko modulis pieņem, ar tās tipu, noklusējumu un nolūku — nolasīta būvēšanas laikā no tipa definīcijas."
---

# Moduļa opciju atsauce

Katra opcija, ko `nuxt-i18n-micro` pieņem, nolasīta būvēšanas laikā no interfeisa `ModuleOptions`
failā
[`packages/types/src/index.ts`](https://github.com/s00d/nuxt-i18n-micro/blob/main/packages/types/src/index.ts).
Nekas šeit nav rakstīts ar roku, tāpēc tas nevar atpalikt no koda — tipi, noklusējumi un
apraksti nāk no pašas deklarācijas.

Lai saprastu, ko šīs opcijas *nozīmē* kopā — kuras kombinācijas ir jēgpilnas, kas mainās, kad
jūs maināt stratēģiju, izstrādātus piemērus — lasiet [Konfigurācija](/guides/configuration). Šī
lapa ir izsmeļošs saraksts; tā ir skaidrojums.
...
Ligzdotās opcijas parādās zem to punkta ceļa, tāpēc `translationPayloads.mode` ir
`mode` atslēga objekta `translationPayloads` iekšpusē.

{/* generated:module-options — do not edit; run `pnpm run docs:generate` */}

## Lokāles un maršrutēšana

Kuras valodas eksistē un kā tās parādās URL.
...
Kur atrodas tulkojumu faili un kā tiek atrisinātas atslēgas.
...
Kā tulkojumi sasniedz pārlūku. [Veiktspēja](/guides/performance) skaidro, ko tie maina izpildlaikā.
...
Meta tagi, kas ģenerēti katrai lokalizētajai lapai.
...
Lokāles izvēle apmeklētājam, kurš to nav izvēlējies.
...
- [Veiktspēja](/guides/performance) — ko payload opcijas maina izpildlaikā
- [Pakotņu API](/api/packages/overview) — katras darbatelpas pakotnes eksportētais API
