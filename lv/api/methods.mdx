---
title: "Metodes"
description: "Visas nuxt-i18n-micro izpildlaika metodes."
---

# 🛠️ Metodes

Šī lapa dokumentē visas pieejamās metodes, ko nodrošina nuxt-i18n-micro. Metodes ir sakārtotas pēc funkcionalitātes vieglākai navigācijai.

## 📊 API pārskats

Katrs palīgs, ko spraudnis injicē, nolasīts no interfeisa `PluginsInjections` — tāpēc šis
saraksts nevar novirzīties no tā, kas ir faktiski pieejams.
...
<Info>
`$defineI18nRoute` un `$clearCache` nodrošina galvenais i18n spraudnis uz
`useNuxtApp()`, bet tos **neatgriež** komponējamā funkcija `useI18n()`, tāpēc tie augstāk esošajā tabulā nav.
(`$getI18nConfig` **ir** pieejams caur abām.) Skatiet to sadaļas
zemāk.
</Info>
...
## 🔄 Maršruta un lokāles pārslēgšana
...
### Izmantošana ar useI18n komponējamo funkciju
...
## 🧭 `useI18nLocale` komponējamā funkcija

Centralizētā komponējamā funkcija lokāles stāvokļa pārvaldībai. Izmantojiet to, nevis tieši manipulējiet ar `useState('i18n-locale')` vai `useCookie('user-locale')`.
...
| Metode                  | Apraksts                                                 |
| ----------------------- | ----------------------------------------------------------- |
| `setLocale(locale)`     | Atomiski iestata lokāli gan `useState`, gan sīkfailā        |
| `getLocale()`           | Atgriež pašreizējo lokāli no stāvokļa vai sīkfaila                 |
| `getPreferredLocale()`  | Atgriež lokāli, kas validēta pret `locales` sarakstu, vai `null`  |
| `isValidLocale(locale)` | Pārbauda, vai lokāles kods ir konfigurētajā `locales` sarakstā |
