...
`nuxt-i18n-micro` 由一组小型包构建而成，其中大多数可以独立使用 — `@i18n-micro/vue` 可用于纯 Vue 应用，`@i18n-micro/core` 可用于任意环境。

下方每一页都是从 API 快照生成的，[`pnpm run api:surface`](https://github.com/s00d/nuxt-i18n-micro/blob/main/docs/guide/maintenance-commands.md#api-surface) 会将其与 TypeScript 源代码进行核对，因此除非该参考发生变化或 CI 失败，否则导出项不能悄无声息地消失。
...
- [集成](/integrations/overview) — 在各框架中使用这些包的指南
