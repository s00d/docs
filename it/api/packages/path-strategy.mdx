...
description: "API pubblica del pacchetto @i18n-micro/path-strategy: risoluzione runtime dei path locale, redirect ed helper SEO, generati dal sorgente."
---

# `@i18n-micro/path-strategy`

38 export su 6 entry point.
...
<code>NoPrefixPathStrategy</code> — 36 members, identico a `NoPrefixPathStrategy` above.
<code>Strategy</code> — 36 members, identico a `NoPrefixPathStrategy` above.
...
<code>PrefixPathStrategy</code> — 36 members, identico a `PrefixPathStrategy` above.
<code>Strategy</code> — 36 members, identico a `PrefixPathStrategy` above.
...
<code>PrefixAndDefaultPathStrategy</code> — 37 members, identico a `PrefixAndDefaultPathStrategy` above.
<code>Strategy</code> — 37 members, identico a `PrefixAndDefaultPathStrategy` above.
...
<code>PrefixExceptDefaultPathStrategy</code> — 44 members, identico a `PrefixExceptDefaultPathStrategy` above.
<code>Strategy</code> — 44 members, identico a `PrefixExceptDefaultPathStrategy` above.
...
<code>PathStrategy</code> — 22 members, identico a `PathStrategy` above.
<code>PathStrategyContext</code> — 15 members, identico a `PathStrategyContext` above.
<code>ResolvedRouteLike</code> — 6 members, identico a `ResolvedRouteLike` above.
<code>RouteLike</code> — 6 members, identico a `RouteLike` above.
<code>RouterAdapter</code> — 3 members, identico a `RouterAdapter` above.
<code>SwitchLocaleOptions</code> — 1 members, identico a `SwitchLocaleOptions` above.
