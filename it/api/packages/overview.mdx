---
title: "API dei pacchetti"
description: "Indice di riferimento per ogni pacchetto del workspace @i18n-micro, con l'elenco di API e tipi esportati generati dagli snapshot di sorgente rispetto ai quali la CI effettua i controlli."
---

# API dei pacchetti

`nuxt-i18n-micro` è costruito a partire da un insieme di piccoli pacchetti, la maggior parte dei quali utilizzabili
da soli — `@i18n-micro/vue` in un'app Vue semplice, `@i18n-micro/core` ovunque.

Ogni pagina qui sotto è generata dallo snapshot API con cui
[`pnpm run api:surface`](https://github.com/s00d/nuxt-i18n-micro/blob/main/docs/guide/maintenance-commands.md#api-surface) confronta i
sorgenti TypeScript, quindi un export non può scomparire senza che questo riferimento cambi
o che la CI fallisca.

{/* generated:packages-index — do not edit; run `pnpm run docs:generate` */}

| Pacchetto | Entry point | Export |
| --- | --- | --- |
...
- [Integrazioni](/integrations/overview) — guide per usare questi pacchetti in ciascun framework
- [Opzioni del modulo](/api/module-options) — opzioni accettate dal modulo Nuxt
- [Metodi](/api/methods) — gli helper runtime disponibili all'interno dei componenti
