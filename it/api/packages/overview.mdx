...
description: "L'API esportata di ogni pacchetto del workspace, letta in fase di build dagli snapshot che la CI verifica rispetto ai sorgenti."
---

# API dei pacchetti

`nuxt-i18n-micro` è costruito da un insieme di piccoli pacchetti, la maggior parte dei quali è utilizzabile
autonomamente — `@i18n-micro/vue` in un'app Vue normale, `@i18n-micro/core` ovunque.

Ogni pagina sottostante è generata dallo snapshot API che
[`pnpm run api:surface`](https://github.com/s00d/nuxt-i18n-micro/blob/main/docs/guide/maintenance-commands.md#api-surface) verifica rispetto ai
...
- [Integrazioni](/integrations/overview) — guide per l'uso di questi pacchetti in ciascun framework
