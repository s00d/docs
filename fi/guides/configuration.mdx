...
description: "Kaikki nuxt-i18n-micro-moduulin asetukset, oletukset ja esimerkit."
---

# Konfiguraatioviite

Täydellinen viite jokaiselle `i18n`-avaimen alla olevalle asetukselle `nuxt.config`-tiedostossa. Tyypit ja oletusarvot vastaavat [`ModuleOptions`](https://github.com/s00d/nuxt-i18n-micro/blob/main/packages/types/src/index.ts)-tyyppiä paketissa `@i18n-micro/types`.

## Pikaviite

Jokainen asetus, jonka moduuli hyväksyy, `ModuleOptions`-tyypistä. Alla olevat osiot selittävät
Alla olevat osiot selittävät miten ne toimivat yhdessä;
[Moduulin asetusten viitteessä](/api/module-options) on täydelliset kuvaukset.
