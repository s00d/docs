...
description: "Jokaisen workspace-paketin viety API, luetaan käännösaikana snapshoteista, joita CI vertaa lähdekoodiin."
---

# Pakettien API:t

`nuxt-i18n-micro` on rakennettu joukosta pieniä paketteja, joista useimpia voi käyttää
itsenäisesti — `@i18n-micro/vue` tavallisessa Vue-sovelluksessa, `@i18n-micro/core` missä tahansa.

Jokainen alla oleva sivu generoidaan API-snapshotista, jota
[`pnpm run api:surface`](https://github.com/s00d/nuxt-i18n-micro/blob/main/docs/guide/maintenance-commands.md#api-surface) vertaa
TypeScript-lähdekoodiin, joten vienti ei voi kadota ilman että joko tämä viite muuttuu
tai CI epäonnistuu.

{/* generated:packages-index — do not edit; run `pnpm run docs:generate` */}

| Paketti | Sisääntulopisteet | Viennit |
| --- | --- | --- |
...
- [Integraatiot](/integrations/overview) — oppaat näiden pakettien käyttöön kussakin frameworkissa
- [Moduulin asetukset](/api/module-options) — Nuxt-moduulin hyväksymät asetukset
- [Metodit](/api/methods) — komponenttien sisällä käytettävissä olevat ajonaikaiset apurit
