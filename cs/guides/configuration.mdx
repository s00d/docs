...
description: "Každá volba modulu Nuxt I18n Micro s výchozími hodnotami, typy a příklady: locales, strategie, meta, adresář překladů a další."
---

# Reference konfigurace

Kompletní reference pro každou volbu pod klíčem `i18n` v `nuxt.config`. Typy a výchozí hodnoty odpovídají [`ModuleOptions`](https://github.com/s00d/nuxt-i18n-micro/blob/main/packages/types/src/index.ts) in `@i18n-micro/types`.
...
[Module Options reference](/api/module-options) obsahuje kompletní popisy.

{/* generated:options-index — do not edit; run `pnpm run docs:generate` */}

| Option | Type | Default | Účel |
| --- | --- | --- | --- |
...
Modul poskytuje rozsáhlé konfigurační volby pro přizpůsobení internacionalizačního nastavení.

### 🌍 Základní nastavení locale
...
Každý objekt locale podporuje:

| Vlastnost        | Typ       | Povinná  | Popis                                                 |
| ---------------- | --------- | -------- | ----------------------------------------------------- |
...
Když je zapnuto, používají se jen kořenové soubory překladů:
...
Když je zapnuto, modul během buildu emituje `.nuxt/i18n.d.ts` s typovanými klíči překladů. Nastavte na `false`, pokud preferujete netypované pomocníky nebo používáte externí generátor.

Tento vestavěný generátor je oddělen od volitelného [`@i18n-micro/types-generator`](/integrations/types-generator) balíčku, který může produkovat přísnější nebo samostatné soubory typů pro monorepos a CI.
...
Toto normálně konfigurujete přes [`defineI18nRoute`](/guides/custom-locale-routes) na stránkách místo v `nuxt.config`. Modul slučuje deklarace na úrovni stránky do této mapy v době buildu.
#### `routeDisableMeta`
...
Podobně jako u `routeLocales` se toto obvykle nastavuje v komponentách stránky, nikoli ručně v konfiguraci.
#### `experimental`
...
##### Pluralizace pro jednotlivé locale
...
**Proměnná prostředí**: `NUXT_I18N_APP_BASE_URL`

```typescript
apiBaseUrl: 'api/_locales'
```

Překlady se budou fetchovat z `/\{apiBaseUrl\}/\{routeName\}/\{locale\}/data.json` (e.g., `/api/_locales/index/en/data.json`).
#### `apiBaseClientHost`
...
**Proměnná prostředí**: `NUXT_I18N_APP_BASE_CLIENT_HOST`
...
**Proměnná prostředí**: `NUXT_I18N_APP_BASE_SERVER_HOST`
