---
title: "API balíčků"
description: "Referenční index každého balíčku ve workspace @i18n-micro s výpisem exportovaných API a typů generovaných ze snapshotů zdroje, proti kterým CI kontroluje."
---

# API balíčků

`nuxt-i18n-micro` je poskládaný ze sady malých balíčků, z nichž většina je použitelná i samostatně: `@i18n-micro/vue` v čisté Vue aplikaci, `@i18n-micro/core` kdekoli.

Každá stránka níže je generovaná z API snapshotu, který [`pnpm run api:surface`](https://github.com/s00d/nuxt-i18n-micro/blob/main/docs/guide/maintenance-commands.md#api-surface) porovnává s TypeScript zdroji. Export nemůže zmizet, aniž by se změnila tato reference nebo selhalo CI.

{/* generated:packages-index — do not edit; run `pnpm run docs:generate` */}

| Balíček | Vstupní body | Exporty |
| --- | --- | --- |
...
- [Integrace](/integrations/overview): průvodci použitím těchto balíčků v jednotlivých frameworcích
- [Volby modulu](/api/module-options): volby přijímané Nuxt modulem
- [Metody](/api/methods): runtime pomocníci dostupní uvnitř komponent
