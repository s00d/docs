---
title: "API balíčků"
description: "Exportované API každého balíčku ve workspace, načtené při buildu ze snapshotů, které CI kontroluje proti zdrojovému kódu."
---

# API balíčků

`nuxt-i18n-micro` je postaven ze sady malých balíčků, z nichž většinu lze použít
samostatně — `@i18n-micro/vue` v čisté Vue aplikaci, `@i18n-micro/core` kdekoli.

Každá stránka níže je generována z API snapshotu, který
[`pnpm run api:surface`](https://github.com/s00d/nuxt-i18n-micro/blob/main/docs/guide/maintenance-commands.md#api-surface) kontroluje proti
TypeScript zdrojům, takže žádný export nemůže zmizet, aniž by se změnila tato reference
nebo selhalo CI.
...
- [Integrace](/integrations/overview) — průvodci pro použití těchto balíčků v jednotlivých frameworcích
