...
description: "Tất cả các tùy chọn module nuxt-i18n-micro, mặc định và ví dụ."
---

# Tham chiếu cấu hình

Tham chiếu đầy đủ cho mỗi tùy chọn dưới khóa `i18n` trong `nuxt.config`. Các kiểu và mặc định khớp với [`ModuleOptions`](https://github.com/s00d/nuxt-i18n-micro/blob/main/packages/types/src/index.ts) trong `@i18n-micro/types`.

## Tham chiếu nhanh

Mọi tùy chọn module chấp nhận, từ kiểu `ModuleOptions`. Các phần bên dưới giải thích
Các phần bên dưới giải thích cách chúng hoạt động cùng nhau; 
[Tham chiếu tùy chọn Module](/api/module-options) có các mô tả đầy đủ.
