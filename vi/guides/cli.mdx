---
title: "Hướng dẫn nuxt-i18n-micro-cli"
description: "CLI để trích xuất, text-to-i18n và các quy trình dịch."
---
...
`nuxt-i18n-micro-cli` là một công cụ dòng lệnh được thiết kế để tinh giản quá trình bản địa hóa và quốc tế hóa trong các dự án Nuxt.js sử dụng module `nuxt-i18n-micro` (Nuxt I18n Micro). Nó cung cấp các tiện ích để trích xuất các khóa bản dịch từ mã nguồn của bạn, quản lý các tệp bản dịch, đồng bộ hóa các bản dịch trên các locale và tự động hóa quy trình dịch bằng cách sử dụng các dịch vụ dịch bên ngoài.

Hướng dẫn này sẽ hướng dẫn bạn cài đặt, cấu hình và sử dụng `nuxt-i18n-micro-cli` để quản lý hiệu quả các bản dịch trong dự án của bạn. Gói trên [npmjs.com](https://www.npmjs.com/package/nuxt-i18n-micro-cli).

## 🔧 Cài đặt và thiết lập

### 📦 Cài đặt nuxt-i18n-micro-cli

Cài đặt toàn cục hoặc như một phụ thuộc dev trong dự án Nuxt của bạn (khuyến nghị cho CI):
...
Dùng **`nuxt-i18n-micro-cli` ≥ 2.1.1** nếu dự án của bạn dùng thư mục `app/` của Nuxt 4 (`app/pages`, `app/components`, …). Các phiên bản CLI cũ chỉ quét `pages/` ở gốc dự án.

### 🛠 Khởi tạo trong dự án của bạn

Sau khi cài đặt, bạn có thể chạy các lệnh `i18n-micro` trong thư mục dự án Nuxt.js.

Đảm bảo dự án của bạn được thiết lập với `nuxt-i18n-micro` và có cấu hình cần thiết trong `nuxt.config.ts` (hoặc `nuxt.config.js`).

### 📄 Đối số phổ biến

- `--cwd`: Chỉ định thư mục làm việc hiện tại (mặc định là `.`).
- `--logLevel`: Đặt mức log (`silent`, `info`, `verbose`).
- `--translationDir`: Thư mục chứa các tệp bản dịch JSON (mặc định: `locales`).
...
| --------------- | --------------------------- | --------------------------------- |
| **Phát triển** | `extract`, `text-to-i18n`   | Tìm và trích xuất các khóa bản dịch |
| **Đồng bộ**        | `sync`, `validate`          | Đảm bảo tất cả các locale có cùng các khóa |
| **Dịch**   | `translate`, `stats`        | Tự động dịch các khóa thiếu       |
| **Bảo trì** | `clean`, `check-duplicates` | Giữ bản dịch gọn gàng            |
