...
description: "Ghi đè cấp trang cho các thẻ meta SEO và link i18n."
---

# Composable `useI18nHead`

`useI18nHead` cho phép bạn tùy chỉnh các thẻ SEO i18n **theo trang** mà không cần một meta plugin tùy chỉnh. Khi `meta: true`, module gộp input của bạn lên trên đầu ra của [`useLocaleHead`](/api/composables/use-locale-head).

Các trường hợp điển hình:

- **Bài viết CMS / blog** — chỉ một số locale được dịch; các URL thay thế khác nhau theo ngôn ngữ (các slug hoặc đường dẫn khác nhau).
- **Canonical động** — URL canonical phải khớp với URL bài viết từ API, không phải `$switchLocalePath`.
- **Open Graph bổ sung** — `og:title`, `og:description`, `og:image` cùng với `og:locale` / `og:url` được tự động tạo.
- **Kiểm soát một phần** — giữ canonical và `og:url` do module tạo, nhưng chỉ thay thế `hreflang` và `og:locale:alternate`.
...
Đăng ký các ghi đè cấp trang cho các thẻ head SEO i18n.
Được gộp lên trên đầu ra của `useLocaleHead` bởi plugin `02.meta`.
...
## Cách nó vừa vặn trong pipeline
