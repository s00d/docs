---
title: "API các gói"
description: "Chỉ mục tham chiếu cho mọi gói workspace @i18n-micro, liệt kê các API và kiểu đã xuất được sinh từ các ảnh chụp mã nguồn mà CI kiểm tra dựa trên."
---

# API các gói

`nuxt-i18n-micro` được xây dựng từ một tập hợp các gói nhỏ, hầu hết có thể dùng
độc lập. Ví dụ, `@i18n-micro/vue` trong ứng dụng Vue thuần, `@i18n-micro/core` ở bất cứ đâu.

Mỗi trang bên dưới được sinh từ ảnh chụp API mà
[`pnpm run api:surface`](https://github.com/s00d/nuxt-i18n-micro/blob/main/docs/guide/maintenance-commands.md#api-surface) kiểm tra dựa trên các
mã nguồn TypeScript. Vì vậy một xuất không thể biến mất mà không có tham chiếu này thay đổi
hoặc CI thất bại.
...
- [Tích hợp](/integrations/overview) — hướng dẫn dùng các gói này trong từng framework
- [Tùy chọn mô-đun](/api/module-options) — các tùy chọn mà mô-đun Nuxt chấp nhận
- [Phương thức](/api/methods) — các trợ giúp runtime có sẵn bên trong các thành phần
