---
title: "Tham chiếu tùy chọn Module"
description: "Mọi tùy chọn module chấp nhận, với kiểu, mặc định và mục đích — được đọc tại thời điểm build từ định nghĩa kiểu."
---

# Tham chiếu tùy chọn Module

Mọi tùy chọn `nuxt-i18n-micro` chấp nhận, được đọc tại thời điểm build từ interface `ModuleOptions`
trong
[`packages/types/src/index.ts`](https://github.com/s00d/nuxt-i18n-micro/blob/main/packages/types/src/index.ts).
Không có gì ở đây được viết bằng tay, nên nó không thể tụt hậu so với mã — các kiểu, mặc định và
mô tả đều đến từ chính khai báo.

Để biết ý nghĩa của các tùy chọn này khi *kết hợp* với nhau — các kết hợp nào có ý nghĩa, những gì thay đổi khi
bạn chuyển đổi chiến lược, các ví dụ thực tế — hãy đọc [Cấu hình](/guides/configuration). Trang
này là danh sách đầy đủ; trang kia là phần giải thích.
...
## Locale và định tuyến

Những ngôn ngữ nào tồn tại và cách chúng xuất hiện trong URL.
...
Các tệp bản dịch nằm ở đâu và các khóa được phân giải như thế nào.
...
Cách bản dịch đến trình duyệt. [Hiệu năng](/guides/performance) giải thích những gì thay đổi tại runtime.
...
Các thẻ meta được tạo cho mỗi trang đã bản địa hóa.
...
Chọn một locale cho khách truy cập chưa chọn.
...
Các phần của module bạn có thể tắt.
...
- [API gói](/api/packages/overview) — API được xuất của mỗi gói trong workspace
