...
description: "nuxt-i18n-micro を Storybook と併用します。"
---
...
**Nuxt**、**Storybook**、**nuxt-i18n-micro** をプロジェクトに統合すると、コンポーネント駆動の開発アプローチで堅牢かつローカライズされたアプリケーションを構築できます。このガイドは、これらのツールを組み合わせてセットアップするための、設定・ローカライズ・Storybook 統合を含む手順を段階的に解説します。

## 🛠 Nuxt の設定

Nuxt プロジェクトでローカライズと Storybook を有効化するために、`nuxt.config.ts` を次のように設定します:
...
Storybook を Nuxt と nuxt-i18n-micro に統合するには、`.storybook/main.ts` を設定し、`mergeTranslations` と `staticDirs` を追加します。
...
2. **[Nuxt 用 Storybook のドキュメント](https://storybook.nuxtjs.org/getting-started/setup)**  
   Storybook を Nuxt に導入するための公式ドキュメントです。詳細な手順とベストプラクティスを掲載しています。

3. **[StackBlitz のインタラクティブサンプル](https://stackblitz.com/~/github.com/s00d/nuxtjs-storybook-i18n-micro)**  
   Nuxt、Storybook、nuxt-i18n-micro の統合サンプルをブラウザ上で直接試せます。
