---
title: "パッケージ API"
description: "各ワークスペースパッケージがエクスポートする API を、CI がソースと突き合わせるスナップショットからビルド時に読み取ります。"
---

# パッケージ API

`nuxt-i18n-micro` は複数の小さなパッケージで構成されており、そのほとんどは単独で利用可能です — `@i18n-micro/vue` はプレーンな Vue アプリで、`@i18n-micro/core` はどこでも利用できます。

以下の各ページは、[`pnpm run api:surface`](https://github.com/s00d/nuxt-i18n-micro/blob/main/docs/guide/maintenance-commands.md#api-surface) が TypeScript ソースと突き合わせている API スナップショットから生成されています。そのため、このリファレンスの変更なしに、あるいは CI を失敗させずにエクスポートが消えることはありません。
...
- [メソッド](/api/methods) — コンポーネント内で利用可能なランタイムヘルパー
