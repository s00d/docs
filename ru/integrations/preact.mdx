...
description: "Используйте @i18n-micro/preact, чтобы получить хуки, компоненты и провайдер для переводов и роутинга по локалям в Preact-приложениях."
---
...
### Базовая настройка (Without Router)
...
### Setup With Router-адаптер (Wouter Example)
...
### Setup With Built-in Router-адаптер
...
### `createI18n(options: PreactI18nОпции)`
...
##### `tn(value: number, options?: Intl.NumberFormatОпции): string`
...
##### `td(value: Date | number | string, options?: Intl.DateTimeFormatОпции): string`
...
##### `tdr(value: Date | number | string, options?: Intl.RelativeTimeFormatОпции): string`
...
#### Router Методы (when routing strategy is provided)
...
### DevTools Integration (Vite Плагин)
...
### Using the Built-in Router-адаптер
...
## Creating Custom Router-адаптерs
...
#### Step 2: Implement Required Методы
...
### Example 2: Preact Router-адаптер
...
### Пример 3: Query Parameter-Based Locale Routing
...
#### Pattern 1: Minimal Adapter (Only Required Методы)
...
### Full Application Настройка with Wouter
