...
description: "Каждая опция, которую принимает модуль, с её типом, значением по умолчанию и назначением — считывается на этапе сборки из определения типов."
---

# Справочник опций модуля

Каждая опция, которую принимает `nuxt-i18n-micro`, считывается на этапе сборки из интерфейса `ModuleOptions`
в
[`packages/types/src/index.ts`](https://github.com/s00d/nuxt-i18n-micro/blob/main/packages/types/src/index.ts).
Ничто здесь не написано вручную, поэтому не может отставать от кода — типы, значения по умолчанию и
описания берутся непосредственно из объявления.

О том, что эти опции *означают* в совокупности — какие комбинации имеют смысл, что меняется при
переключении стратегии, разобранные примеры — читайте в разделе [Конфигурация](/guides/configuration). Эта
страница — исчерпывающий список; та — объяснение.
...
Вложенные опции указываются через точечную нотацию, например `translationPayloads.mode` — это
ключ `mode` внутри объекта `translationPayloads`.

{/* generated:module-options — do not edit; run `pnpm run docs:generate` */}

## Локали и маршрутизация

Какие языки существуют и как они отображаются в URL.
...
Где находятся файлы переводов и как разрешаются ключи.
...
Как переводы достигают браузера. В разделе [Производительность](/guides/performance) объясняется, что эти опции меняют во время выполнения.
...
- [Производительность](/guides/performance) — что опции payload'ов меняют во время выполнения
