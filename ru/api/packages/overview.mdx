---
title: "API пакетов"
description: "Справочный индекс каждого пакета рабочего пространства @i18n-micro со списком экспортируемых API и типов, сгенерированных из снапшотов исходников, которые проверяет CI."
---

# API пакетов

`nuxt-i18n-micro` собран из набора небольших пакетов, большинство из которых можно использовать самостоятельно: `@i18n-micro/vue` в обычном Vue-приложении, `@i18n-micro/core` где угодно.

Каждая страница ниже сгенерирована из снапшота API, с которым команда
[`pnpm run api:surface`](https://github.com/s00d/nuxt-i18n-micro/blob/main/docs/guide/maintenance-commands.md#api-surface) сверяет
TypeScript-исходники. Экспорт не может исчезнуть без изменения этого справочника или падения CI.
