---
title: "API-uri pentru pachete"
description: "API-ul exportat al fiecărui pachet din workspace, citit la build time din snapshoturile pe care CI le verifică față de sursă."
---

# API-uri pentru pachete

`nuxt-i18n-micro` este construit dintr-un set de pachete mici, dintre care majoritatea pot fi folosite pe
cont propriu — `@i18n-micro/vue` într-o aplicație Vue simplă, `@i18n-micro/core` oriunde.

Fiecare pagină de mai jos este generată din snapshot-ul API pe care
[`pnpm run api:surface`](https://github.com/s00d/nuxt-i18n-micro/blob/main/docs/guide/maintenance-commands.md#api-surface) îl verifică față de
sursele TypeScript, așa că un export nu poate dispărea fără ca fie această referință să se schimbe,
fie CI-ul să eșueze.

{/* generated:packages-index — do not edit; run `pnpm run docs:generate` */}

| Pachet | Entry points | Exporturi |
| --- | --- | --- |
...
- [Integrations](/integrations/overview) — ghiduri pentru utilizarea acestor pachete în fiecare framework
- [Module Options](/api/module-options) — opțiunile acceptate de modulul Nuxt
- [Methods](/api/methods) — helperele runtime disponibile în interiorul componentelor
