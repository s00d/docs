...
description: "A API exportada de cada pacote do workspace, lida em tempo de build a partir dos snapshots que o CI valida contra o código-fonte."
---

# APIs de pacotes

O `nuxt-i18n-micro` é construído a partir de um conjunto de pequenos pacotes, a maioria deles utilizável de forma independente — `@i18n-micro/vue` num app Vue puro, `@i18n-micro/core` em qualquer lugar.

Cada página abaixo é gerada a partir do snapshot da API que
[`pnpm run api:surface`](https://github.com/s00d/nuxt-i18n-micro/blob/main/docs/guide/maintenance-commands.md#api-surface) verifica contra o
código-fonte TypeScript, então um export não pode desaparecer sem que esta referência mude ou o CI falhe.

{/* generated:packages-index — do not edit; run `pnpm run docs:generate` */}

| Pacote | Entry points | Exports |
| --- | --- | --- |
