---
title: "APIs dos pacotes"
description: "Índice de referência para cada pacote do workspace @i18n-micro, listando as APIs e tipos exportados, gerado a partir dos snapshots de código-fonte contra os quais o CI verifica."
---

# APIs dos pacotes

O `nuxt-i18n-micro` é construído a partir de um conjunto de pacotes pequenos, a maioria dos quais pode ser usada isoladamente. `@i18n-micro/vue` em um app Vue puro, `@i18n-micro/core` em qualquer lugar.

Cada página abaixo é gerada a partir do snapshot de API que [`pnpm run api:surface`](https://github.com/s00d/nuxt-i18n-micro/blob/main/docs/guide/maintenance-commands.md#api-surface) verifica contra os fontes TypeScript, portanto um export não pode desaparecer sem que esta referência mude ou o CI falhe.

{/* generated:packages-index — do not edit; run `pnpm run docs:generate` */}

| Pacote | Pontos de entrada | Exports |
| --- | --- | --- |
...
- [Integrações](/integrations/overview) guias para usar esses pacotes em cada framework
- [Opções do módulo](/api/module-options) opções aceitas pelo módulo Nuxt
- [Métodos](/api/methods) os helpers de runtime disponíveis dentro dos componentes
