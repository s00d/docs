...
description: "Genera tipus TypeScript a partir de fitxers JSON de locale."
---

# 🔧 Types Generator (`@i18n-micro/types-generator`)

The `@i18n-micro/types-generator` package provides automatic TypeScript type generation for clau de traducciós. It analyzes your JSON fitxer de traducciós and generates strict types, enabling full type-safety and autocomplete support for clau de traducciós.

## 📖 Visió general

`@i18n-micro/types-generator` is an optional dev dependency that enhances your development experience by:

- 🔍 **Automatic Type Generation** - Scans JSON fitxer de traducciós and generates TypeScript types
- ✅ **Type Safety** - Prevents typos and invalid clau de traducciós at compile time
- 🎯 **Autocomplete Support** - Full IDE autocomplete for clau de traducciós
- 🔄 **Hot Reload** - Automatically regenerates types when fitxer de traducciós change
...
export default defineNuxtConfig({
  ...
  i18n: {
    ...
    translationDir: 'locales', // Path to your fitxer de traducciós
  },
})
```
...
The generator automatically includes keys from route-specific fitxer de traducciós:
