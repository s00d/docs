---
title: "Paquet Astro (`@i18n-micro/astro`)"
description: "i18n per a Astro amb @i18n-micro/astro."
---

# Paquet Astro (`@i18n-micro/astro`)
...
**Nota:** The `createAstroRouterAdapter` is now exported from the package, so you don't need to create your own router adapter file. However, you can still create a custom adapter if needed.
...
For Edge runtime environments, use `import.meta.glob` to load translations en temps de compilació:
...
- The `virtual:i18n-micro/config` module works in all environments (it's generated en temps de compilació)
...
The package uses a router adapter pattern to decouple i18n functionality from specific routing implementations. Això et permet:
...
**Nota:** The `createAstroRouterAdapter` is now exported from `@i18n-micro/astro`, so you don't need to create your own adapter file for basic use cases.
...
A router adapter is an implementation of the `I18nRoutingStrategy` interface that defines how i18n interacts with your routing system. Això et permet:
...
- **What is the locale per defecte behavior?** (include prefix, hide prefix)
...
- Empty paths (`/`)
- Root locale (locale per defecte - should it be in URL?)
- Invalid locales
- Query parameters and hash fragments

### Example 1: Subdomain-Based Locale Routing

Aquest exemple mostra how to implement locale routing using subdomains (e.g., `en.example.com`, `fr.example.com`):
...
Aquest exemple mostra how to integrate with a custom routing library:
...
1. **Always handle the locale per defecte**: Decide whether the locale per defecte should appear in URLs or be hidden.
...
| `redirectToDefault`  | `boolean`                                                  | ❌       | `false`          | Redirect to locale per defecte if not found                                            |
| `translationDir`     | `string`                                                   | ❌       | `'src/locales'`  | Directory path for fitxer de traducciós                                               |
...
1. The integration creates a virtual module `virtual:i18n-micro/config` with your configuration (available en temps de compilació)
...
3. **You must explicitly load translations** in your middleware:
   ...
   - **For Edge runtime**: Use `import.meta.glob` to load translations en temps de compilació (see Edge Runtime example below)
...
**Nota:** Per defecte, files in the `pages/` folder are treated as route-specific translations. The utility automatically maps them to route names based on the directory structure.
...
Formats a number according to the locale actual.
...
Formats a date according to the locale actual.
...
Checks if a clau de traducció exists.
...
| `redirectToDefault` | `boolean`             | ❌       | Redirect to locale per defecte if not found (default: `false`) |
| `routingStrategy`   | `I18nRoutingStrategy` | ❌       | Router adapter for routing features                        |
...
4. **Default Locale** - Falls back to the configured locale per defecte
...
Gets the locale actual from Astro context.
...
Gets the locale per defecte from Astro context.
...
- `keys`: **Recommended** - Array of clau de traducciós to include (e.g., `['islands.vue.title', 'islands.apples']`). If omitted, all route-specific translations are included, which may increase bundle size.

**Retorna:** `I18nClientProps` object with:
...
This package supports DevTools integration via the `@i18n-micro/devtools-ui` Vite plugin. See the [DevTools UI Package documentation](/integrations/devtools-ui) for details.
...
You can create type-safe clau de traducciós:
...
11. **Translation File Structure** - Organize fitxer de traducciós logically (by feature, route, or component)
