---
title: "Paquet Preact (`@i18n-micro/preact`)"
description: "i18n per a Preact amb @i18n-micro/preact."
---

# Paquet Preact (`@i18n-micro/preact`)
...
**Nota:** The `createPreactRouterAdapter` (aliased as `createBrowserHistoryAdapter`) is exported from the package and provides a basic History API-based adapter. For router-specific adapters (like wouter), you can create custom adapters as shown above.
...
The package uses a router adapter pattern to decouple i18n functionality from specific router implementations. Això et permet:
...
**Paràmetres:** the [shared `createI18n` options](/integrations/overview#create-i18n-options) — `locale`, `fallbackLocale`, `messages`, `plural`, `missingWarn` and `missingHandler`.
...
// Get localized path for locale actual
const path = localeRoute('/about') // "/fr/about" if locale actual is 'fr'
...
Aquest exemple mostra how to create an adapter for `wouter-preact`:
...
Aquest exemple mostra how to create an adapter for `preact-router`:
