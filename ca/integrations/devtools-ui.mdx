...
description: "UI de DevTools independent i plugin de Vite."
---
...
- **File Operations** - List, load, and save fitxer de traducciós via API
...
export default defineConfig({
  plugins: [
    ...
    i18nDevToolsPlugin({
      ...
      translationDir: 'src/locales', // Optional: fitxer de traducciós directory
...
Get list of all fitxer de traducciós and directory structure.
...
Load a specific fitxer de traducció.

**Query Parameters:**

- `path` - Relative path to the fitxer de traducció (e.g., `en.json` or `pages/home/en.json`)
...
- **Development Only** - The plugin only works en desenvolupament mode (`apply: 'serve'`)
- **No Production Impact** - The plugin is automatically excluded from production builds
- **Zero Runtime Overhead** - No code is included en producció bundles
