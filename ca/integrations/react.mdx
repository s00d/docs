---
title: "Paquet React (`@i18n-micro/react`)"
description: "i18n per a React amb @i18n-micro/react."
---

# Paquet React (`@i18n-micro/react`)
...
Aquest exemple mostra how to create an adapter for Next.js App Router:
...
export function createNextJsRouterAdapter(
  ...
  const resolvePath = (to: string | { path?: string }, locale: string): string | { path?: string } => {
    ...
    // Next.js App Router uses locale prefix for all non-locale per defectes
...
Aquest exemple mostra how to create an adapter for TanStack Router:
