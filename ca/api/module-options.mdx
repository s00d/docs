...
description: "Totes les opcions que accepta el mòdul, amb el seu tipus, valor per defecte i propòsit — llegides en temps de compilació a partir de la definició del tipus."
---

# Referència d'opcions del mòdul

Cada opció que accepta `nuxt-i18n-micro`, llegida en temps de compilació des de la interfície `ModuleOptions`
a
[`packages/types/src/index.ts`](https://github.com/s00d/nuxt-i18n-micro/blob/main/packages/types/src/index.ts).
Res d'aquí està escrit a mà, així que no pot quedar-se enrere respecte al codi — els tipus, els valors per defecte i
les descripcions provenen de la mateixa declaració.

Per saber què *signifiquen* aquestes opcions juntes —quines combinacions tenen sentit, què canvia quan
canvies d'estratègia, exemples treballats— llegeix [Configuració](/guides/configuration). Aquesta
pàgina és la llista exhaustiva; aquella és l'explicació.
...
Les opcions imbricades apareixen sota el seu camí amb punts, així que `translationPayloads.mode` és la
clau `mode` dins de l'objecte `translationPayloads`.

{/* generated:module-options — do not edit; run `pnpm run docs:generate` */}

## Locales i enrutament

Quines llengües existeixen i com apareixen a l'URL.
...
Com arriben les traduccions al navegador. [Rendiment](/guides/performance) explica què canvien en temps d'execució.
...
Meta tags generades per a cada pàgina localitzada.
...
Escollir un locale per a un visitant que no n'ha triat cap.
...
- [Configuració](/guides/configuration) — com funcionen aquestes opcions juntes
- [Rendiment](/guides/performance) — què canvien les opcions de payload en temps d'execució
- [APIs dels paquets](/api/packages/overview) — l'API exportada de cada paquet del workspace
