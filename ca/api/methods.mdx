---
title: "Mètodes"
description: "Tots els mètodes de runtime de nuxt-i18n-micro."
---
...
| [`$defaultLocale`](#defaultlocale) | `() => string \| undefined` | Code of the configured locale per defecte. |
...
| [`$loadPageTranslations`](#loadpagetranslations) | `(locale: string, routeName: string, translations: Translations) => Promise<void>` | Load translations for a page en temps d'execució, for content whose keys are not known en temps de compilació. |
...
| [`$mergeTranslations`](#mergetranslations) | `(newTranslations: Translations) => void` | Merge translations into the active locale en temps d'execució, overriding what is loaded. |
...
// Output: 'en' (assuming the locale actual is English)
```
...
Code of the configured locale per defecte.
...
Merge translations into the active locale en temps d'execució, overriding what is loaded.
...
- Custom error handling for missing clau de traducciós
...
<Tip>
See [FAQ — switch locale without changing the URL](/guides/faq#-switch-locale-without-changing-the-url). For prefix strategies, use `$switchLocalePath()` when you only need a link target.
</Tip>
...
// Switches to the given path with the locale actual
$switchRoute('/about')
```
...
// Switches to a named route with the locale actual
...
// Output: A route object with the locale actual applied, e.g., { name: 'index', params: { locale: 'fr' } }
```
...
- **localeRoutes**: `Record<string, string>` — Opciónal. Custom routes for specific locales
- **disableMeta**: `boolean | string[]` — Opciónal. Disables i18n meta tags for all or specific locales
...
Drop every cached chunk, al client and in the active dictionary.
...
Load translations for a page en temps d'execució, for content whose keys are not known en temps de compilació.
...
| `getLocale()`           | Returns locale actual from state or cookie                 |
