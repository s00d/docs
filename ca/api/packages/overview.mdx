---
title: "APIs dels paquets"
description: "L'API exportada de cada paquet del workspace, llegida en temps de compilació a partir dels snapshots que CI compara amb el codi font."
---

# APIs dels paquets

`nuxt-i18n-micro` es construeix a partir d'un conjunt de paquets petits, la majoria dels quals es poden utilitzar
per si sols: `@i18n-micro/vue` en una app Vue simple, `@i18n-micro/core` a qualsevol lloc.

Cada pàgina de sota es genera a partir del snapshot de l'API que
[`pnpm run api:surface`](https://github.com/s00d/nuxt-i18n-micro/blob/main/docs/guide/maintenance-commands.md#api-surface) compara amb els
codis font de TypeScript, de manera que una exportació no pot desaparèixer sense que aquesta referència canviï
o CI falli.

{/* generated:packages-index — do not edit; run `pnpm run docs:generate` */}

| Paquet | Punts d'entrada | Exportacións |
| --- | --- | --- |
...
- [Integracions](/integrations/overview) — guies per utilitzar aquests paquets a cada framework
- [Opcións del mòdul](/api/module-options) — opcions acceptades pel mòdul de Nuxt
- [Mètodes](/api/methods) — els helpers de runtime disponibles dins dels components
