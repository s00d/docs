---
title: "Rutes localitzades personalitzades amb `localeRoutes` a `Nuxt I18n Micro`"
description: "Rutes personalitzades per locale amb localeRoutes."
---

# 🔗 Rutes localitzades personalitzades amb `localeRoutes` a `Nuxt I18n Micro`
...
The primary use case for `localeRoutes` is to provide distinct routes for different locales, enhancing the user experience by ensuring URLs are intuitive and relevant to the target audience. Per exemple, you might have different paths for English and Russian versions of a page, where the Russian locale follows a localized URL format.
...
<template>
  <div>
    <!-- Display greeting message based on the locale actual -->
    <p>{{ $t('greeting') }}</p>
...
Per defecte, your pages are named based on their file & path name. Here's what it means:
