...
description: "Integra Nuxt I18n Micro amb Storybook: configura @nuxtjs/storybook, afegeix controls de locale i previsualitza components entre idiomes."
---
...
Integrar **Nuxt**, **Storybook** i **nuxt-i18n-micro** al teu projecte et permet construir una aplicació localitzada i robusta amb un enfocament de desenvolupament orientat a components. Aquesta guia ofereix un recorregut pas a pas sobre com configurar aquestes eines juntes, incloent-hi la configuració, la localització i la integració amb Storybook.

## 🛠 Configuració de Nuxt

Per activar la localització i Storybook al teu projecte Nuxt, configura el teu fitxer `nuxt.config.ts` així:
...
Per integrar Storybook amb Nuxt i nuxt-i18n-micro, configura el fitxer `.storybook/main.ts` i afegeix `mergeTranslations` i `staticDirs`.
...
Per veure exemples complets, consulta aquests projectes:
...
2. **[Documentació de Storybook per a Nuxt](https://storybook.nuxtjs.org/getting-started/setup)**  
   La documentació oficial per configurar Storybook amb Nuxt. Ofereix passos detallats i bones pràctiques.
