---
title: "Desplegament amb Firebase AppHosting"
description: "Desplega nuxt-i18n-micro a Firebase AppHosting."
---
...
- ✅ Translations work perfectly en desenvolupament (`nuxt dev`)
...
export default defineNuxtConfig({
  ...
  // Add route rules for fitxer de traducciós
  routeRules: {
    '/_locales/**': {
      headers: {
...
3. Click on a fitxer de traducció request
...
After building, check that fitxer de traducciós are generated:
...
export default defineNuxtConfig({
  ...
  i18n: {
    ...
    meta: true,
    metaBaseUrl: 'https://your-domain.com',
    debug: false, // Enable en desenvolupament if needed
  },
...
- [ ] Network tab shows `Content-Type: application/json` for fitxer de traducciós
...
4. Check that `/_locales/index/en/data.json` (or your locale per defecte) loads with:
   - Status: 200
   - Content-Type: application/json
...
**Solution:** Ensure fitxer de traducciós are being copied to the output directory:
...
1. **Always test en producció-like environment** before deploying
2. **Use debug mode** during initial setup to catch issues early
3. **Monitor cache hit rates** to optimize performance
4. **Keep fitxer de traducciós small** for faster loading
...
- Use route-specific fitxer de traducciós for page content
