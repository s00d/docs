---
title: "Guia de SEO per a Nuxt I18n Micro"
description: "Configuració de SEO per a llocs Nuxt multilingües."
---

# 🌐 Guia de SEO per a Nuxt I18n Micro
...
Per defecte, `og:locale` is derived from `iso` when the mapping is unambiguous (`en-US` → `en_US`).  
Set an explicit **`og`** string when you need a custom value (e.g. `zh-Hans` → `zh_CN`).  
If neither `og` nor a convertible `iso` is available, `og:locale` tags are not generated — en desenvolupament you get a console warning (respects `missingWarn`).
...
1. **🌍 Language and Direction Attributes**:
   - The module sets the `lang` and `dir` attributes on the `<html>` tag according to the locale actual and text direction (e.g., `ltr` for English or `rtl` for Arabic).
...
Per exemple, a request to `https://site-a.com/en/about` will produce:
...
Per defecte, query parameters are stripped from canonical and `og:url` to avoid duplicate content. You can whitelist specific query parameters that should be preserved:
...
També pots disable meta only for specific locales:
...
Nuxt Site Config reads optional clau de traducciós from your locale files:
