...
description: "Totes les opcions del mòdul nuxt-i18n-micro, valors per defecte i exemples."
---
...
| [`strategy`](/api/module-options) | `Strategies` | `'prefix_except_default'` | URL routing strategy for locale prefixes. - `'no_prefix'` — no locale in URL; locale stored in cookie. - `'prefix_except_default'` — prefix all locales except the default. - `'prefix'` — always prefix, including the locale per defecte. - `'prefix_and_default'` — like `prefix`, but the locale per defecte is also accessible without prefix. |
...
| [`translationPayloads.mode`](/api/module-options) | `'premerged' \| 'source'` | `'premerged'` | Translation payload strategy. - `premerged`: build-time `\{page\}/\{locale\}/data.json` matrix (default) - `source`: compact source files merged en temps d'execució (prefer on Edge / large catalogs) |
...
| [`autoDetectPath`](/api/module-options) | `string` | `'/'` | Where cookie / Accept-Language preference redirects may run (when `redirects` is enabled). - `'/'` — only `/` (deep links in the locale per defecte stay reachable; default) - `'no_prefix'` — only paths without a locale prefix - `'*'` — every path, including rewriting an explicit locale prefix   (e.g. `/fr/about` → `/de/about` when the cookie prefers `de`) - any other string — exact path match (e.g. `'/welcome'`) Prefixed strategy cleanup (e.g. `/en` → `/` under `prefix_except_default`) is not gated. |
| [`disableWatcher`](/api/module-options) | `boolean` | `false` | Disable the file watcher that auto-creates missing fitxer de traducciós en desenvolupament mode. |
| [`types`](/api/module-options) | `boolean` | `true` | Generate TypeScript type declarations for `useI18n`, `$t`, and related helpers based on the clau de traducciós in your locale per defecte files. |
| [`routesLocaleLinks`](/api/module-options) | `\{ [key: string]: string \}` | `\{\}` | Map route names to other route names to share the same fitxer de traducciós. |
| [`plural`](/api/module-options) | `PluralFunc` | `built-in pluralization (form index by count)` | Custom pluralization function. |
| [`disablePageLocales`](/api/module-options) | `boolean` | `false` | Disable per-page fitxer de traducciós. |
...
| [`missingWarn`](/api/module-options) | `boolean` | `true` | Show console warnings when a clau de traducció is missing. |
| [`hmr`](/api/module-options) | `boolean` | `true` | Enable Hot Module Replacement for fitxer de traducciós en desenvolupament. |
...
Per defecte, custom properties are typed as `unknown`. To get full TypeScript support, use **module augmentation**. Create a declaration file (e.g., `app/i18n.d.ts` or any `.d.ts` included in your `tsconfig`):
...
- `'prefix'` — always prefix, including the locale per defecte.
- `'prefix_and_default'` — like `prefix`, but the locale per defecte is also accessible without prefix.
...
// Both prefixed and non-prefixed versions for locale per defecte
```
</CodeGroup>
...
**Tipus** `string` · **Per defecte** `'locales'`
...
**Tipus** `boolean` · **Per defecte** `false`

Disable per-page fitxer de traducciós.
...
When enabled, only root-level fitxer de traducciós are used:
...
**Tipus** `string` · **Per defecte** `undefined (no fallback; returns the raw key)`

Global fallback locale code.
When a clau de traducció is missing in the active locale, the module looks it up
...
based on the clau de traducciós in your locale per defecte files.

{/* /generated:option:types */}

When enabled, the module emits `.nuxt/i18n.d.ts` during build with typed clau de traducciós. Set to `false` if you prefer untyped helpers or use an external generator.
...
**Tipus** `Record<string, string[]>` · **Per defecte** —
...
You normally configure this via [`defineI18nRoute`](/guides/custom-locale-routes) in pages rather than in `nuxt.config`. The module merges page-level declarations into this map en temps de compilació.
#### `routeDisableMeta`

{/* generated:option:routeDisableMeta — do not edit; run `pnpm run docs:generate` */}

**Tipus** `Record<string, boolean \| string[]>` · **Per defecte** —
...
**Tipus** `Record<string, unknown>` · **Per defecte** —
...
**Tipus** `string` · **Per defecte** `undefined`
...
(`useRequestURL().origin` al servidor with `X-Forwarded-Host` / `X-Forwarded-Proto`,
`window.location.origin` al client). Ideal for **SEO stacks** that already set `site.url`,
...
**Tipus** `string[]` · **Per defecte** `['page', 'sort', 'filter', 'search', 'q', 'query', 'tag']`
...
**Tipus** `GlobalLocaleRoutes` · **Per defecte** `\{\}`
...
**Tipus** `\{ [key: string]: string \}` · **Per defecte** `\{\}`

Map route names to other route names to share the same fitxer de traducciós.
Per exemple, `\{ 'about-us': 'about' \}` means the `about-us` page will use
...
**Tipus** `string \| RegExp` · **Per defecte** `undefined (uses built-in pattern based on locale codes)`
...
Disable the file watcher that auto-creates missing fitxer de traducciós en desenvolupament mode.
...
Show console warnings when a clau de traducció is missing.
...
<Tip>
You can set a custom handler for missing translations using `setMissingHandler` method. Això et permet send missing translation errors to error tracking services like Sentry.
</Tip>
...
- `'/'` — only `/` (deep links in the locale per defecte stay reachable; default)
...
**Tipus** `PluralFunc` · **Per defecte** `built-in pluralization (form index by count)`
...
<Danger>
The function is serialized via `.toString()` and injected into a virtual module en temps de compilació. This means:
...
helpers from another module are stripped and will be `undefined` en temps d'execució. Keep the
...
**Tipus** `string \| null` · **Per defecte** `null`
...
- **Obligatori** for redirect behavior in prefix strategies (when `redirects: true`)
#### `apiBaseUrl`

{/* generated:option:apiBaseUrl — do not edit; run `pnpm run docs:generate` */}

**Tipus** `string` · **Per defecte** `'_locales'`
...
**Tipus** `TranslationPayloadOptions` · **Per defecte** —
...
`mode: 'source'` keeps layer files compact and merges root/page/fallback en temps d'execució through the built-in `/_locales` route (or Edge `assets:i18n`). Per defecte it disables public asset copies and prerendered payload routes.

<Warning>
With `mode: 'source'`, `publicAssets` defaults to `false` (`prerenderRoutes` is always off unless you opt in). Pure static hosting without a Nitro/edge runtime therefore cannot load translations al client unless you enable one of these outputs, keep `serverHandler` available en temps d'execució, or host payloads externally.
...
També pots disable duplicated local outputs manually and serve translation payloads from a CDN or object storage:
...
**Tipus** `(string \| RegExp)[]` · **Per defecte** `undefined`
...
**Tipus** `string` · **Per defecte** `'localized-'`
...
**Tipus** `string \| number` · **Per defecte** —
...
Per defecte the module fingerprints your translation sources (SHA-256 over every
...
It falls back to `Date.now()` only when there are no fitxer de traducciós to hash.
...
**Tipus** `number` · **Per defecte** `31536000`
...
- Not applied en desenvolupament (`import.meta.dev`) so HMR is not fought by the browser cache
...
**Tipus** `Record<string, Record<string, Intl.NumberFormatOptions>>` · **Per defecte** —
...
**Tipus** `Record<string, Record<string, Intl.DateTimeFormatOptions>>` · **Per defecte** —
...
Enable Hot Module Replacement for fitxer de traducciós en desenvolupament.
When `true`, changes to JSON fitxer de traducciós trigger an automatic reload
...
export default defineNuxtConfig({
  i18n: {
    // Hot updates for fitxer de traducciós in dev mode
    hmr: true,
  },
})
```
...
- **`cacheTtl`** — ensures stale translations are eventually reloaded from storage. Useful for serverless environments or when translations change en temps d'execució.

**Formula for estimating max entries**: `number_of_locales × (number_of_pages + 1)`. Per exemple, 10 locales × 500 pages = ~5010 entries.
</Tip>
