---
title: "Guia de rendiment"
description: "Consells de rendiment i optimitzacions."
---
...
2. **Single-source lookup**: All translations (root + page-specific + fallback) are pre-merged en temps de compilació into a single file per page — no layered search needed
...
`mode: 'source'` keeps layer-merged source files compact and merges root/page/fallback en temps d'execució through the built-in `/_locales` route (or Edge `assets:i18n`). Per defecte it disables public asset copies and prerendered payload routes.

<Warning>
`prerenderRoutes` defaults to `false`. With `mode: 'source'`, `publicAssets` also defaults to `false`. Pure static hosting without a Nitro/edge runtime therefore cannot load translations al client unless you enable one of these outputs, keep `serverHandler` available en temps d'execució, or host payloads externally.
...
- 🗂️ **Use Page-Specific Translations**: Organize your fitxer de traducciós by page to avoid loading unnecessary data.
