...
description: "Her @i18n-micro çalışma alanı paketinin referans indeksi; CI'nin karşılaştırdığı kaynak anlık görüntülerinden üretilen dışa aktarılan API'ler ve türler."
---

# Paket API'leri

`nuxt-i18n-micro`, çoğu tek başına kullanılabilen bir dizi küçük paketten oluşturulmuştur. Örneğin `@i18n-micro/vue` düz bir Vue uygulamasında, `@i18n-micro/core` ise her yerde.

Aşağıdaki her sayfa, TypeScript kaynaklarına karşı [`pnpm run api:surface`](https://github.com/s00d/nuxt-i18n-micro/blob/main/docs/guide/maintenance-commands.md#api-surface)'in kontrol ettiği API anlık görüntüsünden üretilir. Böylece ya bu referans değişmeden ya da CI başarısız olmadan bir dışa aktarım kaybolamaz.

{/* generated:packages-index — do not edit; run `pnpm run docs:generate` */}

| Paket | Giriş noktaları | Dışa aktarımlar |
| --- | --- | --- |
...
- [Entegrasyonlar](/integrations/overview): bu paketleri her çerçevede kullanmak için kılavuzlar
- [Modül seçenekleri](/api/module-options): Nuxt modülünün kabul ettiği seçenekler
- [Metotlar](/api/methods): bileşenler içinde kullanılabilir çalışma zamanı yardımcıları
