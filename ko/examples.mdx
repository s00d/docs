---
title: "Nuxt I18n Micro 예제"
description: "실용적인 Nuxt I18n Micro 예제: 로케일 전환, <i18n-link>, <i18n-switcher>, <i18n-t>, 동적 키, 그리고 번역 등록."
---

# 📚 Nuxt I18n Micro 예제 및 사용법

이 섹션은 Nuxt.js 애플리케이션에서 `Nuxt I18n Micro`를 사용하는 다양한 예제를 제공합니다. 로케일 전환 방법, `<i18n-link>`, `<i18n-switcher>`, `<i18n-t>` 컴포넌트 사용법, 그리고 번역 키를 동적으로 처리하는 방법을 확인할 수 있습니다.
...
이 예제는 버튼을 사용하여 프로그래매틱하게 로케일을 전환하는 방법을 보여줍니다:
...
### `<i18n-switcher>` 컴포넌트 사용

`<i18n-switcher>` 컴포넌트는 사용자 정의 가능한 레이블과 함께 로케일 전환용 드롭다운을 제공합니다:
...
이 시나리오에서도 이전 예제와 동일한 JSON 파일을 사용할 수 있습니다.

## 📝 번역 파일에서 동적 키 렌더링

일부 시나리오에서는 번역 파일 내에 저장된 동적 키를 반복하며 그 값을 조건에 따라 렌더링하고 싶을 수 있습니다. 아래 예제는 `Nuxt I18n Micro`를 사용하여 이를 달성하는 방법을 보여줍니다.

### 예제: 동적 키 렌더링

이 예제는 특정 번역 경로(여기서는 `dynamic`)에서 키 배열을 가져와 반복합니다. 각 키는 `$has`로 존재 여부를 확인한 뒤 `$t`로 값을 렌더링합니다.
...
이 예제에서는 번역 파일 내에 저장된 객체를 처리합니다. 객체의 키를 가져와 반복하면서 키 이름과 관련 값을 동적으로 렌더링합니다.
...
## 🌟 슬롯 및 보간을 사용한 `<i18n-t>` 번역

`<i18n-t>` 컴포넌트는 동적 콘텐츠와 HTML 태그가 포함된 번역을 렌더링하는 데 유용합니다:
...
단일 페이지에서 여러 번역 사용을 보여주는 복잡한 구조입니다:
...
`Nuxt I18n Micro`의 `$tc` 함수는 개수와 로케일 설정에 따라 복수형을 처리합니다. 항목 수나 알림처럼 개수에 따라 달라지는 메시지를 동적으로 조정할 때 유용합니다.

### 예제: 복수형을 위한 `$tc` 사용

다음 예제에서는 `$tc`를 사용해 사과 개수를 나타내는 메시지를 표시합니다. 번역 키는 제공된 개수에 따라 여러 복수형을 처리합니다.
...
JSON 파일에서 서로 다른 복수형을 처리하도록 번역을 정의하는 방법입니다:
...
- **`$tc('apples', 0)`**: 개수가 0일 때 사용되는 첫 번째 형식을 반환합니다 (`"no apples"`).
- **`$tc('apples', 1)`**: 개수가 1일 때 사용되는 두 번째 형식을 반환합니다 (`"one apple"`).
- **`$tc('apples', 10)`**: 개수가 2 이상일 때 사용되는 개수 값이 포함된 세 번째 형식을 반환합니다 (`"10 apples"`).

### 더 복잡한 복수형 규칙 예제

애플리케이션이 더 복잡한 복수형 규칙(예: 0, 1, 2, 소수, 다수, 기타에 대한 특정 케이스)을 처리해야 한다면 번역 문자열을 그에 맞게 확장할 수 있습니다:
...
`$tn` 함수는 `Intl.NumberFormat` API를 사용하여 현재 로케일에 따라 숫자 형식을 지정합니다. 통화, 백분율 등 사용자의 지역 설정에 맞게 숫자를 표시할 때 유용합니다.
...
`$td` 함수는 `Intl.DateTimeFormat` API를 사용하여 현재 로케일에 따라 날짜와 시간의 형식을 지정합니다. 사용자에게 익숙한 형식으로 날짜와 시간을 표시할 때 유용합니다.
...
`$tdr` 함수는 `Intl.RelativeTimeFormat` API를 사용하여 현재 로케일에 따라 날짜를 상대적 시간("5 minutes ago", "2 days ago" 등)으로 표시합니다. 현재 날짜와 시간을 기준으로 시간 차이를 표시할 때 유용합니다.
