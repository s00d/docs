---
title: "모듈 옵션 참조"
description: "모듈이 허용하는 모든 옵션과 해당 타입, 기본값 및 목적 — 빌드 타임에 타입 정의에서 읽어옵니다."
---

# 모듈 옵션 참조

`nuxt-i18n-micro`가 허용하는 모든 옵션으로, [`packages/types/src/index.ts`](https://github.com/s00d/nuxt-i18n-micro/blob/main/packages/types/src/index.ts)의 `ModuleOptions` 인터페이스에서 빌드 타임에 읽어옵니다.
여기에 있는 것은 손으로 작성된 것이 아니므로 코드에 뒤처질 수 없습니다 — 타입, 기본값 및 설명은 선언 자체에서 옵니다.

이러한 옵션이 함께 *의미하는* 것 — 어떤 조합이 의미가 있는지, 전략을 변경할 때 무엇이 바뀌는지, 실제 예제 — 에 대해서는 [Configuration](/guides/configuration)을 읽어보세요. 이 페이지는 전체 목록이고, 그 페이지는 설명입니다.
...
중첩된 옵션은 점 표기법 아래에 나타나므로 `translationPayloads.mode`는 `translationPayloads` 객체 내부의 `mode` 키입니다.

{/* generated:module-options — do not edit; run `pnpm run docs:generate` */}

## 로케일 및 라우팅

어떤 언어가 존재하며 URL에 어떻게 표시되는지.
...
번역이 브라우저에 도달하는 방식. [Performance](/guides/performance)는 이러한 옵션이 런타임에 무엇을 변경하는지 설명합니다.
...
- [Configuration](/guides/configuration) — 이러한 옵션이 함께 작동하는 방식
- [Performance](/guides/performance) — 페이로드 옵션이 런타임에 무엇을 변경하는지
- [Package APIs](/api/packages/overview) — 모든 워크스페이스 패키지의 내보낸 API
