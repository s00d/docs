---
title: "구성 참조"
description: "모든 nuxt-i18n-micro 모듈 옵션, 기본값 및 예제."
---

# 구성 참조

`nuxt.config`의 `i18n` 키 아래에 있는 모든 옵션에 대한 완전한 참조입니다. 타입과 기본값은 `@i18n-micro/types`의 [`ModuleOptions`](https://github.com/s00d/nuxt-i18n-micro/blob/main/packages/types/src/index.ts)와 일치합니다.

## 빠른 참조

`ModuleOptions` 타입에서 모듈이 허용하는 모든 옵션입니다. 아래 섹션에서는 이러한 옵션이 어떻게 함께 작동하는지 설명합니다.
전체 설명은 [Module Options 참조](/api/module-options)를 확인하세요.
...
이 모듈은 국제화 설정을 맞춤화하기 위한 광범위한 구성 옵션을 제공합니다.
...
지원되는 로케일 목록입니다.
각 항목은 로케일 코드와 선택적 메타데이터(ISO, 방향, 표시 이름 등)를 정의합니다.
...
| 속성         | 타입      | 필수 | 설명                                           |
| ---------------- | --------- | -------- | ----------------------------------------------------- |
| `code`           | `string`  | ✅       | 고유 식별자(예: `'en'`)                      |
| `displayName`    | `string`  | ❌       | 스위처를 위한 사람이 읽을 수 있는 이름(예: `'English'`) |
| `iso`            | `string`  | ❌       | ISO 코드(예: `'en-US'`)                            |
| `dir`            | `string`  | ❌       | 텍스트 방향(`'ltr'` 또는 `'rtl'`)                   |
| `disabled`       | `boolean` | ❌       | `true`이면 드롭다운에서 비활성화                         |
| `baseUrl`        | `string`  | ❌       | 로케일별 도메인의 base URL                  |
