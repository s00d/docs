...
Nuxt I18n Micro 支持服务端翻译与语言信息读取,让你可以在服务端翻译内容并访问语言详情。这在需要在返回客户端之前完成本地化的 API 或服务端渲染应用中尤为有用。

翻译使用 Nuxt I18n 配置中定义的语言消息,并根据检测到的语言动态解析。

默认情况下(`translationPayloads.mode: 'premerged'`),根级文件会在构建期被烘焙到每个页面的 payload 中,形式为 `\{page\}/\{locale\}/data.json`。服务端加载的是与客户端在 `/\{apiBaseUrl\}/...` 下获取相同的预构建文件,因此所有键(共享的和页面级的)在服务端处理函数中都可用。

在 `translationPayloads.mode: 'source'` 下,当调用 `loadTranslationsFromServer()`(或 `/_locales` 路由)时,紧凑的源文件会在运行时合并——在 **Edge** 上通过 Nitro `serverAssets`,在 **Node** 上从可选的公共副本读取。详情请参见 [性能指南 —— Serverless Payload 输出](/guides/performance#serverless-payload-output) 与 [缓存与存储架构](/api/cache)。

有关运行时与服务端 API 的完整列表,请参见 [API 参考](/api/overview)。

## 🛠️ 配置服务端中间件

要启用服务端翻译与语言信息,请使用模块提供的中间件函数:

- `useTranslationServerMiddleware` —— 用于翻译内容
- `useLocaleServerMiddleware` —— 用于访问语言信息

这两个中间件函数会在所有服务端路由中自动可用。

语言检测逻辑通过 `detectCurrentLocale` 工具函数在两个中间件之间共享,以确保模块内的一致性。

## ✨ 在服务端处理函数中使用翻译

你可以在任何 `eventHandler` 中通过翻译中间件无缝地翻译内容。
...
- 用户的语言会自动从查询参数、Cookie 或请求头中检测出来。
- `t` 函数会针对检测到的语言取回对应的翻译。
...
语言中间件会返回一个 `LocaleInfo` 对象,包含以下属性:
...
若需要手动指定语言(例如测试或特定请求),可将其传给翻译中间件:
...
5. **fallback 语言**:模块选项中配置的
6. **默认语言**:模块选项中配置的
7. **硬编码兜底**:`'en'`

> **注意**:语言检测逻辑在 `useLocaleServerMiddleware` 与 `useTranslationServerMiddleware` 之间共享,以保证模块内的一致性。
...
你可以将语言信息与翻译中间件结合使用,以获得完整的国际化能力:
...
2. **提供 fallback**:在检测失败时给出合理的默认值
3. **缓存语言信息**:对性能敏感的应用可考虑缓存检测结果
4. **处理边界情况**:对不受支持的语言给出合理的错误响应
5. **与翻译组合使用**:将语言信息与翻译中间件结合,以获得完整的 i18n 支持
