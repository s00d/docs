...
description: "面向多语言 Nuxt 站点的 SEO 配置。"
---
...
有效的 SEO(搜索引擎优化)对于确保你的多语言站点能被全球用户通过搜索引擎访问和发现至关重要。`Nuxt I18n Micro` 通过自动生成必要的 meta 标签与属性,告知搜索引擎站点的结构与内容,从而简化多语言站点的 SEO 管理。

本指南将说明 `Nuxt I18n Micro` 如何在无需额外配置的情况下处理 SEO,以提升站点的可见性与用户体验。
...
启用 `Nuxt I18n Micro` 中的 `meta` 选项时,模块会自动管理以下 SEO 相关内容:
...
### 使用 `$setI18nRouteParams` 处理各语言不同的 slug
