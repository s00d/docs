...
description: "在 Storybook 中使用 nuxt-i18n-micro。"
---
...
将 **Nuxt**、**Storybook** 与 **nuxt-i18n-micro** 集成到项目中,可以让你以组件驱动的方式构建一个健壮、本地化的应用。本指南将逐步演示如何将这些工具协同配置,包括基础配置、本地化以及 Storybook 集成。

## 🛠 Nuxt 配置

要在 Nuxt 项目中启用本地化和 Storybook,可按如下方式配置 `nuxt.config.ts` 文件:
...
要将 Storybook 与 Nuxt 及 nuxt-i18n-micro 集成,请配置 `.storybook/main.ts` 文件,并添加 `mergeTranslations` 与 `staticDirs`。
...
1. **[GitHub 上的 Nuxt + Storybook + i18n 示例](https://github.com/s00d/nuxtjs-storybook-i18n-micro)**  
   该仓库演示了如何集成 Nuxt、Storybook 与 nuxt-i18n-micro,并附带安装说明与配置细节。

2. **[Storybook for Nuxt 官方文档](https://storybook.nuxtjs.org/getting-started/setup)**  
   Storybook 与 Nuxt 集成的官方文档,提供了详细步骤与最佳实践。

3. **[StackBlitz 上的交互式示例](https://stackblitz.com/~/github.com/s00d/nuxtjs-storybook-i18n-micro)**  
   直接在浏览器中体验 Nuxt、Storybook 与 nuxt-i18n-micro 的集成示例。
