---
title: "方法"
description: "nuxt-i18n-micro 的所有运行时方法。"
---

# 🛠️ 方法

本页记录了 nuxt-i18n-micro 提供的所有可用方法。方法按功能进行分组,便于查阅。

## 📊 API 总览

插件注入的每个辅助方法都从 `PluginsInjections` 接口读取——因此该列表
不会与实际可用内容出现偏差。
...
<Info>
`$defineI18nRoute` 与 `$clearCache` 由主 i18n 插件通过
`useNuxtApp()` 提供,但**不会**由 `useI18n()` composable 返回,因此不在上面的表格中。
(`$getI18nConfig` 则**可以**通过两者获得。) 具体请参阅下面对应章节。
</Info>
...
用于集中管理语言状态的 composable。请使用它,而非直接操作 `useState('i18n-locale')` 或 `useCookie('user-locale')`。
