---
title: "快速开始"
description: "安装并配置 Nuxt I18n Micro。"
---
...
<Tip>
- **根级文件**(`locales/en.json` 等)——在整个应用中共享的翻译(菜单、页脚、通用 UI)。当 `translationPayloads.mode: 'premerged'`(默认)时,它们会在构建期合并进每个页面。
- **页面级文件**(`locales/pages/<route>/<locale>.json`)——特定页面的独有翻译,仅在访问该页面时加载。
- **动态路由**——`pages/articles/[id].vue` 映射到 `locales/pages/articles-id/`(方括号替换为短横线)。
- **自动生成**——所有缺失的翻译文件都会在 `nuxt dev` 时自动创建。
</Tip>
...
- **翻译**:默认将 `locales/` 放在**项目根目录**(推荐做法),而不是放在 `app/` 内
...
可选的同目录布局:`i18n: \{ translationDir: 'app/locales' \}`(参见 [fixture `n3`](https://github.com/s00d/nuxt-i18n-micro/tree/main/test/fixtures/n3))。

若需使用可选的 CLI 辅助工具 `text-to-i18n`,请使用 [`nuxt-i18n-micro-cli` ≥ 2.1.1](/guides/cli#-text-to-i18n-command),这样 `app/pages` 和 `app/components` 会从项目根目录被扫描。
</Info>
...
在完成基础配置后,可继续探索以下进阶主题:

- **[路由策略](/guides/routing-strategies)** —— 语言前缀和重定向的工作方式
