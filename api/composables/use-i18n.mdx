...
description: "Access every nuxt-i18n-micro runtime helper in components and scripts, with both dollar-prefixed and bare aliases like t, tc, and switchLocale."
---
...
The same functions the plugin injects as `$t`, `$tc`, and so on, available under both the dollar-prefixed name and a bare alias. So `const { t } = useI18n()` and `const { $t } = useI18n()` both work.
...
Every helper it returns (`$t`, `$tc`, `$has`, `$switchLocale`, and the rest) is listed with its signature and documented in full on the [methods reference](/api/methods).
