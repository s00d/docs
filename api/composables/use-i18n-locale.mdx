...
description: "Centralized locale state, cookies, sync utilities, and validation helpers for building custom language detection flows on top of nuxt-i18n-micro."
---

`useI18nLocale` is the centralized entry point for i18n locale management. It combines `useState('i18n-locale')`, the locale cookie, and sync utilities behind one object.
...
<Tip>
**`setLocale()` vs `$switchLocale()`**

`setLocale()` updates locale preference (state + cookie) only. To also reload the active translation context on the client, use `$switchLocale()` from `useNuxtApp()` or `useI18n()`. This matters especially for `no_prefix`, where the URL does not change. See [FAQ → switch locale without changing the URL](/guides/faq).
</Tip>

## See also

- [Custom language detection](/guides/custom-language-detection) for configuring auto-detection
- [Routing strategies](/guides/routing-strategies) for routing strategies and locale priority
- [Methods reference](/api/methods) for overlap with the runtime plugin API
