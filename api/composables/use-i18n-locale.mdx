...
description: "The useI18nLocale composable exposes centralized locale state, cookie sync, and custom locale detection hooks for Nuxt I18n Micro projects."
---
...
<Tip>
`setLocale()` updates locale preference (state + cookie) only. To also reload the active translation context on the client, use `$switchLocale()` from `useNuxtApp()` or `useI18n()`. This matters especially for `no_prefix`, where the URL does not change. See [FAQ — switch locale without changing the URL](/guides/faq#-switch-locale-without-changing-the-url).
</Tip>

## See Also

- [Custom Language Detection](/guides/custom-language-detection) — configuring auto-detection
- [Strategy](/guides/routing-strategies) — routing strategies and locale priority
