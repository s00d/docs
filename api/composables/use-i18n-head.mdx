...
description: "Per-page overrides for i18n SEO meta and link tags in nuxt-i18n-micro, including custom canonical URLs and disabled hreflang entries."
---

`useI18nHead` lets you customize i18n SEO tags **per page** without a custom meta plugin. When `meta: true`, the module merges your input on top of [`useLocaleHead`](/api/composables/use-locale-head) output.

Typical cases:

- **CMS or blog articles** where only some locales are translated and alternate URLs differ per language (different slugs or paths).
- **Dynamic canonical** where the canonical URL must match the article URL from the API rather than `$switchLocalePath`.
- **Extra Open Graph** such as `og:title`, `og:description`, `og:image` alongside auto-generated `og:locale` and `og:url`.
- **Partial control** to keep module-generated canonical and `og:url` while replacing only `hreflang` and `og:locale:alternate`.

## Signature

```ts
useI18nHead(input?: MaybeRefOrGetter<I18nHeadInput | null>): {
  pageHead: any
  resetPageHead: () => void
}
```

Register page-level overrides for i18n SEO head tags. Merged on top of `useLocaleHead` output by the `02.meta` plugin.
...
If you previously resolved a "public origin" composable for canonical or alternate absolute URLs, use module config instead:
...
Then pass **paths or full URLs** in `replace.hreflang` or `replace.canonical`. The module uses `useRequestURL` with `X-Forwarded-Host` and `X-Forwarded-Proto` on SSR.
...
- [SEO guide](/guides/seo) for automatic meta generation and page-level overrides
- [`useLocaleHead`](/api/composables/use-locale-head) for the low-level head API
