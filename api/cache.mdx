...
description: "How translation payloads are cached across client, Nitro server, and browser with the TranslationStorage singleton in nuxt-i18n-micro v3."
---
...
### Deterministic cache busting with `i18n.dateBuild`
...
Behavior: SSR loads from `#i18n-internal/payload-source`. On **Node**, `readFile` under `public/<apiBaseUrl>` (`{page}/{locale}/data.json` in premerged mode). On **Edge**, `assets:i18n`. `premerged` reads one file; `source` merges root/page/fallback via `@i18n-micro/utils/source-loader`.
...
Dictionaries are **not** written into `nuxtApp.payload`. The server keeps them in memory for SSR render; the client plugin `await`s `switchContext`, which loads `/{apiBaseUrl}/:page/:locale/data.json` (same default posture as `@nuxtjs/i18n` with `experimental.preload: false`).
...
The in-memory dictionary honors two module options:

- `cacheMaxSize` — maximum number of entries. `0` means no limit.
- `cacheTtl` — time-to-live in seconds. `0` means entries never expire.

Both apply to the per-page dictionary the client holds after a route switch. See [module options → Other](/api/module-options#other).
...
When `hmr: true` (the default in development), changes to translation JSON files under `translationDir` invalidate both:

- the process-global server cache (`SERVER_CC_KEY`), and
- the client-side `TranslationStorage` singleton, via a Vite HMR channel.

Result: an edit to `locales/en.json` reflects in the running app without a full page reload. HMR is disabled in production regardless of this setting.
...
- [Performance guide](/guides/performance): how caching impacts performance.
- [Module options → payloads and caching](/api/module-options#payloads-and-caching): the config options that shape this pipeline.
- [`$clearCache`](/api/methods#clearcache): flush the cache from client code.
