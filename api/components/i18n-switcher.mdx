...
description: "Customizable dropdown component for switching between locales in Vue, with slots, custom labels, and full inline-style theming controls."
---
...
<Note>
Locales with `disabled: true` are **omitted** from the dropdown. They remain available via `$getLocales()` for SEO and head tags. Only switchable locales get `NuxtLink` + `hreflang`.
</Note>
...
Every part is unstyled by default. The `custom*Style` props take inline style objects, and the slots below replace the markup entirely when styling is not enough.
...
Every visual part of the switcher can be replaced. The slots are listed with the props table above; each one takes template content:
...
Slots whose name ends in `-item`, `-link-content`, or starts with `before-item` receive the `locale` object they belong to.
