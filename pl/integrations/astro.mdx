...
### Konfiguracja With Router Adapter
...
**Uwaga:** The `createAstroRouterAdapter` is now exported from the package, so you don't need to create your own router adapter file. However, you can still create a custom adapter if needed.
...
**Uwaga:** The `createAstroRouterAdapter` is now exported from `@i18n-micro/astro`, so you don't need to create your own adapter file for basic use cases.
...
### Przykład 1: Subdomain-Based Locale Routing
...
### Przykład 2: Query Parameter-Based Locale Routing
...
### Przykład 3: Suffix-Based Locale Routing
...
### Przykład 4: Integration with Third-Party Router
...
**Uwaga:** By default, files in the `pages/` folder are treated as route-specific translations. The utility automatically maps them to route names based on the directory structure.
...
Prepares translation props for islands. **Ważne:** Always pass specific keys to minimize bundle size.
...
### Przykład: Complete Islands Page
...
**Ważne:** Types are automatically available when you add the integration to your `astro.config.mjs`. If you're not using the integration (only middleware), you may need to manually add the type reference (see Manual Type Reference below).
