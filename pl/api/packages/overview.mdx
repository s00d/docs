---
title: "API pakietów"
description: "Indeks dokumentacji dla każdego pakietu workspace @i18n-micro, wymieniający eksportowane API i typy generowane ze snapshotów źródeł, z którymi CI sprawdza zgodność."
---

# API pakietów

`nuxt-i18n-micro` jest zbudowany z zestawu małych pakietów, z których większość można używać samodzielnie — `@i18n-micro/vue` w zwykłej aplikacji Vue, `@i18n-micro/core` wszędzie.

Każda strona poniżej jest generowana ze snapshotu API, z którym
[`pnpm run api:surface`](https://github.com/s00d/nuxt-i18n-micro/blob/main/docs/guide/maintenance-commands.md#api-surface) porównuje źródła TypeScript, więc eksport nie może zniknąć bez zmiany tej dokumentacji lub bez pęknięcia CI.

{/* generated:packages-index — do not edit; run `pnpm run docs:generate` */}

| Pakiet | Punkty wejścia | Eksporty |
| --- | --- | --- |
...
- [Integracje](/integrations/overview) — przewodniki po używaniu tych pakietów w każdym frameworku
- [Opcje modułu](/api/module-options) — opcje akceptowane przez moduł Nuxt
- [Metody](/api/methods) — helpery runtime dostępne wewnątrz komponentów
