---
title: "API pakietów"
description: "Eksportowane API każdego pakietu w workspace, czytane w czasie budowania z migawek CI porównywanych ze źródłem."
---

# API pakietów

`nuxt-i18n-micro` jest zbudowany z zestawu małych pakietów, z których większości można używać samodzielnie — `@i18n-micro/vue` w zwykłej aplikacji Vue, `@i18n-micro/core` w dowolnym miejscu.

Każda strona poniżej jest generowana ze migawki API, którą
[`pnpm run api:surface`](https://github.com/s00d/nuxt-i18n-micro/blob/main/docs/guide/maintenance-commands.md#api-surface) porównuje ze źródłami TypeScript, więc żaden eksport nie może zniknąć bez zmiany tej dokumentacji lub błędu w CI.
...
- [Metody](/api/methods) — helpery runtime dostępne w komponentach
