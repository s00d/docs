---
title: "Pakke-API'er"
description: "Referenceindeks for hver @i18n-micro workspace-pakke med eksporterede API'er og typer, genereret fra kildesnapshots, som CI tjekker imod."
---

# Pakke-API'er

`nuxt-i18n-micro` er bygget af et sæt små pakker, hvoraf de fleste kan bruges alene — `@i18n-micro/vue` i en ren Vue-app, `@i18n-micro/core` overalt.

Hver side nedenfor er genereret ud fra API-snapshottet, som [`pnpm run api:surface`](https://github.com/s00d/nuxt-i18n-micro/blob/main/docs/guide/maintenance-commands.md#api-surface) tjekker mod TypeScript-kilderne. En eksport kan derfor ikke forsvinde, uden at enten denne reference ændres, eller CI fejler.

{/* generated:packages-index — do not edit; run `pnpm run docs:generate` */}

| Pakke | Indgangspunkter | Eksporter |
| --- | --- | --- |
...
- [Integrationer](/integrations/overview) — guides til brug af disse pakker i hvert framework
- [Modulindstillinger](/api/module-options) — indstillinger, Nuxt-modulet accepterer
- [Metoder](/api/methods) — runtime-hjælperne, der er tilgængelige i komponenter
