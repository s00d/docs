---
title: "Pakke-API'er"
description: "Den eksporterede API for hver workspace-pakke, læst på build-tid fra snapshots CI tjekker mod kildekoden."
---

# Pakke-API'er

`nuxt-i18n-micro` er bygget af et sæt små pakker, hvoraf de fleste kan bruges
selvstændigt — `@i18n-micro/vue` i en almindelig Vue-app, `@i18n-micro/core` hvor som helst.

Hver side nedenfor er genereret fra API-snapshottet, som
[`pnpm run api:surface`](https://github.com/s00d/nuxt-i18n-micro/blob/main/docs/guide/maintenance-commands.md#api-surface) tjekker mod
TypeScript-kilderne, så en eksport kan ikke forsvinde uden at enten denne reference ændres,
eller CI fejler.

{/* generated:packages-index — do not edit; run `pnpm run docs:generate` */}

| Pakke | Entry points | Eksporter |
| --- | --- | --- |
...
- [Moduloptioner](/api/module-options) — options accepteret af Nuxt-modulet
- [Metoder](/api/methods) — runtime-hjælperne tilgængelige inde i komponenter
