---
title: "Пакет Vue (`@i18n-micro/vue`)"
description: "Використовуйте @i18n-micro/vue, щоб додати композабли, компоненти та плагін для перекладів і locale-aware маршрутизації у самостійних застосунках Vue 3."
---

# Vue Package (`@i18n-micro/vue`)

Пакет `@i18n-micro/vue` надає легке та високопродуктивне рішення інтернаціоналізації для застосунків Vue 3. Він поділяє ту саму ядерну логіку, що й Nuxt I18n Micro, пропонуючи реактивні переклади, підтримку посайтових перекладів та повну підтримку TypeScript.

## Огляд

`@i18n-micro/vue` розроблено для застосунків Vue 3, яким потрібна інтернаціоналізація без Nuxt-фреймворка. Він надає:

- **Легкість** — використовує спільну ядерну логіку з `@i18n-micro/core`
- **Реактивність** — автоматичні оновлення компонентів при зміні перекладів
...
- **Незалежність від маршрутизатора** — працює з будь-яким маршрутизатором або без нього
- **Інтеграція з DevTools** — вбудована підтримка Vue DevTools для керування перекладами
...
Пакет потребує Vue 3 та опційно Vue Router:
...
### Базове налаштування (без маршрутизатора)
...
### Налаштування з адаптером маршрутизатора
...
## Інтеграція з маршрутизатором

### Інтерфейс I18nRoutingStrategy
...
### Створення адаптера маршрутизатора
...
### Задання стратегії маршрутизації
...
### Кастомний адаптер маршрутизатора
