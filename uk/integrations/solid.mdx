---
title: "Пакет Solid (`@i18n-micro/solid`)"
description: "Використовуйте @i18n-micro/solid, щоб додати сигнали, компоненти та провайдер для перекладів і locale-aware маршрутизації у застосунках SolidJS."
---

# Solid Package (`@i18n-micro/solid`)

Пакет `@i18n-micro/solid` надає легке та високопродуктивне рішення інтернаціоналізації для застосунків SolidJS. It shares the same core logic as Nuxt I18n Micro, offering reactive translations, route-specific support, and full TypeScript support.
...
- **Реактивність** — автоматичні оновлення компонентів при зміні перекладів using SolidJS signals
...
- **Незалежність від маршрутизатора** — працює з будь-яким маршрутизатором або без нього via `I18nRoutingStrategy` interface
- **Інтеграція з DevTools** — вбудована підтримка DevTools для керування перекладами
