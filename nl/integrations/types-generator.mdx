...
description: "Gebruik @i18n-micro/types-generator om TypeScript-types voor je vertaalsleutels te genereren uit locale-JSON-bestanden."
---

# 🔧 Types Generator (`@i18n-micro/types-generator`)

De `@i18n-micro/types-generator` package biedt eenutomatic TypeScript type generation for vertaalsleutels. It analyzes your JSON vertaalbestands and generates strict types, enabling full type-safety and autocomplete support for vertaalsleutels.

## 📖 Overzicht

`@i18n-micro/types-generator` is an optional dev dependency that enhances your development experience by:

- 🔍 **Automatic Type Generation** - Scans JSON vertaalbestands and generates TypeScript types
- ✅ **Type Safety** - Prevents typos and invalid vertaalsleutels at compile time
- 🎯 **Autocomplete Support** - Volledig IDE autocomplete for vertaalsleutels
- 🔄 **Hot Reload** - Automatically regenerates types when vertaalbestands change
- 🚀 **Zero Runtime Overhead** - Types are compile-time only, no impact on bundle size

## 🚀 Installation

Install het pakket as a dev dependency:
...
Add de module to your `nuxt.config.ts`:
...
The generator will automatisch:

- Scan all JSON files in your `locales` directory
- Generate types in `.nuxt/i18n-micro.d.ts`
- Watch for changes and regenerate types automatisch

### For Vue/Vite Projects

Add de plugin to your `vite.config.ts`:
...
The generator ondersteunt both flat and nested translation structures:
...
Sometimes you need to use dynamic keys (bijv., based on user input or API responses). The generator biedt een `ScopedKey` helper type for safe dynamic key usage.
...
3. **Philosophy** - i18n-micro is ontworpen om be lichtgewicht and fast
...
De `ScopedKey<Scope>` type stelt je in staat om narrow dynamic keys by prefix:
...
The generator automatisch includes keys from route-specific vertaalbestands:
...
1. **Regenerate types** - The generator should auto-regenerate, but you can handmatig trigger it
...
The generated file uses TypeScript's module augmentation to extend `DefineLocaleMessage` in `@i18n-micro/types`, which automatisch updates the `TranslationKey` type throughout your project.

## 🎯 Best Practices

1. **Keep translations organized** - Use nested objects for related keys
2. **Use consistent naming** - Follow a naming convention (bijv., `errors.*`, `buttons.*`)
3. **Leverage ScopedKey** - Gebruik `ScopedKey` for dynamic keys instead of plain assertions
4. **Check types in CI** - Run type checking in your CI pipeline
5. **Don't edit generated files** - De `.d.ts` file is auto-generated and zal worden overwritten
