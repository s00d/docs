---
title: "DevTools-UI-integratie"
description: "Voeg @i18n-micro/devtools-ui toe aan je Vite-app voor een framework-agnostisch DevTools-paneel met een JSON-editor en live previews."
---

# DevTools UI Package (`@i18n-micro/devtools-ui`)

De `@i18n-micro/devtools-ui` package biedt een standalone development tools interface for managing translations. It includes a Vite plugin for automatic integration and a universal bridge system for framework-onafhankelijk i18n support.

## Overzicht

`@i18n-micro/devtools-ui` is a self-contained package that biedt:

- **Vite Plugin** - Automatisch button injection and API endpoints
- **Universal Bridge** - Framework-agnostic bridge creation for any i18n implementation
- **Web Component** - Aangepast element for the DevTools UI
- **File Operations** - List, load, and save vertaalbestands via API
...
The easiest way to use DevTools is through the Vite plugin, which automatisch injects a floating button and biedt eenll necessary API endpoints:
...
The plugin automatisch injects a floating button in the bottom right corner of your application:
...
The plugin biedt de following API endpoints:

#### `GET /__i18n_api/files`

Get list of all vertaalbestands and directory structure.
...
Load a specific vertaalbestand.

**Query Parameters:**

- `path` - Relative path to the vertaalbestand (bijv., `en.json` or `pages/home/en.json`)
...
The package biedt een universal bridge creation function that works with any i18n implementation:
...
- **Development Only** - The plugin only works in ontwikkelmodus (`apply: 'serve'`)
- **No Production Impact** - The plugin is automatisch excluded from productiebuilds
- **Zero Runtime Overhead** - No code is included in productie bundles
