---
title: "Node.js-integratie"
description: "Gebruik @i18n-micro/node om vertalingen in Node.js-servers, workers en scripts te laden zonder een browserframework."
---

# 🌐 Node.js Runtime

Gebruik `@i18n-micro/node` to add i18n translations to any Node.js application, CLI tool, or backend service. Dit pakket biedt de same translation logic as the Nuxt module, but for pure Node.js environments.
...
The package ondersteunt the same file structure as the Nuxt module:
...
- `plural?: PluralFunc` - Aangepast pluralization function
- `missingWarn?: boolean` - Show warnings for missing translations
- `missingHandler?: (locale: string, key: string, routeName: string) => void` - Aangepast handler

### `i18n.loadTranslations(dir?: string): Promise<void>`

Load translations from directory (recursive, ondersteunt pages structure).
...
Clear cache and reload translations from disk. Use this when vertaalbestands change.
...
De `currentRoute` feature stelt je in staat om set a route context that zal worden used by default in all `t()` calls:
...
- 📦 **Same JSON structure** - Gebruik de same vertaalbestands as your Nuxt app
...
Je kunt use `@i18n-micro/node` in Nuxt server API routes to provide server-side translations:
...
Je kunt implement custom localedetectie logic based on your needs:
...
5. **Use route context**: Set `currentRoute` to automatisch use route-specific translations
6. **Reload on file changes**: Gebruik `reload()` method with file watchers in development for hot-reloading
...
- **Route context**: Gebruik `setRoute()` to avoid passing route name to every `t()` call
- **No external calls**: All translations are loaded from local files, no network requests

## 📚 Related Documentation

- **[Folder Structure](/guides/folder-structure)** - Learn about vertaalbestand organization
