...
description: "Gebruik @i18n-micro/react om vertalingen, meervoudsvormen en gelokaliseerde routering aan React-apps toe te voegen met een router-adapter."
---

# React Package (`@i18n-micro/react`)

De `@i18n-micro/react` package biedt een lichtgewicht, high-performance internationalisatie solution for React applications. It shares the same core logic as Nuxt I18n Micro, offering reactive translations, route-specific support, and full TypeScript support.

## Overzicht

`@i18n-micro/react` is ontworpen voor React applications that need internationalisatie without the Nuxt framework. It biedt:

- **Lightweight** - Uses shared core logic from `@i18n-micro/core`
- **Reactive** - Automatisch component updates when translations change using `useSyncExternalStore`
...
De `useI18n` hook biedt eenccess to i18n functionality in React components. It uses `useSyncExternalStore` internally for reactivity, ensuring components re-render when translations or locale change. The hook automatisch uses the routing strategy from `I18nProvider` if available.
...
- Uses the routing strategy's `linkComponent` if available (bijv., `Link` from `react-router-dom`)
...
| `linkComponent`           | ❌       | Component to use for rendering links (bijv., `Link` from react-router-dom) |
...
Dit voorbeeld laat zien how to create an adapter for Next.js App Router:
...
Dit voorbeeld laat zien how to create an adapter for TanStack Router:
...
Zie de "Full Application Setup" example above for a complete router integration example using the adapter pattern.

## Best practices

1. **Use Router Adapter**: Always create and provide a router adapter for proper locale path handling
2. **Create Adapter in RouterRoot**: Create de router adapter inside a component that has access to router hooks (`useLocation`, `useNavigate`)
