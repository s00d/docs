---
title: "Astro-integratie"
description: "Gebruik @i18n-micro/astro voor localeprefixen, hreflang en vertalingen in Astro-pagina's met SSR en island-integraties per framework."
---

# Astro Package (`@i18n-micro/astro`)

De `@i18n-micro/astro` package biedt een lichtgewicht, high-performance internationalisatie solution for Astro applications. It shares the same core logic as Nuxt I18n Micro, offering SSR support, automatic localedetectie, Astro components, and full TypeScript support.

## Overzicht

`@i18n-micro/astro` is ontworpen voor Astro applications that need internationalisatie with server-side rendering support. It biedt:

- **Lightweight** - Uses shared core logic from `@i18n-micro/core`
- **SSR Support** - Volledig server-side rendering compatibility
...
- **Type-safe** - Volledig TypeScript support
- **Router-agnostic** - Works with any routing strategy or without routing
- **DevTools Integration** - Optioneel development tools via Vite plugin
- **SEO Optimized** - Automatisch meta tags and hreflang generation

## Installatie

Install het pakket using your preferred package manager:
...
**Let op:** De `createAstroRouterAdapter` is now exported from the package, so you don't need to create your own router adapter file. Echter, you can still create a custom adapter if needed.
...
- **Edge Runtime** (Cloudflare Workers, Vercel Edge, Deno Deploy): Gebruik `import.meta.glob` instead of `loadTranslationsIntoI18n`
- De `virtual:i18n-micro/config` module works in all environments (it's generated at build time)
...
The package uses a router adapter pattern to decouple i18n functionality from specific routing implementations. Hiermee kun je you to:

- Use any routing strategy (file-based routing, custom routing, or no routing)
- Implement routing logic in je application, not in the i18n package
- Keep the core package lichtgewicht and routing-agnostic

De `I18nRoutingStrategy` interface defines the contract between i18n and your routing:
...
**Let op:** De `createAstroRouterAdapter` is now exported from `@i18n-micro/astro`, so you don't need to create your own adapter file for basic use cases.

### Setting Router Adapter in Middleware

Je kunt set the routing strategy in middleware:
...
De `I18nRoutingStrategy` interface defines how i18n interacts with your routing:
...
Als je nodig hebt custom routing logic, here's a complete example:
...
A router adapter is an implementation of the `I18nRoutingStrategy` interface that defines how i18n interacts with your routing system. Hiermee kun je you to:

- Support any routing strategy (file-based, programmatic, custom)
- Customize localedetectie and path generation
- Integrate with third-party routing libraries
- Implement domain-based or subdomain-based locale routing

### Interface Reference

De `I18nRoutingStrategy` interface defines the following methods:

| Method                                                      | Required | Description                                                 |
| ----------------------------------------------------------- | -------- | ----------------------------------------------------------- |
| `getCurrentPath()`                                          | ✅       | Retourneert the current path (used for active link detection)   |
| `getRouteName(path, locales)`                               | ❌       | Extracts route name from path (bijv., `/en/about` → `about`) |
...
| `getRoute()`                                                | ❌       | Retourneert current route object with query params              |
...
- **What is de standaardlocale behavior?** (include prefix, hide prefix)
...
- Empty paths (`/`)
- Root locale (standaardlocale - should it be in URL?)
- Invalid locales
- Query parameters and hash fragments

### Example 1: Subdomain-Based Locale Routing

Dit voorbeeld laat zien how to implement locale routing using subdomains (bijv., `en.example.com`, `fr.example.com`):
...
This example uses query parameters for locale (bijv., `/?locale=fr`):
...
This example uses locale suffixes (bijv., `/about.en`, `/about.fr`):
...
Dit voorbeeld laat zien how to integrate with a custom routing library:
...
1. **Always handle de standaardlocale**: Decide whether de standaardlocale should appear in URLs or be hidden.
...
5. **Support both SSR and client-side**: Gebruik de `getCurrentUrl` parameter for SSR and `window.location` fallback for client-side:
...
Je kunt extend the built-in adapter and override specific methods:
...
| -------------------- | ---------------------------------------------------------- | -------- | ---------------- | ---------------------------------------------------------------------------------- |
| `locale`             | `string`                                                   | ✅       | -                | Default locale-code (bijv., `'en'`)                                                 |
...
| `autoDetect`         | `boolean`                                                  | ❌       | `true`           | Enable automatic localedetectie                                                  |
| `redirectToDefault`  | `boolean`                                                  | ❌       | `false`          | Redirect to standaardlocale if not found                                            |
| `translationDir`     | `string`                                                   | ❌       | `'src/locales'`  | Directory path for vertaalbestands                                               |
...
When `translationDir` is specified, the integration creates a virtual module `virtual:i18n-micro/config` with your configuratie. Echter, **you must explicitly load translations in your middleware** using either:

1. **Node.js Runtime**: `loadTranslationsIntoI18n()` function (uses `node:fs`)
2. **Edge Runtime**: `import.meta.glob()` (build-time loading)

⚠️ **Important**: De `loadTranslationsIntoI18n` function uses Node.js filesystem APIs and will **NOT work** in Edge runtime environments (Cloudflare Workers, Vercel Edge, Deno Deploy). For these environments, use `import.meta.glob` instead.
...
1. The integration creates a virtual module `virtual:i18n-micro/config` with your configuratie (available at build time)
2. Je kunt import the config in your middleware to avoid duplicating settings
3. **Je moet explicitly load translations** in your middleware:
   - **For Node.js runtime**: Gebruik `loadTranslationsIntoI18n` utility to load translations from the directory
   - **For Edge runtime**: Gebruik `import.meta.glob` to load translations at build time (see Edge Runtime example below)

**⚠️ Belangrijk: Runtime Compatibility**

De `loadTranslationsIntoI18n` function uses Node.js filesystem APIs (`node:fs`) and **will NOT work** in Edge runtime environments zoals:
...
**Let op:** Standaard, files in the `pages/` folder are treated as route-specific translations. The utility automatisch maps them to route names op basis van de directory structure.
...
Same as `t()` but altijd returns a string.
...
Formats a number volgens de huidige locale.
...
Formats a date volgens de huidige locale.
...
Formats a relative time (bijv., "2 hours ago").
...
Checks if a vertaalsleutel exists.
...
Creates a lichtgewicht copy of the instance with a new locale, sharing the same cache. Useful for per-request instances in middleware.
...
Creates Astro middleware for automatic localedetectie and i18n instance setup.
...
| `autoDetect`        | `boolean`             | ❌       | Enable automatic localedetectie (default: `true`)        |
| `redirectToDefault` | `boolean`             | ❌       | Redirect to standaardlocale if not found (default: `false`) |
| `routingStrategy`   | `I18nRoutingStrategy` | ❌       | Router adapter for routing features                        |

**Retourneert:** `MiddlewareHandler`
...
The middleware automatisch detects the locale from:

1. **URL Path** - Checks if the first path segment is a locale-code (bijv., `/fr/about`)
...
4. **Default Locale** - Falls back to the configured standaardlocale
...
Utility function for custom localedetectie logic. Set `localeCookie` to `null` to disable cookie-based detection.
...
Provides helper functions for translations and routing in Astro components and pages. The utility automatisch uses the routing strategy from `Astro.locals.routingStrategy` if available, with fallback implementations if not provided.
...
Gets de huidige locale from Astro context.
...
Gets de standaardlocale from Astro context.
...
Translation component met ondersteuning voor pluralization, formatting, and HTML rendering.
...
Localized link component that automatisch handles locale prefixes using the routing strategy from `Astro.locals.routingStrategy` if available.
...
| `activeClass` | `string` | ❌       | `'active'` | CSS class for actieve locale link |
...
The component automatisch:

- Filters out disabled locales
- Highlights de huidige locale
- Generates localized paths voor elke locale using the routing strategy
- Uses locale display names from configuratie
...
Astro Islands allow you to use interactive components (Vue, React, Svelte) within Astro pages. De `@i18n-micro/astro` package provides framework-specifiek adapters for seamless i18n integration in islands.
...
- **Framework-specific**: Native APIs voor elke framework (Vue provide/inject, React Context, Svelte stores)

### Setup

Eerst, ensure you have the required framework-integraties installed:
...
Gebruik `getI18nProps()` to prepare translations for islands. **Important**: Pass only the keys your islands actually need to minimize bundle size:
...
Prepares translation props for islands. **Belangrijk:** Always pass specific keys to minimize bundle size.

**Parameters:**

- `astro`: Astro global object
- `keys`: **Recommended** - Array of vertaalsleutels to include (bijv., `['islands.vue.title', 'islands.apples']`). If omitted, all route-specific translations are included, which may increase bundle size.

**Retourneert:** `I18nClientProps` object with:
...
Initializes i18n provider for Vue islands. Retourneert a reactive state ref.
...
All adapters provide the same vertaalmethoden:

- `t(key, params?, defaultValue?, routeName?)` - Basis translation
...
2. **Gebruik `client:load` or `client:only`**: Islands need client-side hydration
...
### Voorbeeld: Complete Islands Page
...
- `link[rel="alternate"][hreflang="x-default"]` - Standaard language fallback for unmatched user languages
...
Dit pakket ondersteunt DevTools integration via the `@i18n-micro/devtools-ui` Vite plugin. Zie de [DevTools UI Package documentation](/integrations/devtools-ui) for details.

## Geavanceerd gebruik

### Custom Router Adapter

Je kunt create custom router adapters for specific routing needs:
...
Je kunt define translations specific to routes:
...
De `clone()` method allows creating per-request instances that share the same cache, improving performance in SSR scenarios.

## TypeScript-ondersteuning

### Global Types (App.Locals)

The integration automatisch injects types for `App.Locals` via Astro's `injectTypes` hook. Je moet see `i18n`, `locale`, `defaultLocale`, `locales`, `currentUrl`, and `routingStrategy` available in your Astro components and middleware without any manual configuratie.

**Belangrijk:** Types are automatisch available when you add the integration to your `astro.config.mjs`. If you're not using the integration (only middleware), you may need to handmatig add the type reference (see Manual Type Reference below).
...
If for some reason types are not showing up automatisch (bijv., with custom TypeScript configuratie or older Astro versions), you can handmatig add the reference to your `src/env.d.ts` file:
...
Je kunt create type-safe vertaalsleutels:
...
2. **Use Middleware for Locale Detection** - Always set up middleware for automatic localedetectie
...
8. **Cache Management** - Gebruik `clone()` for per-request instances to share cache efficiënt
9. **Error Handling** - Always handle translation loading errors gracefully
10. **DevTools in Development** - Use Vite plugin for easier translation management tijdens ontwikkeling
11. **Translation File Structure** - Organize vertaalbestands logically (by feature, route, or component)
12. **Router Adapter Reusability** - Create de router adapter once and reuse it in both integration and middleware
