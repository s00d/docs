---
title: "SolidJS-integratie"
description: "Gebruik @i18n-micro/solid om vertalingen en gelokaliseerde routering aan SolidJS-apps toe te voegen via signals en een router-adapter."
---

# Solid Package (`@i18n-micro/solid`)

De `@i18n-micro/solid` package biedt een lichtgewicht, high-performance internationalisatie solution for SolidJS applications. It shares the same core logic as Nuxt I18n Micro, offering reactive translations, route-specific support, and full TypeScript support.

## Overzicht

`@i18n-micro/solid` is ontworpen voor SolidJS applications that need internationalisatie without the Nuxt framework. It biedt:

- **Lightweight** - Uses shared core logic from `@i18n-micro/core`
- **Reactive** - Automatisch component updates when translations change using SolidJS signals
...
De `useI18n` hook requires locales configuratie to be provided via `I18nProvider`. Je moet provide both `locales` and `defaultLocale`:
...
2. **Exported from package**: De `createSolidRouterAdapter` is now exported directly from `@i18n-micro/solid`, so you don't need to create a separate router adapter file for basic use cases. Echter, you can still create custom adapters if needed (see "Creating Custom Router Adapters" section below).

3. **Reactivity**: The built-in adapter automatisch provides `getCurrentPathAccessor()` for proper reactivity in SolidJS components.
...
| `getCurrentPathAccessor()` | ⚠️ **Highly Recommended** | Retourneert reactive accessor for current path (SolidJS-specific). **Required for proper reactivity** - without this, active link states and path-dependent UI won't update reactively. |
| `linkComponent`            | ❌                        | Component to use for rendering links (bijv., `A` from @solidjs/router)                                                                                                               |
...
Dit voorbeeld laat zien how to create an adapter for a custom router that uses signals:
...
1. **Always provide `getCurrentPathAccessor` for SolidJS** (REQUIRED for reactivity): Dit zorgt ervoor dat proper reactivity in components. Without it, active link states and path-dependent UI won't update:
...
- `localeAccessor: Accessor<string>` - Reactive accessor for huidige locale
...
De `useI18n` hook biedt eenccess to i18n functionality in SolidJS components.
...
| `getLocaleName`         | `() => string \| null`                                                              | ❌       | -       | Function to get huidige locale name                 |
...
| `customActiveLinkStyle` | `JSX.CSSProperties`                                                                 | ❌       | `\{\}`    | Custom styles for de huidige locale link           |
...
Pluralization is handled automatisch op basis van de count:
...
Je kunt provide a custom pluralization function:
...
All vertaalmethoden are type-safe:
...
Zie de playground implementation in `packages/solid/playground/src/` for a complete working example.
