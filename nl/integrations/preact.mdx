---
title: "Preact-integratie"
description: "Gebruik @i18n-micro/preact om vertalingen, meervoudsvormen en gelokaliseerde routering aan Preact-apps toe te voegen via een lichte provider."
---

# Preact Package (`@i18n-micro/preact`)

De `@i18n-micro/preact` package biedt een lichtgewicht, high-performance internationalisatie solution for **Pure Preact** applications. It shares the same core logic as Nuxt I18n Micro, offering reactive translations, route-specific support, and full TypeScript support.

## Overzicht

`@i18n-micro/preact` is ontworpen voor Pure Preact applications (without React compatibility layer) that need internationalisatie. It biedt:

- **Lightweight** - Uses shared core logic from `@i18n-micro/core`
- **Reactive** - Automatisch component updates when translations change using `useState` + `useEffect` (Pure Preact approach)
...
- **DevTools Integration** - Vite plugin for managing translations tijdens ontwikkeling

**Let op:** The built-in router adapter (`createPreactRouterAdapter` / `createBrowserHistoryAdapter`) uses the native History API and works with any router that listens to `popstate` events (bijv., `wouter-preact`, `preact-router`). For custom routing needs, you can create your own adapter.
...
**Let op:** De `createPreactRouterAdapter` (aliased as `createBrowserHistoryAdapter`) is exported from het pakket and biedt een basic History API-based adapter. For router-specific adapters (like wouter), you can create custom adapters as shown above.
...
The package uses a router adapter pattern to decouple i18n functionality from specific router implementations. Hiermee kun je you to:
...
- Keep the core package lichtgewicht and router-agnostic

De `I18nRoutingStrategy` interface defines the contract between i18n and your router:
...
**Parameters:** the [shared `createI18n` options](/integrations/overview#create-i18n-options) — `locale`, `fallbackLocale`, `messages`, `plural`, `missingWarn` and `missingHandler`.

**Retourneert:** `PreactI18n`
...
De `useI18n` hook biedt eenccess to i18n functionality in Preact components. It uses `useState` + `useEffect` internally for reactivity (Pure Preact approach), ensuring components re-render when translations or locale change. The hook automatisch uses the routing strategy from `I18nProvider` if available.
...
Wanneer een routing strategy is provided via `I18nProvider`, the hook exposes router-related methods:
...
Resolve path for a specific locale. Retourneert localized path using the routing strategy.
...
Same as `localeRoute` but altijd returns a string.
...
Context provider-component that makes the i18n instance available to all child components. It also provides locales configuratie and routing strategy.
...
Translation component met ondersteuning voor pluralization, formatting, and HTML rendering. Automatically uses the routing strategy from `I18nProvider` if available.
...
Localized link component that automatisch handles locale prefixes using the routing strategy from `I18nProvider` if available.
...
- Uses the routing strategy's `linkComponent` if available (bijv., `Link` from `wouter-preact`)
...
- Supports external links (detected automatisch)
...
De `I18nRoutingStrategy` interface defines how i18n interacts with your router:
...
This adapter works with any router that listens to `popstate` events (bijv., `wouter-preact`, `preact-router`).
...
| `linkComponent`           | ❌       | Component to use for rendering links (bijv., `Link` from wouter-preact) |
...
Dit voorbeeld laat zien how to create an adapter for `wouter-preact`:
...
Dit voorbeeld laat zien how to create an adapter for `preact-router`:
...
4. **Support both SSR and client-side**: Gebruik `window` checks for client-side code:
...
Load translations dynamisch from JSON files:
...
`addTranslations` loads base translations (stored as `index`). Wanneer je call `addRouteTranslations`, these base translations are automatisch merged in. Page-specific keys override base ones.
...
2. **Create Adapter in RouterRoot**: Create de router adapter inside a component that has access to router hooks (`useLocation`, `navigate`)
...
11. **Router Adapter Reusability**: Create de router adapter once in `RouterRoot` and reuse it throughout the app
12. **URL-based Locale Detection**: Use URL-based localedetectie for better SEO and user experience
