---
title: "Vue-integratie"
description: "Gebruik @i18n-micro/vue om vertalingen aan losstaande Vue 3-apps toe te voegen met dezelfde componenten en helpers als de Nuxt-module."
---

# Vue Package (`@i18n-micro/vue`)

De `@i18n-micro/vue` package biedt een lichtgewicht, high-performance internationalisatie solution for Vue 3 applications. It shares the same core logic as Nuxt I18n Micro, offering reactive translations, route-specific support, and full TypeScript support.

## Overzicht

`@i18n-micro/vue` is ontworpen voor Vue 3 applications that need internationalisatie without the Nuxt framework. It biedt:

- **Lightweight** - Uses shared core logic from `@i18n-micro/core`
- **Reactive** - Automatisch component updates when translations change
...
De `useI18n` composable requires locales configuratie to be provided via Vue's dependency injection system.

**Recommended approach**: Pass `locales` and `defaultLocale` directly to `createI18n` - they zal worden automatisch provided:
...
**Alternative approach**: Als je nodig hebt to provide locales handmatig (bijv., dynamisch), you can use the injection keys:
...
Hier is een complete example of creating a Vue Router adapter:
...
Je kunt set the routing strategy in two ways:
...
Je kunt create adapters for any router library. Hier is een voorbeeld for a custom router:
...
| ----------------- | ---------------------------------------------------------- | -------- | ---------------- | ---------------------------------------------- |
| `locale`          | `string`                                                   | ✅       | -                | Current locale-code (bijv., `'en'`)             |
...
**Retourneert:** `I18nPlugin`

The returned object contains:

- `global`: De `VueI18n` instance for direct access
...
Retourneert the full translation tree for de actieve locale and route — the same dictionary `t()` reads from.

##### `setTranslation(key: TranslationKey, value: unknown): void`

Replaces the value at `key` in the active dictionary. Dit is a replace, not a merge; use `mergeTranslations` when siblings should survive.
...
De `useI18n` composable biedt eenccess to i18n functionality in Vue components.
...
Generates SEO meta tags and links for localewisseling. Includes canonical URL, `hreflang` alternate links for all locales, `hreflang="x-default"` link pointing to de standaardlocale URL, and Open Graph locale meta tags.
...
| `customActiveLinkStyle` | `CSSProperties`                                                                     | ❌       | `\{\}`    | Custom styles for de huidige locale link           |
...
Zie de playground implementation in `packages/vue/playground/src/` for a complete working example.
