---
title: "Voorbeelden"
description: "Nuxt I18n Micro-recepten: paginaspecifieke vertalingen, dynamische localewisseling, SEO-hreflang en routegebaseerde restricties met code die je kunt kopiëren en aanpassen."
---

# 📚 Nuxt I18n Micro Examples and Usage

Deze sectie biedt various examples demonstrating how to use `Nuxt I18n Micro` in your Nuxt.js application. You'll see how to switch locales, use the `<i18n-link>`, `<i18n-switcher>`, and `<i18n-t>` components, and dynamisch handle vertaalsleutels.

## 🛠️ Basic Setup

Hier is een basic setup to get started with `Nuxt I18n Micro`:
...
Dit voorbeeld toont how to switch locales programmatisch using buttons:
...
De `<i18n-switcher>` component biedt een dropdown for localewisseling with customizable labels:
...
De `<i18n-link>` component automatisch handles locale-specific routing:
...
In some scenarios, you may want to iterate over dynamic keys stored within your vertaalbestands and render their values conditionally. The example below demonstrates how to achieve this using `Nuxt I18n Micro`.

### Voorbeeld: Rendering Dynamic Keys
...
### Voorbeeld: Rendering Dynamic Keys from an Object

In dit voorbeeld, we handle an object stored within your vertaalbestand. We fetch and iterate over the keys of the object, dynamisch rendering both the key names and their associated values.
...
De `<i18n-t>` component is useful for rendering translations with dynamic content and HTML tags:
...
Hier is een complex structure demonstrating multiple translation uses within a single page:
...
De `$tc` function in `Nuxt I18n Micro` handles pluralization op basis van de count and locale settings. Dit is nuttig for dynamisch adjusting messages that involve counts, zoals items, notifications, or other entities that can vary in number.

### Voorbeeld: Using `$tc` for Pluralization

In het volgende example, we display a message indicating the number of apples using `$tc`. De vertaling key handles multiple plural forms op basis van de count provided.
...
- **`$tc('apples', 0)`**: Retourneert the first form, used when the count is zero (`"no apples"`).
- **`$tc('apples', 1)`**: Retourneert the second form, used when the count is one (`"one apple"`).
- **`$tc('apples', 10)`**: Retourneert the third form with the count value, used when the count is two of meer (`"10 apples"`).

### Additional Example with More Complex Pluralization

Als je application needs to handle more complex pluralization rules (bijv., specific cases for zero, one, two, few, many, other), you can extend the translation strings accordingly:
...
- **`$tc('apples', 3)`**: Could return `"a few apples"`, afhankelijk van the rules defined for the count.

## 🌐 Using `$tn` for Number Formatting

De `$tn` function formats numbers volgens de huidige locale using the `Intl.NumberFormat` API. Dit is nuttig for displaying numbers in a way that matches the user's regional settings, zoals currency, percentages, or other number formats.

### Voorbeeld: Using `$tn` for Number Formatting
...
De `$td` function formats dates and times volgens de huidige locale using the `Intl.DateTimeFormat` API. Dit is nuttig for displaying dates and times in formats that are familiar to the user op basis van deir locale settings.

### Voorbeeld: Using `$td` for Date and Time Formatting
...
De `$tdr` function formats dates as relative times (bijv., "5 minutes ago", "2 days ago") volgens de huidige locale using the `Intl.RelativeTimeFormat` API. Dit is nuttig for displaying time differences relative to the current date and time.

### Voorbeeld: Using `$tdr` for Relative Date Formatting
