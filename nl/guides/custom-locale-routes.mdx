---
title: "Aangepaste locale-routes"
description: "Definieer aangepaste locale-routes per pagina in Nuxt I18n Micro met defineI18nRoute en globalLocaleRoutes, inclusief dynamische parameters."
---
...
De `localeRoutes` feature in `Nuxt I18n Micro` stelt je in staat om define custom routes for specific locales, offering flexibility and control over the routing structure of your application. Deze functie is particularly useful when certain locales require different URL structures, tailored paths, or need to follow specific regional or linguistic conventions.

## 🚀 Primary Use Case of `localeRoutes`

The primary use case for `localeRoutes` is to provide distinct routes for different locales, enhancing the user experience by ensuring URLs are intuitive and relevant to the target audience. Bijvoorbeeld, you might have different paths for English and Russian versions of a page, where the Russian locale follows a localized URL format.

### 📄 Voorbeeld: Defining `localeRoutes` in `$defineI18nRoute`
...
- **Custom Behavior**: With `localeRoutes`, specific locales can have their own routes, overriding the default path with locale-specific routes defined in the configuratie.

## 🌱 Use Cases for `localeRoutes`

### 📄 Voorbeeld: Using `localeRoutes` in a Page
...
- **Documentation Sites**: Provide distinct routes voor elke locale to better match the localized content structure.
...
Standaard, your pages are named op basis van deir file & path name. Here's what it means:
...
When working with dynamic routes that contain slugs or parameters, you can use `localeRoutes` with dynamic segments and `$setI18nRouteParams` to provide locale-specific slugs voor elke locale.

### Dynamic Route Parameters in `localeRoutes`

Je kunt define dynamic routes using Nuxt's route parameter syntax (`[...slug]` for catch-all routes, `[param]` for single parameters, or `:param()` for named parameters):
...
De `$setI18nRouteParams` function stelt je in staat om set different parameter values (like slugs) voor elke locale. Dit is especially useful when your content has locale-specific URLs.

**Important**: `$setI18nRouteParams` should be called after fetching data that contains locale-specific slugs, meestal inside `useFetch` or `useAsyncData`.
...
### Complete Voorbeeld: Product Pages with Locale-Specific Slugs

Hier is een complete example showing how to use `localeRoutes` and `$setI18nRouteParams` together for a product detail page:
...
1. **Route Definition**: `localeRoutes` defines the base path structure voor elke locale, including dynamic segments like `[...slug]` or `[id]`.

2. **Parameter Setting**: `$setI18nRouteParams` sets the actual parameter values voor elke locale. Wanneer een user switches locales using `$switchLocalePath`, de module uses these parameters to construct the correct URL.
...
4. **Navigation**: Gebruik `I18nLink` or `$localeRoute` with the route name and parameters to navigate between localized versions of dynamic routes.

### Important Notes

- **Timing**: `$setI18nRouteParams` should be called after fetching data that contains locale-specific slugs, meestal inside `useFetch` or `useAsyncData`.

- **Parameter Names**: The parameter names in `$setI18nRouteParams` must match the route parameter names (bijv., `slug` for `[...slug]` or `id` for `[id]`).

- **Route Names**: Bij gebruik van `definePageMeta(\{ name: 'route-name' \})`, use this name when linking to the route with `I18nLink` or `$localeRoute`.

- **Catch-all Routes**: For catch-all routes (`[...slug]`), the slug parameter should be an array or a string that zal worden converted to an array.

## 🧩 Programmatic routes (`pages:extend`) — #244

Routes created in `pages:extend` have no place for `defineI18nRoute()`, and several routes vaak share one wrapper SFC. File-keyed scanning then collapses them onto a single entry.
...
Gebruik `splitLocaleRoutes` from `@i18n-micro/utils/split-locale-routes`:
...
- **🔧 Maintain Consistency**: Keep a consistent routing pattern across locales unless there's a strong reason to deviate. Deze aanpak helps in maintaining clarity and reducing complexity.
