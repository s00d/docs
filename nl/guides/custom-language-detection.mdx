---
title: "Aangepaste taaldetectie"
description: "Bouw aangepaste taaldetectiestromen voor Nuxt I18n Micro met server-plugins, headers en cookies via useI18nLocale."
---
...
## ✨ Voorbeeld: Server-Side Detection with `useI18nLocale`

Dit is the **recommended approach** for all strategies.
...
- Cookie-based localedetectie is disabled by default (`localeCookie: null`)
...
If users can handmatig switch language, respect their choice over auto-detection:
...
| `useI18nLocale().getLocale()`          | Retourneert huidige locale from state or cookie                       |
| `useI18nLocale().getPreferredLocale()` | Retourneert preferred locale (validated against `locales` list)       |
| Plugin `order: -10`                    | Ensures your plugin runs before i18n initialization               |
| `enforce: 'pre'`                       | Runs in the "pre" plugin group                                    |

**Do NOT:**

- Gebruik `useCookie('user-locale')` directly — `useI18nLocale()` manages cookies internally
- Gebruik `useState('i18n-locale')` directly — use `useI18nLocale().setLocale()` instead
