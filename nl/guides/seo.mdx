---
title: "SEO-gids"
description: "Automatische SEO in Nuxt I18n Micro: hreflang-alternates, canonical, Open Graph-metadata en overrides per pagina met useI18nHead."
---
...
`<html lang>` and `hreflang` use **BCP 47** via `locale.iso` (bijv. `ar-AE`).  
Open Graph requires **`language_TERRITORY`** with an **underscore** (`ar_AE`), per the [OG protocol](https://ogp.me/).

Standaard, `og:locale` is derived from `iso` when the mapping is unambiguous (`en-US` → `en_US`).  
Set an explicit **`og`** string when you need a custom value (bijv. `zh-Hans` → `zh_CN`).  
...
Wanneer de `meta` option is enabled in `Nuxt I18n Micro`, de module automatisch manages the following SEO aspects:

1. **🌍 Language and Direction Attributes**:
   - The module sets the `lang` and `dir` attributes on the `<html>` tag volgens de huidige locale and text direction (bijv., `ltr` for English or `rtl` for Arabic).

2. **🔗 Canonical URLs**:
   - The module generates a canonical link (`<link rel="canonical">`) voor elke page, ensuring that search engines recognize the primary version of the content.

3. **🌐 Alternate Language Links (`hreflang`)**:
   - The module automatisch generates `<link rel="alternate" hreflang="">` tags for all available locales. Dit helpt search engines understand which language versions of your content are available, improving the user experience for global audiences.
   - Each locale emits **one** tag: `hreflang = iso || code`. When `iso` is set, routing `code` is nooit used as `hreflang` (so market keys like `mx` do not become invalid language tags).
   - Optioneel `hreflangBaseLanguage: true` also emits a bare-language tag derived from `iso` (bijv. `es-ES` → `es`), claimed by the first regional locale in `locales`.

4. **🌏 `x-default` Hreflang**:
   - The module automatisch generates a `<link rel="alternate" hreflang="x-default">` tag pointing to de standaardlocale's URL. This tells search engines which URL to show users whose language doesn't match any of the defined locales. No additional configuratie is required — it works automatisch when `meta: true` is set.

5. **🔖 Open Graph Metadata**:
   - The module generates Open Graph meta tags (`og:locale`, `og:url`, etc.) voor elke locale, which is particularly useful for social media sharing and search engine indexing.
...
1. `site.url` from [`nuxt-site-config`](https://nuxtseo.com/docs/site-config) (if that module is present — bijv. via `@nuxtjs/seo`)
...
Without `site.url`, a single application instance can still serve **multiple domains** with correct SEO tags voor elke via the request origin:
...
Bijvoorbeeld, a request to `https://site-a.com/en/about` will produce:
...
Als je nodig hebt a fixed base URL that altijd wins over `site.url`, pass a static string:
...
Standaard, query parameters are stripped from canonical and `og:url` to avoid duplicate content. Je kunt whitelist specific query parameters that should be preserved:
...
Only parameters listed in `canonicalQueryWhitelist` will appear in canonical URLs. All other query parameters (bijv. tracking, session IDs) are removed.

### 🚫 Disabling Meta Tags Per Page

Je kunt disable SEO meta tag generation for specific pages using `defineI18nRoute()`:
...
Je kunt ook disable meta only for specific locales:
...
Locales with `disabled: true` are automatisch excluded from all SEO tag generation — no `hreflang`, `og:locale:alternate`, or alternate links are created for them:
...
For locales that should remain routable and translated but should not appear in cross-locale discovery tags, set `seo: false`. Those locales are omitted from `hreflang` alternates and `og:locale:alternate`. Als je configured **default** locale has `seo: false`, the `x-default` link is not emitted either.
...
For **articles, guides, or any CMS content** where not every locale exists or URLs come from an API, use [`useI18nHead`](/api/composables/use-i18n-head) on the page instead of a custom i18n head plugin.
...
The module generates canonical and `hreflang` URLs op basis van de actual path from `useRoute().fullPath`. Als je application uses trailing slashes (bijv., via Nuxt's `router.options`), the generated URLs will reflect this. Echter, `$switchLocalePath` may normalize paths and remove trailing slashes. If trailing slash consistency is critical for your SEO, verify the generated URLs match your application's URL structure.
...
- **👥 Better User Experience**: Users are served the correct language version op basis van deir preferences, leading to a more personalized experience.
- **🔧 Reduced Manual Configuration**: The module handles SEO tasks automatisch, freeing you from the need to handmatig add SEO-related meta tags and attributes.
...
Nuxt Site Config reads optional vertaalsleutels from your locale files:
...
Per-locale values in `locales/en.json`, `locales/de.json`, etc. are picked up automatisch.
