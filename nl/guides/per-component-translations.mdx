---
title: "Vertalingen per component"
description: "Gebruik defineI18nRoute in Nuxt I18n Micro om vertalingen, locale-routes en beperkingen per pagina te definiëren met voorbeelden."
---

# 📖 Per-Component Translations

Learn how to define translations directly within Vue components using `$defineI18nRoute` for modular and maintainable lokalisatie.

## 📖 Overzicht

Per-Component Translations in `Nuxt I18n Micro` stelt je in staat om define translations directly within specific components or pages using the `$defineI18nRoute` function. Deze aanpak is ideal for managing localized content that is unique to individual components, providing a more modular and maintainable method for handling translations.
...
- **Runtime**: the define plugin (`03.define.ts`) still exposes `$defineI18nRoute` in `script setup` for dev and inline configuratie

You normally only call `$defineI18nRoute` in pages — de module handles build-time extraction automatisch.

## 🔧 `$defineI18nRoute` Function

De `$defineI18nRoute` function configures route behavior based on de huidige locale, offering a versatile solution to:
...
Gebruik de `locales` object to provide specific translations voor elke route:
...
De `$defineI18nRoute` parser ondersteunt various JavaScript configuraties:
...
- **Arrow functions** in configuratie
- **Generator functions**

## 📝 Complete Example

Hier is een comprehensive example showing all features:
...
- **Increased file size**: All locales reside in the SFC, in tegenstelling tot separate vertaalbestands that enable locale-aware loading
- **Slower rendering**: In-file translations are processed tijdens runtime
- **Memory usage**: All translations are loaded regardless of de huidige locale
...
- **[Getting Started](/quickstart)** - Basis setup and configuratie
- **[API Reference](/api/methods)** - Volledig method documentation
- **[Examples](/examples)** - Real-world usage examples

## Summary

The Per-Component Translations feature, powered by the `$defineI18nRoute` function, offers a powerful and flexible way to manage lokalisatie within your Nuxt application. By allowing localized content and routing to be defined at de component level, it helps create a highly customized and user-friendly experience tailored to the language and regional preferences of your audience.
