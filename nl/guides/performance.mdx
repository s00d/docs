---
title: "Prestatiegids"
description: "Optimaliseer Nuxt I18n Micro: payload-modi, HTTP-caching, SSR-overdracht en tips voor edge- en serverless-omgevingen."
---
...
`Nuxt I18n Micro` is designed with performance in mind, offering a significant improvement over traditional internationalisatie (i18n) modules like `nuxt-i18n`. Deze gids biedt an in-depth look at the performance benefits of using `Nuxt I18n Micro`, and how it compares to other solutions.

## 🤔 Why Focus on Performance?

In large-scale projects and high-traffic environments, performance bottlenecks can lead to slow build times, increased memory usage, and poor server response times. These issues become more pronounced with complex i18n setups involving large vertaalbestands. `Nuxt I18n Micro` was built to address these challenges head-on by optimizing for speed, memory efficiency, and minimal impact on your application's bundle size.

## 📊 Performance Comparison

We conducted a series of tests on identical fixtures via `pnpm test:performance` (`pnpm -C scripts cli performance`) against **`@nuxtjs/i18n@10.6.0`**. Full methodology and charts: [Performance Test Results](/guides/performance-results).
...
Zie de [full benchmark report](/guides/performance-results) for charts, Autocannon results, and fixture details.
...
<Note>
Interactive chart omitted in Mintlify docs. Zie de benchmark tables on this page or the [VitePress docs](https://s00d.github.io/nuxt-i18n-micro/guide/performance-results) for chart: `chart`.
</Note>
...
Deze aanpak keeps the routing configuratie lichtgewicht and ensures fast route resolution regardless of the number of locales.

### 📂 Streamlined Translation Loading

The module ondersteunt only JSON files for translations, with a clear separation between global and page-specific files. Dit zorgt ervoor dat that only the necessary translation data is loaded at any given time, further enhancing performance.

### 🔒 GlobalThis Singleton Cache

Starting from v3.0.0, de module uses a `globalThis` singleton pattern with `Symbol.for` to guarantee a single cache instance across the entire Node.js process. This prevents:
...
De `$t()` function uses a direct lookup strategy optimized for speed:
...
Translations loaded during SSR stay in **server memory** for `$t` during render. They are **not** copied into `nuxtApp.payload` / the HTML document (same idea as `@nuxtjs/i18n` with `experimenteel.preload: false`).
...
difference between a five-minute and a five-hour debugging session. Er zijn four, in the
order a request meets them:

| Layer | Where | Lifetime | Cleared by |
| --- | --- | --- | --- |
| Browser / CDN | `Cache-Control` on `/\{apiBaseUrl\}/**` | `httpCacheDuration`, `immutable` | a new `?v=` — d.w.z. a deploy that changed translations |
...
- **`immutable` also requires `?v=`.** Without a buster the header is
  ...
  `max-age` on a URL that nooit changes pins the first response a browser ever saw.
...
those static files in the platform's own configuratie.
</Warning>
...
Gebruik `translationPayloads.mode: 'source'` for compact Edge embeds (and optional compact public copies):
...
`mode: 'source'` keeps layer-merged source files compact and merges root/page/fallback tijdens runtime through the built-in `/_locales` route (or Edge `assets:i18n`). Standaard it disables public asset copies and prerendered payload routes.

<Warning>
`prerenderRoutes` defaults to `false`. With `mode: 'source'`, `publicAssets` also defaults to `false`. Pure static hosting without a Nitro/edge runtime therefore cannot load translations on the client unless you enable one of these outputs, keep `serverHandler` available tijdens runtime, or host payloads externally.
...
During build, de module warns when generated payload output exceeds `translationPayloads.warnFileCount` (default 500) or `translationPayloads.warnSizeBytes` (default 10 MB). It also warns when all local outputs are disabled without external payload hosts configured.

## 📝 Tips for Maximizing Performance

Hier zijn a few tips to ensure you get the best performance out of `Nuxt I18n Micro`:

- 📉 **Limit Locale Data**: Only include the locales you need in je project to keep the bundle size small.
- 🗂️ **Use Page-Specific Translations**: Organize your vertaalbestands by page to avoid loading unnecessary data.
...
For detailed results of the performance tests, please refer to the [Performance Test Results](/guides/performance-results).
