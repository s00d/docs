...
description: "Alle configuratieopties voor Nuxt I18n Micro: locales, strategie, SEO, caching, opties per pagina en meer, met praktische voorbeelden."
---
...
Every option de module accepts, from the `ModuleOptions` type. The sections below explain
...
| --- | --- | --- | --- |
| [`locales`](/api/module-options) | `Locale[]` | `[]` | List of ondersteund locales. |
| [`meta`](/api/module-options) | `boolean` | `true` | Generate SEO meta tags (`hreflang`, `canonical`, `og:url`, `og:locale`) automatisch. |
| [`strategy`](/api/module-options) | `Strategies` | `'prefix_except_default'` | URL routing strategy for locale prefixes. - `'no_prefix'` — no locale in URL; locale stored in cookie. - `'prefix_except_default'` — prefix all locales except the default. - `'prefix'` — altijd prefix, including de standaardlocale. - `'prefix_and_default'` — like `prefix`, but de standaardlocale is also accessible without prefix. |
| [`metaBaseUrl`](/api/module-options) | `string` | `undefined` | Base URL for SEO meta tags (canonical, og:url, hreflang). - A concrete URL string (bijv. `'https://example.com'`) — used as-is (highest priority). - `undefined` — falls back to `site.url` from `nuxt-site-config` when that module is   present, otherwise the current request origin   (`useRequestURL().origin` on server, `window.location.origin` on client). |
...
| [`hreflangBaseLanguage`](/api/module-options) | `boolean` | `false` | Also emit a bare-language `hreflang` derived from each locale's `iso` (bijv. `es-ES` → also `es`). |
...
| [`translationPayloads.mode`](/api/module-options) | `'premerged' \| 'source'` | `'premerged'` | Translation payload strategy. - `premerged`: build-time `\{page\}/\{locale\}/data.json` matrix (default) - `source`: compact source files merged tijdens runtime (prefer on Edge / large catalogs) |
...
| [`autoDetectPath`](/api/module-options) | `string` | `'/'` | Where cookie / Accept-Language preference redirects may run (when `redirects` is enabled). - `'/'` — only `/` (deep links in de standaardlocale stay reachable; default) - `'no_prefix'` — only paths without a locale prefix - `'*'` — every path, including rewriting an explicit locale prefix   (bijv. `/fr/about` → `/de/about` when the cookie prefers `de`) - any other string — exact path match (bijv. `'/welcome'`) Prefixed strategy cleanup (bijv. `/en` → `/` under `prefix_except_default`) is not gated. |
| [`disableWatcher`](/api/module-options) | `boolean` | `false` | Disable the file watcher that auto-creates missing vertaalbestands in ontwikkelmodus. |
| [`types`](/api/module-options) | `boolean` | `true` | Generate TypeScript type declarations for `useI18n`, `$t`, and related helpers op basis van de vertaalsleutels in your standaardlocale files. |
| [`routesLocaleLinks`](/api/module-options) | `\{ [key: string]: string \}` | `\{\}` | Map route names to other route names to share the same vertaalbestands. |
| [`plural`](/api/module-options) | `PluralFunc` | `built-in pluralization (form index by count)` | Custom pluralization function. |
| [`disablePageLocales`](/api/module-options) | `boolean` | `false` | Disable per-page vertaalbestands. |
...
| [`debug`](/api/module-options) | `boolean` | `false` | Enable verbose debug logging for localedetectie, route generation, and translation loading. |
| [`vueDevtools`](/api/module-options) | `boolean` | `true` | Register Vue DevTools inspector and timeline in dev (browser extension). |
| [`globalLocaleRoutes`](/api/module-options) | `GlobalLocaleRoutes` | `\{\}` | Global route-level locale configuratie. |
...
| [`noPrefixRedirect`](/api/module-options) | `boolean` | `false` | For `no_prefix` strategy: enable redirect from a locale-prefixed URL (bijv. `/en/about`) to the unprefixed version (`/about`). |
...
| [`localizedRouteNamePrefix`](/api/module-options) | `string` | `'localized-'` | Prefix prepended to localized route names (bijv. `'localized-index'`). |
...
| [`missingWarn`](/api/module-options) | `boolean` | `true` | Show console warnings when a vertaalsleutel is missing. |
| [`hmr`](/api/module-options) | `boolean` | `true` | Enable Hot Module Replacement for vertaalbestands in development. |
...
| [`experimenteel`](/api/module-options) | `Record<string, unknown>` | — | Bucket for experimenteel/unstable options. |

{/* /generated:options-index */}

# Option details

The module provides extensive configuratie options to customize your internationalisatie setup.
...
List of ondersteund locales.
Each entry defines a locale-code plus optional metadata (ISO, direction, display name, etc.).

{/* /generated:option:locales */}

Each locale object ondersteunt:

| Property         | Type      | Required | Description                                           |
| ---------------- | --------- | -------- | ----------------------------------------------------- |
| `code`           | `string`  | ✅       | Unique identifier (bijv., `'en'`)                      |
| `displayName`    | `string`  | ❌       | Human-readable name for switchers (bijv., `'English'`) |
| `iso`            | `string`  | ❌       | ISO code (bijv., `'en-US'`)                            |
...
Je kunt add any custom properties to locale objects. They are passed through to the runtime and accessible via `$getLocales()`:
...
Standaard, custom properties are typed as `unknown`. To get full TypeScript support, use **module augmentation**. Create a declaration file (bijv., `app/i18n.d.ts` or any `.d.ts` included in your `tsconfig`):
...
- `'prefix'` — altijd prefix, including de standaardlocale.
- `'prefix_and_default'` — like `prefix`, but de standaardlocale is also accessible without prefix.
...
Disable per-page vertaalbestands.
...
When enabled, only root-level vertaalbestands are used:
...
Wanneer een vertaalsleutel is missing in de actieve locale, de module looks it up
...
op basis van de vertaalsleutels in your standaardlocale files.

{/* /generated:option:types */}

When enabled, de module emits `.nuxt/i18n.d.ts` during build with typed vertaalsleutels. Set to `false` if you prefer untyped helpers or use an external generator.
...
Maps a route path (bijv. `'/about'`) to an array of allowed locale-codes.
Routes not listed have no restrictions (all locales allowed).

{/* /generated:option:routeLocales */}

Maps a route path (bijv. `'/about'`) to allowed locale-codes. Routes not listed allow all configured locales.
...
Like `routeLocales`, this is meestal set in page components, not handmatig in config.
#### `experimenteel`

{/* generated:option:experimenteel — do not edit; run `pnpm run docs:generate` */}

**Type** `Record<string, unknown>` · **Default** —

Bucket for experimenteel/unstable options.
Contents may change or be removed without notice between minor versions.

{/* /generated:option:experimenteel */}

Most former `experimenteel.*` flags were promoted to top-level options in v3 (for example `hmr`). Prefer documented top-level options. Gebruik `experimenteel` only when following a release note or migration guide that references a specific key.
...
Generate SEO meta tags (`hreflang`, `canonical`, `og:url`, `og:locale`) automatisch.
...
- A concrete URL string (bijv. `'https://example.com'`) — used as-is (highest priority).
...
<Tip>
For routes added in `pages:extend` (especially many sharing one wrapper SFC), build both `pages` and `globalLocaleRoutes` from one list with [`splitLocaleRoutes`](/guides/custom-locale-routes#programmatic-routes-pagesextend--244) — keyed by route **name**, not file path.
</Tip>
...
Map route names to other route names to share the same vertaalbestands.
Bijvoorbeeld, `\{ 'about-us': 'about' \}` means the `about-us` page will use
...
<Danger>
At build time, de module **validates** that every locale-code in your `locales` list matches the `customRegexMatcher` pattern. If any locale-code does not match, **the build will fail** with the error:
...
Enable verbose debug logging for localedetectie, route generation, and translation loading.
...
Disable the file watcher that auto-creates missing vertaalbestands in ontwikkelmodus.
...
Show console warnings when a vertaalsleutel is missing.
...
<Tip>
Je kunt set a custom handler for missing translations using `setMissingHandler` method. Hiermee kun je you to send missing translation errors to error tracking services like Sentry.
</Tip>
...
Set to `false` to disable automatic component registration (bijv. if you don't use them
and want to reduce de module footprint).
...
For the Nuxt module de functie is serialized with `.toString()` into
`.nuxt/i18n.plural.mjs` — it must be self-contained (no imports / outer scope).
A file path string is **not** ondersteund.
...
Gebruik `$tc('apples', count)` or `$tc('cart', \{ count, name \})` when a form needs more than `\{count\}` — pass **`count` and other params in the second argument object**, not as a third argument (third is `defaultValue` only).

De `$tc` call invokes the `plural` function, which:

1. Calls `t(key)` to get the raw translation string (bijv. `"no apples | one apple | \{count\} apples"`)
...
For languages with complex pluralization rules (bijv., Russian, Arabic, Polish), override the `plural` option.

<Danger>
The function is serialized via `.toString()` and injected into a virtual module at build time. Dit betekent:

- **Must use `function` keyword** — NOT shorthand method syntax, NOT arrow functions with external references
- **No imports or external references** — de functie must be fully self-contained (a file path like `plural: '~/i18n/plural.ts'` is **not** ondersteund)
...
**Voorbeeld: Russian pluralization** (4 forms: zero, one, few, many):
...
**Voorbeeld: Simple English (default behavior)**:
...
<Warning>
`plural: '~/i18n/plural.ts'` (or any string path) is **not** ondersteund. The Nuxt module
serializes de functie with `.toString()` into a virtual file; external imports and
helpers from another module are stripped and zal worden `undefined` tijdens runtime. Keep the
function fully inlined in `nuxt.config.ts` (zie de danger note above).
</Warning>
#### `localeCookie`
...
<Warning>
While the configured default is `null` (disabled), de module **automatisch overrides** this to `'user-locale'` when using `strategy: 'no_prefix'`. Dit betekent:

- **`no_prefix`**: Cookie is **altijd enabled** (`'user-locale'`), even if you don't set it explicitly. Dit is required because the URL contains no locale information.
- **All other strategies**: Cookie is `null` (disabled) unless you set it explicitly.

If you set `localeCookie` explicitly, your value is altijd used regardless of strategy.
</Warning>

<Warning>
Bij gebruik van prefix strategies (`prefix`, `prefix_except_default`, `prefix_and_default`) with `redirects: true` (the default), you **must** set `localeCookie` for redirect behavior to work correctly. Without a cookie, the redirect plugin cannot remember the user's locale preference across page reloads, and redirects will only work based on `Accept-Language` header (if `autoDetectLanguage: true`) or `defaultLocale`.
</Warning>
...
De vertalings zal worden fetched from `/\{apiBaseUrl\}/\{routeName\}/\{locale\}/data.json` (bijv., `/api/_locales/index/en/data.json`).
#### `apiBaseClientHost`
...
When `apiBaseClientHost` is set, client-side translations zal worden fetched from `\{apiBaseClientHost\}/\{apiBaseUrl\}/\{routeName\}/\{locale\}/data.json` (bijv., `https://cdn.example.com/_locales/index/en/data.json`).
#### `apiBaseServerHost`
...
When `apiBaseServerHost` is set, server-side translations zal worden fetched from `\{apiBaseServerHost\}/\{apiBaseUrl\}/\{routeName\}/\{locale\}/data.json` (bijv., `https://internal-cdn.example.com/_locales/index/en/data.json`).
...
`mode: 'source'` keeps layer files compact and merges root/page/fallback tijdens runtime through the built-in `/_locales` route (or Edge `assets:i18n`). Standaard it disables public asset copies and prerendered payload routes.

<Warning>
With `mode: 'source'`, `publicAssets` defaults to `false` (`prerenderRoutes` is altijd off unless you opt in). Pure static hosting without a Nitro/edge runtime therefore cannot load translations on the client unless you enable one of these outputs, keep `serverHandler` available tijdens runtime, or host payloads externally.
...
<Warning>
When `apiBaseServerHost` or `apiBaseClientHost` is set, de module fetches already merged JSON from that origin. External hosts must serve the same `/\{apiBaseUrl\}/:page/:locale/data.json` responses as the built-in route. `mode: 'source'` applies to local payload dirs, not to an external CDN unless that CDN also serves runtime-merged payloads.
</Warning>

Je kunt ook disable duplicated local outputs handmatig and serve translation payloads from a CDN or object storage:
...
Gebruik `publicDir` to change the public output folder when payloads are copied. It defaults to `apiBaseUrl` (`_locales`). In premerged mode `publicAssets` writes `\{page\}/\{locale\}/data.json` there (same paths the client fetches). `prerenderRoutes` is an optional Nitro prerender of the handler routes — meestal redundant when that tree was already copied.
...
(bijv. `es-ES` → also `es`). The first regional locale in `locales` claims
the bare tag for that language. Routing `code` is nooit used — only `iso || code`.
...
(bijv. `/en/about`) to the unprefixed version (`/about`).
...
Internal Nuxt paths (`/__nuxt_error`, etc.) are altijd excluded automatisch.
...
Prefix prepended to localized route names (bijv. `'localized-index'`).
...
When not provided, de module falls back to `Date.now()` (non-deterministic).
For reproducible/rolling deployments, set this to a stable value
(bijv. a git SHA or build number).

{/* /generated:option:dateBuild */}

Standaard de module fingerprints your translation sources (SHA-256 over every
...
It falls back to `Date.now()` only when there are no vertaalbestands to hash.

Set it explicitly to take control (bijv. to tie payload URLs to a release rather than to content):
...
- Reaches only responses served through Nitro. Payloads copied into `public/` and served
  by the hosting platform take that platform's headers — zie de Performance guide.

Analogous to `@nuxtjs/i18n` experimenteel `httpCacheDuration` (v10.2.0), but as an
...
Enable Hot Module Replacement for vertaalbestands in development.
When `true`, changes to JSON vertaalbestands trigger an automatic reload
...
<Tip>
For most projects the default (unlimited, no expiration) is fine — translations are small and finite. Echter, if your project has **thousands of pages** with `disablePageLocales: false` and **many locales**, the server cache can grow significantly. In long-running Node.js servers this may lead to excessive memory usage.

- **`cacheMaxSize`** — caps the number of cached entries. Useful for bounding memory.
- **`cacheTtl`** — ensures stale translations are eventually reloaded from storage. Useful for serverless environments or when translations change tijdens runtime.

**Formula for estimating max entries**: `number_of_locales × (number_of_pages + 1)`. Bijvoorbeeld, 10 locales × 500 pages = ~5010 entries.
</Tip>
