---
title: "Package-API's"
description: "Referentie-index voor elk @i18n-micro-workspacepakket, met de geëxporteerde API's en types die zijn gegenereerd uit bron-snapshots waar CI tegen controleert."
---

# Package-API's

`nuxt-i18n-micro` is opgebouwd uit een reeks kleine pakketten, waarvan de meeste zelfstandig bruikbaar zijn — `@i18n-micro/vue` in een gewone Vue-app, `@i18n-micro/core` overal.

Elke pagina hieronder wordt gegenereerd uit de API-snapshot die
[`pnpm run api:surface`](https://github.com/s00d/nuxt-i18n-micro/blob/main/docs/guide/maintenance-commands.md#api-surface) controleert tegen de
TypeScript-bronnen. Zo kan er geen export verdwijnen zonder dat deze referentie wijzigt of CI faalt.
...
- [Integraties](/integrations/overview) — gidsen voor het gebruik van deze pakketten per framework
- [Module-opties](/api/module-options) — opties die de Nuxt-module accepteert
- [Methoden](/api/methods) — de runtime-helpers die beschikbaar zijn binnen componenten
