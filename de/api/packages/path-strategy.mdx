...
description: "Öffentliche API des Pakets @i18n-micro/path-strategy: Locale-Pfadauflösung, Redirects und SEO-Helfer zur Laufzeit, aus dem Quellcode generiert."
---

# `@i18n-micro/path-strategy`

38 Exports über 6 Einstiegspunkte.
...
<code>NoPrefixPathStrategy</code> — 36 members, identisch mit `NoPrefixPathStrategy` oben.
<code>Strategy</code> — 36 members, identisch mit `NoPrefixPathStrategy` oben.
...
<code>PrefixPathStrategy</code> — 36 members, identisch mit `PrefixPathStrategy` oben.
<code>Strategy</code> — 36 members, identisch mit `PrefixPathStrategy` oben.
...
<code>PrefixAndDefaultPathStrategy</code> — 37 members, identisch mit `PrefixAndDefaultPathStrategy` oben.
<code>Strategy</code> — 37 members, identisch mit `PrefixAndDefaultPathStrategy` oben.
...
<code>PrefixExceptDefaultPathStrategy</code> — 44 members, identisch mit `PrefixExceptDefaultPathStrategy` oben.
<code>Strategy</code> — 44 members, identisch mit `PrefixExceptDefaultPathStrategy` oben.
...
<code>PathStrategy</code> — 22 members, identisch mit `PathStrategy` oben.
<code>PathStrategyContext</code> — 15 members, identisch mit `PathStrategyContext` oben.
<code>ResolvedRouteLike</code> — 6 members, identisch mit `ResolvedRouteLike` oben.
<code>RouteLike</code> — 6 members, identisch mit `RouteLike` oben.
<code>RouterAdapter</code> — 3 members, identisch mit `RouterAdapter` oben.
<code>SwitchLocaleOptions</code> — 1 members, identisch mit `SwitchLocaleOptions` oben.
