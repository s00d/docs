...
description: "كل خيار تقبله الوحدة، مع نوعه وقيمته الافتراضية والغرض منه — يُقرأ في وقت البناء من تعريف الأنواع."
---

# مرجع خيارات الوحدة

كل خيار يقبله `nuxt-i18n-micro`، يُقرأ في وقت البناء من واجهة `ModuleOptions` في
[`packages/types/src/index.ts`](https://github.com/s00d/nuxt-i18n-micro/blob/main/packages/types/src/index.ts).
لم يُكتب أي شيء هنا يدويًا، لذا لا يمكن أن يتخلّف عن الشيفرة — الأنواع والقيم الافتراضية والأوصاف تأتي من الإعلان نفسه.

لمعرفة *معنى* هذه الخيارات معًا — أي التوليفات تكون منطقية، وما الذي يتغيّر عند تبديل الاستراتيجية، مع أمثلة عملية — اطّلع على [التهيئة](/guides/configuration). هذه الصفحة هي القائمة الشاملة؛ وتلك هي التفسير.
...
تظهر الخيارات المتداخلة تحت مسارها المنقّط، فعلى سبيل المثال `translationPayloads.mode` هو مفتاح `mode` داخل كائن `translationPayloads`.

{/* generated:module-options — do not edit; run `pnpm run docs:generate` */}

## اللغات والتوجيه

اللغات الموجودة وكيفية ظهورها في عنوان URL.
...
مكان ملفات الترجمة وكيفية حلّ المفاتيح.
...
كيف تصل الترجمات إلى المتصفح. يشرح [الأداء](/guides/performance) ما تُغيّره هذه في وقت التشغيل.
...
وسوم الميتا المُولَّدة لكل صفحة مترجمة.
...
اختيار لغة للزائر الذي لم يختر واحدة.
...
- [التهيئة](/guides/configuration) — كيف تعمل هذه الخيارات معًا
- [الأداء](/guides/performance) — ما تُغيّره خيارات الحمولة في وقت التشغيل
- [واجهات الحزم البرمجية](/api/packages/overview) — واجهة API المُصدَّرة لكل حزمة في مساحة العمل
