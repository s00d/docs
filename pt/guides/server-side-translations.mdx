---
title: "Utilizar traduções no lado do servidor"
description: "Utilize useTranslationServerMiddleware e useLocaleServerMiddleware para traduzir strings e ler locales em rotas de API do Nuxt I18n Micro."
---
...
Nuxt I18n Micro suporta server-side translations and locale information, allowing you to translate content and access locale details on the server. This is particularly useful for APIs or server-rendered applications where localization is needed before reaching the client.
...
With `translationPayloads.mode: 'source'`, compact source files are merged at runtime when `loadTranslationsFromServer()` (or the `/_locales` route) is called — on **Edge** via Nitro `serverAssets`, on **Node** from the optional public copy. Consulte [Performance Guide — Serverless Payload Output](/guides/performance#serverless-payload-output) and [Cache & Storage Architecture](/api/cache) for details.
...
Both middleware functions are automaticamente available in all server routes.
...
Pode seamlessly translate content in any `eventHandler` by using the translation middleware.
...
export default defineEventHandler(async (event) => {
  ...
  return {
    message: t('greeting'), // Devolve the translated value for the key "greeting"
  }
})
```
...
- The user's locale is detected automaticamente from query parameters, cookies, or headers.
- O `t` function retrieves the appropriate translation for the detected locale.
...
export default defineEventHandler(async (event) => {
  ...
  return {
    message: t('welcome'), // Devolve the French translation for "welcome"
  }
})
```
...
Both middleware functions automaticamente determine the user's locale using the following priority order:
...
Pode combine locale information with translation middleware for comprehensive internationalization:
