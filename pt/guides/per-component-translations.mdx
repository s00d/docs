...
description: "Utilize $defineI18nRoute e traduções por componente para adicionar chaves com âmbito local, restringir locales e sobrepor rotas por componente."
---
...
You normally only call `$defineI18nRoute` in pages — the module handles build-time extraction automaticamente.

## 🔧 `$defineI18nRoute` Function

O `$defineI18nRoute` function configures route behavior based on the current locale, offering a versatile solution to:
...
O `$defineI18nRoute` parser suporta various JavaScript configurations:
