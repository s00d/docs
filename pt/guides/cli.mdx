---
title: "CLI do Nuxt I18n Micro"
description: "Utilize a CLI do nuxt-i18n-micro para inicializar projetos, sincronizar ficheiros de tradução, gerar tipos TypeScript e migrar de outros módulos i18n."
---
...
`nuxt-i18n-micro-cli` is a command-line tool designed to streamline the localization and internationalization process in Nuxt.js projects using the `nuxt-i18n-micro` module (Nuxt I18n Micro). Fornece utilities to extract translation keys from your codebase, manage translation files, synchronize translations across locales, and automate the translation process using external translation services.

Este guia will walk you through installing, configuring, and using `nuxt-i18n-micro-cli` to effectively manage your project's translations. Package on [npmjs.com](https://www.npmjs.com/package/nuxt-i18n-micro-cli).
...
**Limitations**: separate npm package (not bundled with the module); does not read `nuxt.config` automaticamente; outputs `$t(...)`; complex dynamic strings may need `extract` / `search` instead.

### 📊 `stats` Command

**Description**: O `stats` command is used to display translation statistics for each locale in your Nuxt.js project. It helps you understand the progress of your translations by showing how many keys are translated compared to the total number of keys available.
...
**Description**: O `translate` command automaticamente translates missing keys using external translation services. This command simplifies the translation process by leveraging APIs from services like Google Translate, DeepL, and others to fill in missing translations.
...
O `translate` command suporta multiple translation services. Some of the supported services are:
...
**Description**: O `export-csv` command exports translation keys and values from JSON files, including their file paths, into a CSV format. This command is useful for teams who prefer working with translation data in spreadsheet software.
...
**Description**: O `import-csv` command imports translation data from CSV files, updating the corresponding JSON files. This is useful for applying bulk translation updates from spreadsheets.
...
**Description**: O `replace-values` command allows you to perform bulk replacements of translation values across all locales. It suporta both simple text replacements and advanced replacements using regular expressions (regex). Também pode use capturing groups in regex patterns and reference them in the replacement string, making it ideal for more complex replacement scenarios.
...
- `--search`: The text or regex pattern to search for in translations. Este é um required option.
- `--replace`: The replacement text to be used for the found translations. Este é um required option.
...
Use capturing groups to dynamically insert part of the matched string into the replacement. Por exemplo, replace "Hello [name]" with "Hi [name]" while keeping the name intact:
