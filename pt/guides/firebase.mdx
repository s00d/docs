---
title: "Implementar no Firebase"
description: "Implemente uma aplicação Nuxt I18n Micro no Firebase App Hosting: build, prerender, considerações de cache e como estruturar os payloads de tradução."
---
...
Deve see `data.json` for each locale under each page.
...
Firebase automaticamente handles Gzip/Brotli compression, but ensure your build is optimized:
...
`Nuxt I18n Micro` automaticamente lazy-loads translations on a per-page basis. Translations are only loaded when needed, reducing initial bundle size. This is built-in functionality - no additional configuration required.
...
<Tip>
The module automaticamente sets proper `Content-Type` headers starting from version **v3.0.0+**. If you're experiencing issues, update to the latest version:
