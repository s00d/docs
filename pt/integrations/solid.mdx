---
title: "Pacote SolidJS (`@i18n-micro/solid`)"
description: "Utilize o @i18n-micro/solid para adicionar signals, componentes e um provider para traduções e encaminhamento consciente do locale em aplicações SolidJS."
---

# Solid Package (`@i18n-micro/solid`)

O `@i18n-micro/solid` package fornece um lightweight, high-performance internationalization solution for SolidJS applications. It shares the same core logic as Nuxt I18n Micro, offering reactive translations, route-specific support, and full TypeScript support.

## Visão geral

`@i18n-micro/solid` foi concebido para SolidJS applications that need internationalization without the Nuxt framework. Fornece:

- **Leve** — Uses shared core logic from `@i18n-micro/core`
- **Reativo** — Automatic component updates when translations change using SolidJS signals
...
- **Agnóstico ao router** — Works with any router or without a router via `I18nRoutingStrategy` interface
- **Integração com DevTools** — Built-in DevTools support for managing translations
...
O `useI18n` hook requires locales configuration to be provided via `I18nProvider`. Tem de provide both `locales` and `defaultLocale`:
...
interface I18nRoutingStrategy {
  ...
  /**
   * (Optional) Devolve reactive accessor for current path
...
2. **Exported from package**: O `createSolidRouterAdapter` is now exported directly from `@i18n-micro/solid`, so you don't need to create a separate router adapter file for basic use cases. However, you can still create custom adapters if needed (see "Creating Custom Router Adapters" section below).

3. **Reactivity**: The built-in adapter automaticamente provides `getCurrentPathAccessor()` for proper reactivity in SolidJS components.
...
| `getCurrentPathAccessor()` | ⚠️ **Highly Recommended** | Devolve reactive accessor for current path (SolidJS-specific). **Required for proper reactivity** - without this, active link states and path-dependent UI won't update reactively. |
...
### Exemplo 1: Custom Router with Signals

Este exemplo shows how to create an adapter for a custom router that uses signals:
...
O `useI18n` hook fornece umccess to i18n functionality in SolidJS components.
...
const MyComponent = () => {
  ...
  return (
    <>
      ...
      {/* External link (automaticamente detected) */}
...
Pluralization is handled automaticamente based on the count:
...
Pode provide a custom pluralization function:
...
Consulte playground implementation in `packages/solid/playground/src/` for a complete working example.
