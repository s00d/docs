---
title: "Pacote Astro (`@i18n-micro/astro`)"
description: "Utilize o @i18n-micro/astro para servir conteúdo localizado em SSR e static com deteção de locale, componentes por framework e traduções por rota."
---

# Astro Package (`@i18n-micro/astro`)

O `@i18n-micro/astro` package fornece um lightweight, high-performance internationalization solution for Astro applications. It shares the same core logic as Nuxt I18n Micro, offering SSR support, automatic locale detection, Astro components, and full TypeScript support.

## Visão geral

`@i18n-micro/astro` foi concebido para Astro applications that need internationalization with server-side rendering support. Fornece:

- **Leve** — Uses shared core logic from `@i18n-micro/core`
...
- **Pluralização** — Built-in plural form support
- **Formatação** — Number, date, and relative time formatting
- **Com tipagem segura** — Full TypeScript support
- **Agnóstico ao router** — Works with any routing strategy or without routing
- **Integração com DevTools** — Optional development tools via Vite plugin
- **SEO Optimized** - Automatic meta tags and hreflang generation

## Instalação

Instale o pacote com o seu gestor de pacotes preferido:
...
### Configuração básica (Without Router Adapter)
...
**Nota:** O `createAstroRouterAdapter` is now exported from the package, so you don't need to create your own router adapter file. However, you can still create a custom adapter if needed.
...
- O `virtual:i18n-micro/config` module works in all environments (it's generated at build time)
...
O `I18nRoutingStrategy` interface defines the contract between i18n and your routing:
...
**Nota:** O `createAstroRouterAdapter` is now exported from `@i18n-micro/astro`, so you don't need to create your own adapter file for basic use cases.

### Setting Router Adapter in Middleware

Pode set the routing strategy in middleware:
...
O `I18nRoutingStrategy` interface defines how i18n interacts with your routing:

```typescript
interface I18nRoutingStrategy {
  /**
   * Devolve current path (without locale prefix if needed, or full path)
...
O `I18nRoutingStrategy` interface defines the following methods:

| Method                                                      | Required | Description                                                 |
| ----------------------------------------------------------- | -------- | ----------------------------------------------------------- |
| `getCurrentPath()`                                          | ✅       | Devolve the current path (used for active link detection)   |
...
| `getRoute()`                                                | ❌       | Devolve current route object with query params              |
...
### Exemplo 1: Subdomain-Based Locale Routing

Este exemplo shows how to implement locale routing using subdomains (e.g., `en.example.com`, `fr.example.com`):
...
### Exemplo 2: Query Parameter-Based Locale Routing

Este exemplo uses query parameters for locale (e.g., `/?locale=fr`):
...
### Exemplo 3: Suffix-Based Locale Routing

Este exemplo uses locale suffixes (e.g., `/about.en`, `/about.fr`):
...
### Exemplo 4: Integration with Third-Party Router

Este exemplo shows how to integrate with a custom routing library:
...
Pode extend the built-in adapter and override specific methods:
...
⚠️ **Important**: O `loadTranslationsIntoI18n` function uses Node.js filesystem APIs and will **NOT work** in Edge runtime environments (Cloudflare Workers, Vercel Edge, Deno Deploy). For these environments, use `import.meta.glob` instead.
...
2. Pode import the config in your middleware to avoid duplicating settings
3. **Tem de explicitly load translations** in your middleware:
...
O `loadTranslationsIntoI18n` function uses Node.js filesystem APIs (`node:fs`) and **will NOT work** in Edge runtime environments such as:
...
**Nota:** By default, files in the `pages/` folder are treated as route-specific translations. The utility automaticamente maps them to route names based on the directory structure.
...
The middleware automaticamente detects the locale from:
...
Provides helper functions for translations and routing in Astro components and pages. The utility automaticamente uses the routing strategy from `Astro.locals.routingStrategy` if available, with fallback implementations if not provided.
...
Localized link component that automaticamente handles locale prefixes using the routing strategy from `Astro.locals.routingStrategy` if available.
...
Astro Islands allow you to use interactive components (Vue, React, Svelte) within Astro pages. O `@i18n-micro/astro` package provides framework-specific adapters for seamless i18n integration in islands.
...
**Devolve:** `I18nClientProps` object with:
...
Initializes i18n provider for Vue islands. Devolve a reactive state ref.
...
This package suporta DevTools integration via the `@i18n-micro/devtools-ui` Vite plugin. Consulte [DevTools UI Package documentation](/integrations/devtools-ui) for details.

## Utilização avançada

### Custom Router Adapter

Pode create custom router adapters for specific routing needs:
...
Pode define translations specific to routes:
...
O `clone()` method allows creating per-request instances that share the same cache, improving performance in SSR scenarios.

## Suporte a TypeScript

### Global Types (App.Locals)

The integration automaticamente injects types for `App.Locals` via Astro's `injectTypes` hook. Deve see `i18n`, `locale`, `defaultLocale`, `locales`, `currentUrl`, and `routingStrategy` available in your Astro components and middleware without any manual configuration.

**Important:** Types are automaticamente available when you add the integration to your `astro.config.mjs`. If you're not using the integration (only middleware), you may need to manually add the type reference (see Manual Type Reference below).
...
export const onRequest: MiddlewareHandler = (context, next) => {
  // Types are automaticamente available
...
If for some reason types are not showing up automaticamente (e.g., with custom TypeScript configuration or older Astro versions), you can manually add the reference to your `src/env.d.ts` file:
...
Pode create type-safe translation keys:
