---
title: "Pacote Node.js (`@i18n-micro/node`)"
description: "Utilize o @i18n-micro/node para adicionar carregamento e formatação de traduções a Nitro, Node.js, workers e outros ambientes de servidor sem frontend."
---

# 🌐 Node.js Runtime

Use `@i18n-micro/node` to add i18n translations to any Node.js application, CLI tool, or backend service. Este pacote fornece the same translation logic as the Nuxt module, but for pure Node.js environments.
...
The package suporta the same file structure as the Nuxt module:
...
Load translations from directory (recursive, suporta pages structure).
...
// With currentRoute
i18n.setRoute('home')
i18n.t('title') // Uses 'home' route automaticamente
```
...
O `currentRoute` feature allows you to set a route context that will be used by default in all `t()` calls:
...
// Now t() will automaticamente use 'home' route
...
Pode utilizar `@i18n-micro/node` in Nuxt server API routes to provide server-side translations:
...
Pode implement custom locale detection logic based on your needs:
...
5. **Use route context**: Set `currentRoute` to automaticamente use route-specific translations
