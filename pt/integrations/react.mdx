---
title: "Pacote React (`@i18n-micro/react`)"
description: "Utilize o @i18n-micro/react para adicionar hooks, componentes e um provider para traduções e encaminhamento consciente do locale em aplicações React."
---

# React Package (`@i18n-micro/react`)

O `@i18n-micro/react` package fornece um lightweight, high-performance internationalization solution for React applications. It shares the same core logic as Nuxt I18n Micro, offering reactive translations, route-specific support, and full TypeScript support.

## Visão geral

`@i18n-micro/react` foi concebido para React applications that need internationalization without the Nuxt framework. Fornece:

- **Leve** — Uses shared core logic from `@i18n-micro/core`
- **Reativo** — Automatic component updates when translations change using `useSyncExternalStore`
...
- **Agnóstico ao router** — Works with any router or without a router
...
O pacote requer React and optionally React Router:
...
O `useI18n` hook fornece umccess to i18n functionality in React components. It uses `useSyncExternalStore` internally for reactivity, ensuring components re-render when translations or locale change. The hook automaticamente uses the routing strategy from `I18nProvider` if available.
...
### Exemplo 1: Next.js Router Adapter

Este exemplo shows how to create an adapter for Next.js App Router:
...
### Exemplo 2: TanStack Router Adapter

Este exemplo shows how to create an adapter for TanStack Router:
...
Consulte "Full Application Setup" example above for a complete router integration example using the adapter pattern.
