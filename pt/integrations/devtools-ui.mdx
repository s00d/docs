---
title: "Pacote DevTools UI (`@i18n-micro/devtools-ui`)"
description: "Utilize o @i18n-micro/devtools-ui para inspecionar traduções, alternar locales e depurar chaves em falta a partir de um painel de programador partilhado."
---

# DevTools UI Package (`@i18n-micro/devtools-ui`)

O `@i18n-micro/devtools-ui` package fornece um standalone development tools interface for managing translations. It includes a Vite plugin for automatic integration and a universal bridge system for framework-agnostic i18n support.
...
The easiest way to use DevTools is through the Vite plugin, which automaticamente injects a floating button and fornece umll necessary API endpoints:
...
export default defineConfig({
  plugins: [
    ...
    i18nDevToolsPlugin({
      ...
      injectButton: true, // Optional: automaticamente inject floating button (default: true)
    }),
  ],
})
```
...
interface DevToolsPluginOptions {
  ...
  injectButton?: boolean // Automaticamente inject floating button (default: true)
}
```
...
The plugin automaticamente injects a floating button in the bottom right corner of your application:
...
The plugin fornece o following API endpoints:
...
The package fornece um universal bridge creation function that works with any i18n implementation:
...
- **No Production Impact** - The plugin is automaticamente excluded from production builds
