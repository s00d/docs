---
title: "Pacote Vue (`@i18n-micro/vue`)"
description: "Utilize o @i18n-micro/vue para adicionar composables, componentes e um plugin para traduções e encaminhamento consciente do locale em aplicações Vue 3 autónomas."
---

# Vue Package (`@i18n-micro/vue`)

O `@i18n-micro/vue` package fornece um lightweight, high-performance internationalization solution for Vue 3 applications. It shares the same core logic as Nuxt I18n Micro, offering reactive translations, route-specific support, and full TypeScript support.

## Visão geral

`@i18n-micro/vue` foi concebido para Vue 3 applications that need internationalization without the Nuxt framework. Fornece:

- **Leve** — Uses shared core logic from `@i18n-micro/core`
- **Reativo** — Automatic component updates when translations change
...
- **Integração com DevTools** — Built-in Vue DevTools support for managing translations
...
O pacote requer Vue 3 and optionally Vue Router:
...
const i18n = createI18n({
  locale: 'en',
  fallbackLocale: 'en',
  // Automaticamente provided to the app - no need for manual provide calls
  locales: [
...
const i18n = createI18n({
  locale: 'en',
  fallbackLocale: 'en',
  // Automaticamente provided to the app
  locales: [
    { code: 'en', displayName: 'English', iso: 'en-US' },
    { code: 'fr', displayName: 'Français', iso: 'fr-FR' },
  ],
...
O `useI18n` composable requires locales configuration to be provided via Vue's dependency injection system.

**Recommended approach**: Pass `locales` and `defaultLocale` directly to `createI18n` - they will be automaticamente provided:
...
const i18n = createI18n({
  locale: 'en',
  fallbackLocale: 'en',
  locales, // Automaticamente provided
  defaultLocale: 'en', // Automaticamente provided
  messages: {
    /* ... */
  },
})
...
Pode set the routing strategy in two ways:
...
Pode create adapters for any router library. Here's an example for a custom router:
...
- `global`: O `VueI18n` instance for direct access
...
Devolve the full translation tree for the active locale and route — the same dictionary `t()` reads from.

##### `setTranslation(key: TranslationKey, value: unknown): void`

Replaces the value at `key` in the active dictionary. Este é um replace, not a merge; use `mergeTranslations` when siblings should survive.
...
O `useI18n` composable fornece umccess to i18n functionality in Vue components.
...
export async function render(url: string) {
  ...
  app.use(router)
  app.use(i18n)
  // locales and defaultLocale are automaticamente provided by the plugin
...
Consulte playground implementation in `packages/vue/playground/src/` for a complete working example.
