---
title: "Pacote Preact (`@i18n-micro/preact`)"
description: "Utilize o @i18n-micro/preact para adicionar hooks, componentes e um provider para traduções e encaminhamento consciente do locale em aplicações Preact."
---

# Preact Package (`@i18n-micro/preact`)

O `@i18n-micro/preact` package fornece um lightweight, high-performance internationalization solution for **Pure Preact** applications. It shares the same core logic as Nuxt I18n Micro, offering reactive translations, route-specific support, and full TypeScript support.

## Visão geral

`@i18n-micro/preact` foi concebido para Pure Preact applications (without React compatibility layer) that need internationalization. Fornece:

- **Leve** — Uses shared core logic from `@i18n-micro/core`
- **Reativo** — Automatic component updates when translations change using `useState` + `useEffect` (Pure Preact approach)
- **Pure Preact** - Uses Preact's native JSX runtime (`h`) for minimal bundle size (~3KB)
- **Agnóstico ao router** — Works with any router (wouter, preact-router, or custom) or without a router
...
- **Integração com DevTools** — Vite plugin for managing translations during development

**Nota:** The built-in router adapter (`createPreactRouterAdapter` / `createBrowserHistoryAdapter`) uses the native History API and works with any router that listens to `popstate` events (e.g., `wouter-preact`, `preact-router`). For custom routing needs, you can create your own adapter.
...
### Configuração básica (Without Router)
...
**Nota:** O `createPreactRouterAdapter` (aliased as `createBrowserHistoryAdapter`) is exported from the package and fornece um basic History API-based adapter. For router-specific adapters (like wouter), you can create custom adapters as shown above.
...
O `I18nRoutingStrategy` interface defines the contract between i18n and your router:
...
O `useI18n` hook fornece umccess to i18n functionality in Preact components. It uses `useState` + `useEffect` internally for reactivity (Pure Preact approach), ensuring components re-render when translations or locale change. The hook automaticamente uses the routing strategy from `I18nProvider` if available.
...
Resolve path for a specific locale. Devolve localized path using the routing strategy.
...
Translation component with support for pluralization, formatting, and HTML rendering. Automaticamente uses the routing strategy from `I18nProvider` if available.
...
Localized link component that automaticamente handles locale prefixes using the routing strategy from `I18nProvider` if available.
...
- Supports external links (detected automaticamente)
...
O `I18nRoutingStrategy` interface defines how i18n interacts with your router:
...
### Exemplo 1: Wouter Adapter

Este exemplo shows how to create an adapter for `wouter-preact`:
...
### Exemplo 2: Preact Router Adapter

Este exemplo shows how to create an adapter for `preact-router`:
...
### Exemplo 3: Query Parameter-Based Locale Routing
...
`addTranslations` loads base translations (stored as `index`). When you call `addRouteTranslations`, these base translations are automaticamente merged in. Page-specific keys override base ones.
