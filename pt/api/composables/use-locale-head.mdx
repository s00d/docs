...
O composable `useLocaleHead` é um utilitário do `Nuxt I18n Micro` que o ajuda a gerir atributos de SEO e meta tags HTML para rotas localizadas. Gera dinamicamente atributos para o `lang` e o `dir` do documento HTML e cria tags `meta` e `link` para melhorar o SEO do seu conteúdo localizado.
...
## Quando utilizar antes o `useI18nHead`

Quando `meta: true` (predefinição), prefira o [`useI18nHead`](/api/composables/use-i18n-head) para substituições **por página** (hreflang personalizados, `og:title` extra, desativação parcial). Utilize `useLocaleHead` diretamente apenas quando `meta: false` ou quando gere o `useHead` por si.
