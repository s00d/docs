...
description: "Substituições ao nível da página para meta SEO i18n e tags de link."
---

# Composable `useI18nHead`

O `useI18nHead` permite-lhe personalizar tags de SEO i18n **por página** sem um plugin meta personalizado. Quando `meta: true`, o módulo funde o seu input sobre a saída de [`useLocaleHead`](/api/composables/use-locale-head).

Casos típicos:

- **Artigos de CMS / blog** — apenas alguns locales estão traduzidos; os URLs alternativos diferem por idioma (slugs ou caminhos diferentes).
- **Canónico dinâmico** — o URL canónico tem de corresponder ao URL do artigo devolvido pela API, e não a `$switchLocalePath`.
- **Open Graph adicional** — `og:title`, `og:description`, `og:image` a par do `og:locale` / `og:url` gerados automaticamente.
- **Controlo parcial** — manter o canónico e `og:url` gerados pelo módulo, mas substituir apenas `hreflang` e `og:locale:alternate`.
...
## Exemplo 4 — Substituir canonical e `og:url` a partir de dados da API
...
## Exemplo 7 — Dois tipos de conteúdo, o mesmo padrão (notícias vs guias)
...
## Exemplo 8 — Desativar os alternates incorporados e fornecer a sua própria lista
...
## Exemplo 10 — Página de produto: desativar hreflang numa experiência de um único locale
...
## Exemplo 11 — Atualizações reativas após um fetch no cliente
...
- [SEO Guide](/guides/seo) — automatic meta generation and page-level overrides
