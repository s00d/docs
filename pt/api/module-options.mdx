...
description: "Todas as opções que o módulo aceita, com o tipo, predefinição e propósito — lidas em build-time a partir da definição de tipo."
---

# Referência de opções do módulo

Todas as opções que o `nuxt-i18n-micro` aceita, lidas em build-time a partir da interface `ModuleOptions` em
[`packages/types/src/index.ts`](https://github.com/s00d/nuxt-i18n-micro/blob/main/packages/types/src/index.ts).
Nada aqui é escrito à mão, portanto não pode ficar atrás do código — tipos, predefinições e
descrições vêm da própria declaração.

Para saber o que estas opções *significam* em conjunto — que combinações fazem sentido, o que muda quando
alterna a estratégia, exemplos concretos — leia [Configuração](/guides/configuration). Esta
página é a lista exaustiva; aquela é a explicação.
...
As opções aninhadas aparecem no seu caminho por pontos, por isso `translationPayloads.mode` é a
chave `mode` dentro do objeto `translationPayloads`.
...
Como as traduções chegam ao navegador. [Desempenho](/guides/performance) explica o que estas alteram em runtime.
...
- [Configuração](/guides/configuration) — como estas opções funcionam em conjunto.
- [Desempenho](/guides/performance) — o que as opções de payload alteram em runtime.
- [APIs de pacotes](/api/packages/overview) — a API exportada de cada pacote da workspace.
