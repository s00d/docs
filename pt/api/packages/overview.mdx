---
title: "APIs de pacotes"
description: "Índice de referência para cada pacote do workspace @i18n-micro, listando APIs exportadas e tipos gerados a partir dos snapshots de código-fonte que a CI valida."
---

# APIs de pacotes

O `nuxt-i18n-micro` é construído a partir de um conjunto de pequenos pacotes, a maioria dos quais utilizáveis por si só. Por exemplo, `@i18n-micro/vue` numa aplicação Vue simples, ou `@i18n-micro/core` em qualquer lugar.

Cada página abaixo é gerada a partir do snapshot da API que
[`pnpm run api:surface`](https://github.com/s00d/nuxt-i18n-micro/blob/main/docs/guide/maintenance-commands.md#api-surface) valida contra o código-fonte TypeScript. Assim, uma exportação não pode desaparecer sem que esta referência mude ou a CI falhe.

{/* generated:packages-index — do not edit; run `pnpm run docs:generate` */}

| Pacote | Pontos de entrada | Exportações |
| --- | --- | --- |
...
- [Integrações](/integrations/overview) — guias para utilizar estes pacotes em cada framework
- [Opções do módulo](/api/module-options) — opções aceites pelo módulo Nuxt
- [Métodos](/api/methods) — os helpers de runtime disponíveis dentro de componentes
