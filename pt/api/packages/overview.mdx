...
description: "A API exportada de cada pacote da workspace, lida em build-time a partir dos snapshots que o CI valida contra o código-fonte."
---

# APIs de pacotes

O `nuxt-i18n-micro` é construído a partir de um conjunto de pacotes pequenos, a maioria dos quais utilizáveis
de forma isolada — `@i18n-micro/vue` numa aplicação Vue simples, `@i18n-micro/core` em qualquer lado.

Cada página abaixo é gerada a partir do snapshot de API que
[`pnpm run api:surface`](https://github.com/s00d/nuxt-i18n-micro/blob/main/docs/guide/maintenance-commands.md#api-surface) verifica contra os
sources TypeScript, portanto um export não pode desaparecer sem que esta referência mude
ou o CI falhe.
...
- [Integrações](/integrations/overview) — guias de utilização destes pacotes em cada framework.
- [Opções do módulo](/api/module-options) — opções aceites pelo módulo Nuxt.
- [Métodos](/api/methods) — os helpers de runtime disponíveis nos componentes.
