---
title: "API של חבילות"
description: "אינדקס הפניות עבור כל חבילת workspace של @i18n-micro, המפרט APIs מיוצאים וטיפוסים שנוצרו מתמונות מקור שה־CI בודק מולן."
---

# API של חבילות

`nuxt-i18n-micro` בנוי מקבוצה של חבילות קטנות, שרובן שמישות בעצמן — `@i18n-micro/vue` באפליקציית Vue רגילה, `@i18n-micro/core` בכל מקום.

כל עמוד למטה נוצר מתמונת ה־API ש־[`pnpm run api:surface`](https://github.com/s00d/nuxt-i18n-micro/blob/main/docs/guide/maintenance-commands.md#api-surface) בודק מול מקורות ה־TypeScript, כך שייצוא לא יכול להיעלם ללא שינוי בהפניה זו או כישלון של ה־CI.

{/* generated:packages-index — do not edit; run `pnpm run docs:generate` */}

| חבילה | נקודות כניסה | ייצואים |
| --- | --- | --- |
...
- [אינטגרציות](/integrations/overview) — מדריכים לשימוש בחבילות אלה בכל framework
- [אפשרויות מודול](/api/module-options) — אפשרויות המתקבלות על ידי מודול Nuxt
- [מתודות](/api/methods) — עוזרי זמן הריצה הזמינים בתוך רכיבים
