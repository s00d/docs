...
description: "API ציבורי של החבילה @i18n-micro/path-strategy: פתרון נתיב שפה בזמן ריצה, הפניות מחדש ועוזרי SEO, שנוצרו מהמקור."
---

# `@i18n-micro/path-strategy`

38 ייצואים ב־6 נקודות כניסה.
...
<code>NoPrefixPathStrategy</code> — 36 חברים, זהה ל־`NoPrefixPathStrategy` למעלה.
<code>Strategy</code> — 36 חברים, זהה ל־`NoPrefixPathStrategy` למעלה.
...
<code>PrefixPathStrategy</code> — 36 חברים, זהה ל־`PrefixPathStrategy` למעלה.
<code>Strategy</code> — 36 חברים, זהה ל־`PrefixPathStrategy` למעלה.
...
<code>PrefixAndDefaultPathStrategy</code> — 37 חברים, זהה ל־`PrefixAndDefaultPathStrategy` למעלה.
<code>Strategy</code> — 37 חברים, זהה ל־`PrefixAndDefaultPathStrategy` למעלה.
...
<code>PrefixExceptDefaultPathStrategy</code> — 44 חברים, זהה ל־`PrefixExceptDefaultPathStrategy` למעלה.
<code>Strategy</code> — 44 חברים, זהה ל־`PrefixExceptDefaultPathStrategy` למעלה.
...
<code>PathStrategy</code> — 22 חברים, זהה ל־`PathStrategy` למעלה.
<code>PathStrategyContext</code> — 15 חברים, זהה ל־`PathStrategyContext` למעלה.
<code>ResolvedRouteLike</code> — 6 חברים, זהה ל־`ResolvedRouteLike` למעלה.
<code>RouteLike</code> — 6 חברים, זהה ל־`RouteLike` למעלה.
<code>RouterAdapter</code> — 3 חברים, זהה ל־`RouterAdapter` למעלה.
<code>SwitchLocaleOptions</code> — 1 חברים, זהה ל־`SwitchLocaleOptions` למעלה.
