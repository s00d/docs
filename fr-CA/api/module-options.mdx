...
description: "Chaque option acceptée par le module, avec son type, sa valeur par défaut et son objectif — lue à la compilation à partir de la définition de type."
---

# Référence des options du module

Chaque option acceptée par `nuxt-i18n-micro`, lue à la compilation à partir de l'interface `ModuleOptions` dans
[`packages/types/src/index.ts`](https://github.com/s00d/nuxt-i18n-micro/blob/main/packages/types/src/index.ts).
Rien ici n'est écrit à la main, alors cela ne peut pas prendre du retard sur le code — les types, les valeurs par défaut et
les descriptions proviennent de la déclaration elle-même.

Pour comprendre ce que ces options *signifient* ensemble — quelles combinaisons ont du sens, ce qui change lorsque
vous changez de stratégie, des exemples pratiques — consultez [Configuration](/guides/configuration). Cette
page est la liste exhaustive ; l'autre est l'explication.
...
Les options imbriquées apparaissent sous leur chemin pointé, de sorte que `translationPayloads.mode` est la
clé `mode` à l'intérieur de l'objet `translationPayloads`.
...
## Charges utiles et cache

Comment les traductions arrivent au navigateur. [Performance](/guides/performance) explique ce qu'elles changent à l'exécution.
...
Choisir une langue pour un visiteur qui n'en a pas sélectionné une.
