...
description: "Index de référence pour chaque paquet de l'espace de travail @i18n-micro, listant les API et types exportés, générés à partir de captures de source vérifiées par l'IC."
---

# API des paquets

`nuxt-i18n-micro` est construit à partir d'un ensemble de petits paquets, dont la plupart peuvent s'utiliser seuls : `@i18n-micro/vue` dans une application Vue simple, `@i18n-micro/core` n'importe où.

Chaque page ci-dessous est générée à partir de la capture d'API que [`pnpm run api:surface`](https://github.com/s00d/nuxt-i18n-micro/blob/main/docs/guide/maintenance-commands.md#api-surface) vérifie par rapport aux sources TypeScript. Un export ne peut donc pas disparaître sans que cette référence change ou que l'IC échoue.
...
- [Intégrations](/integrations/overview) : guides pour utiliser ces paquets dans chaque cadriciel
- [Options du module](/api/module-options) : options acceptées par le module Nuxt
- [Méthodes](/api/methods) : assistants d'exécution disponibles dans les composants
