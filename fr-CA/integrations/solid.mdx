...
description: "Use @i18n-micro/solid to add signals, components, and a provider for translations and locale-aware routing in SolidJS applications."
---

# Solid Package (`@i18n-micro/solid`)

Le `@i18n-micro/solid` paquet fournit a lightweight, high-performance internationalization solution for SolidJS applications. It shares the same core logic as Nuxt I18n Micro, offering reactive translations, route-specific support, and full TypeScript support.
...
Le `useI18n` hook requires locales configuration to be provided via `I18nProvider`. You must provide both `locales` and `defaultLocale`:
...
2. **Exported from package**: Le `createSolidRouterAdapter` is now exported directly from `@i18n-micro/solid`, so you don't need to create a separate router adapter file for basic use cases. However, you can still create custom adapters if needed (see "Creating Custom Router Adapters" section below).
...
### Exemple 1: Custom Router with Signals
...
Le `useI18n` hook provides access to i18n functionality in SolidJS components.
...
### Intégration au routeur Example
