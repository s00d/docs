...
description: "Use @i18n-micro/preact to add translations, locale switching, and locale-aware routing to Preact apps with a Nuxt-like developer experience."
---

# Preact Package (`@i18n-micro/preact`)

Le `@i18n-micro/preact` paquet fournit a lightweight, high-performance internationalization solution for **Pure Preact** applications. It shares the same core logic as Nuxt I18n Micro, offering reactive translations, route-specific support, and full TypeScript support.
...
### Configuration de base (Without Router)
...
### Configuration With Router Adapter (Wouter Example)
...
**Note :** Le `createPreactRouterAdapter` (aliased as `createBrowserHistoryAdapter`) is exported from the package and provides a basic History API-based adapter. For router-specific adapters (like wouter), you can create custom adapters as shown above.

### Utilisation in Components
...
Le `I18nRoutingStrategy` interface defines the contract between i18n and your router:
...
### Configuration With Built-in Router Adapter
...
**Paramètres :** the [shared `createI18n` options](/integrations/overview#create-i18n-options) — `locale`, `fallbackLocale`, `messages`, `plural`, `missingWarn` and `missingHandler`.
...
Le `useI18n` hook provides access to i18n functionality in Preact components. It uses `useState` + `useEffect` internally for reactivity (Pure Preact approach), ensuring components re-render when translations or locale change. The hook automatically uses the routing strategy from `I18nProvider` if available.
...
### Intégration DevTools (Vite Plugin)
...
Le `I18nRoutingStrategy` interface defines how i18n interacts with your router:
...
### Exemple 2: Preact Router Adapter
...
### Exemple 3: Query Parameter-Based Locale Routing
