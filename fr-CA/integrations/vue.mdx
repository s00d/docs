...
description: "Utilisez @i18n-micro/vue pour ajouter des composables, des composants et un plugin pour les traductions et le routage linguistique dans les applications Vue 3 autonomes."
---

# Vue Package (`@i18n-micro/vue`)

Le `@i18n-micro/vue` paquet fournit a lightweight, high-performance internationalization solution for Vue 3 applications. It shares the same core logic as Nuxt I18n Micro, offering reactive translations, route-specific support, and full TypeScript support.
...
Le `useI18n` composable requires locales configuration to be provided via Vue's dependency injection system.
...
- `global`: Le `VueI18n` instance for direct access
...
Le `useI18n` composable provides access to i18n functionality in Vue components.
