...
description: "Use @i18n-micro/astro to add locale routing, translations, and SEO helpers to Astro projects with the shared @i18n-micro/core runtime."
---

# Astro Package (`@i18n-micro/astro`)

Le `@i18n-micro/astro` paquet fournit a lightweight, high-performance internationalization solution for Astro applications. It shares the same core logic as Nuxt I18n Micro, offering SSR support, automatic locale detection, Astro components, and full TypeScript support.
...
### Configuration de base (Without Router Adapter)
...
### Configuration With Router Adapter
...
**Note :** Le `createAstroRouterAdapter` is now exported from the package, so you don't need to create your own router adapter file. However, you can still create a custom adapter if needed.
...
- Le `virtual:i18n-micro/config` module works in all environments (it's generated at build time)

### Utilisation in Astro Components
...
### Adaptateur de routeur Abstraction
...
Le `I18nRoutingStrategy` interface defines the contract between i18n and your routing:
...
**Note :** Le `createAstroRouterAdapter` is now exported from `@i18n-micro/astro`, so you don't need to create your own adapter file for basic use cases.
...
## Intégration au routeur

### I18nRoutingStrategy Interface

Le `I18nRoutingStrategy` interface defines how i18n interacts with your routing:
...
Le `I18nRoutingStrategy` interface defines the following methods:
...
### Exemple 1: Subdomain-Based Locale Routing
...
### Exemple 2: Query Parameter-Based Locale Routing
...
### Exemple 3: Suffix-Based Locale Routing
...
### Exemple 4: Integration with Third-Party Router
...
⚠️ **Important**: Le `loadTranslationsIntoI18n` function uses Node.js filesystem APIs and will **NOT work** in Edge runtime environments (Cloudflare Workers, Vercel Edge, Deno Deploy). For these environments, use `import.meta.glob` instead.
...
Le `loadTranslationsIntoI18n` function uses Node.js filesystem APIs (`node:fs`) and **will NOT work** in Edge runtime environments such as:
...
Astro Islands allow you to use interactive components (Vue, React, Svelte) within Astro pages. Le `@i18n-micro/astro` paquet fournit framework-specific adapters for seamless i18n integration in islands.
...
**Retourne :** `I18nClientProps` object with:
...
### Adaptateur de routeur personnalisé
...
Le `clone()` method allows creating per-request instances that share the same cache, improving performance in SSR scenarios.
