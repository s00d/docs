---
title: "Integrasi Preact"
description: "Gunakan @i18n-micro/preact untuk terjemahan Preact, pengalih lokal, dan komponen perutean di aplikasi Preact SSR/CSR."
---

# Preact Package (`@i18n-micro/preact`)

The `@i18n-micro/preact` package menyediakan a lightweight, high-performance internationalization solution untuk **Pure Preact** applications. It shares the same core logic as Nuxt I18n Micro, offering reactive translations, route-specific support, dan full TypeScript support.

## Ikhtisar

`@i18n-micro/preact` is designed untuk Pure Preact applications (without React compatibility layer) that need internationalization. It provides:
...
- **Pure Preact** - Uses Preact's native JSX runtime (`h`) untuk minimal bundle size (~3KB)
- **Router-agnostic** - Works dengan any router (wouter, preact-router, atau custom) atau tanpa a router
...
- **DevTools Integration** - Vite plugin untuk managing translations during development

**Catatan:** The built-in router adapter (`createPreactRouterAdapter` / `createBrowserHistoryAdapter`) uses the native History API dan works dengan any router that listens to `popstate` events (e.g., `wouter-preact`, `preact-router`). For custom routing needs, you can create your own adapter.
...
### Pengaturan With Router Adapter (Wouter Example)

For applications dengan routing using `wouter-preact`:
...
**Catatan:** The `createPreactRouterAdapter` (aliased as `createBrowserHistoryAdapter`) is exported dari the package dan menyediakan a basic History API-based adapter. For router-specific adapters (like wouter), you can create custom adapters as shown above.
...
- Use any router library (wouter, preact-router, custom router, atau no router)
...
### Pengaturan With Built-in Router Adapter
...
Creates a new i18n instance untuk your Preact application.

**Parameters:** the [shared `createI18n` options](/integrations/overview#create-i18n-options) — `locale`, `fallbackLocale`, `messages`, `plural`, `missingWarn` dan `missingHandler`.
...
The `useI18n` hook menyediakan access to i18n functionality in Preact components. It uses `useState` + `useEffect` internally untuk reactivity (Pure Preact approach), ensuring components re-render when translations atau locale change. The hook automatically uses the routing strategy dari `I18nProvider` if available.
...
Saat sebuah routing strategy is provided via `I18nProvider`, the hook exposes router-related methods:
...
Switch locale dan navigate ke localized path. Uses the routing strategy if available.
...
Resolve path untuk a specific locale. Returns localized path using the routing strategy.
...
Same as `localeRoute` tetapi always mengembalikan a string.
...
Context provider component that makes the i18n instance available to all child components. It also menyediakan locales configuration dan routing strategy.

#### Props

| Prop              | Type                       | Required | Description                                 |
| ----------------- | -------------------------- | -------- | ------------------------------------------- |
| `i18n`            | `PreactI18n`               | ✅       | The i18n instance created dengan `createI18n` |
...
Translation component dengan support untuk pluralization, formatting, dan HTML rendering. Automatically uses the routing strategy dari `I18nProvider` if available.
...
Localized link component that automatically menangani locale prefixes using the routing strategy dari `I18nProvider` if available.
...
| `activeStyle`  | `JSX.CSSProperties`                                                                 | ❌       | -       | Styles to apply when link adalahctive                                  |
...
- Uses the routing strategy's `linkComponent` if available (e.g., `Link` dari `wouter-preact`)
- Falls back to a native `<a>` tag dengan `onClick` handler if no `linkComponent` is provided
- Handles active state detection using `getCurrentPath()` dari the routing strategy
- Supports external links (detected automatically)

### `<I18nSwitcher>`

Language switcher component that menghasilkan links untuk all available locales. Uses the routing strategy dari `I18nProvider` untuk path generation if available.
...
- Provides a dropdown interface untuk locale selection
...
The package exports `createPreactRouterAdapter` (also available as `createBrowserHistoryAdapter`) untuk basic History API-based routing:
...
Thadalahdapter works dengan any router that listens to `popstate` events (e.g., `wouter-preact`, `preact-router`).
...
| `linkComponent`           | ❌       | Component to use untuk rendering links (e.g., `Link` dari wouter-preact) |
| `push(target)`            | ❌       | Function to navigate to another route/locale                           |
| `replace(target)`         | ❌       | Function to replace current route                                      |
| `resolvePath(to, locale)` | ❌       | Generate path untuk specific locale                                      |
...
### Contoh 1: Wouter Adapter

Contoh ini shows how to create an adapter untuk `wouter-preact`:
...
### Contoh 2: Preact Router Adapter

Contoh ini shows how to create an adapter untuk `preact-router`:
...
### Contoh 3: Query Parameter-Based Locale Routing
...
4. **Support both SSR dan client-side**: Use `window` checks untuk client-side code:
...
5. **Test dengan all locales**: Ensure your adapter works correctly dengan all configured locales, including edge cases like:
...
- **Solusi**: Check that your `getCurrentPath` correctly mengembalikan the current path, dan `resolvePath` properly menangani locale prefixes

**Masalah**: Links don't switch locale

- **Solusi**: Ensure `push` dan `replace` methods correctly update the URL dan dispatch events untuk your router

**Masalah**: Active state detection doesn't work

- **Solusi**: Verify `getCurrentPath` mengembalikan the correct path format that matches your routing strategy

## Async Translation Loading

### Loading dari JSON Files

Load translations dynamically dari JSON files:
...
Load translations dari an API endpoint:
...
`addTranslations` loads base translations (stored as `index`). Saat Anda call `addRouteTranslations`, these base translations are automatically merged in. Page-specific keys override base ones.
...
### Full Application Setup dengan Wouter
...
4. **Use Route-Specific Translations**: Organize translations by route untuk better maintainability
5. **Handle Missing Translations**: Set up a missing handler untuk production error tracking
6. **Preload Common Locales**: Preload frequently used locales di background
...
10. **Use Components**: Prefer `I18nLink` dan `I18nSwitcher` components over manual link generation
11. **Router Adapter Reusability**: Create the router adapter once in `RouterRoot` dan reuse it throughout the app
12. **URL-based Locale Detection**: Use URL-based locale detection untuk better SEO dan user experience
