---
title: "Integrasi SolidJS"
description: "Gunakan @i18n-micro/solid untuk terjemahan SolidJS, sinyal, dan komponen perutean di aplikasi SolidStart dan Solid CSR."
---

# Solid Package (`@i18n-micro/solid`)

The `@i18n-micro/solid` package menyediakan a lightweight, high-performance internationalization solution untuk SolidJS applications. It shares the same core logic as Nuxt I18n Micro, offering reactive translations, route-specific support, dan full TypeScript support.

## Ikhtisar

`@i18n-micro/solid` is designed untuk SolidJS applications that need internationalization tanpa the Nuxt framework. It provides:
...
- **Router-agnostic** - Works dengan any router atau tanpa a router via `I18nRoutingStrategy` interface
- **DevTools Integration** - Built-in DevTools support untuk managing translations
...
The package memerlukan SolidJS:
...
- Use any router library (@solidjs/router, custom router, atau no router)
...
For applications dengan routing (using @solidjs/router):
...
The `useI18n` hook memerlukan locales configuration to be provided via `I18nProvider`. Anda harus provide both `locales` dan `defaultLocale`:
...
The package exports `createSolidRouterAdapter` untuk @solidjs/router integration:
...
1. **Must be dipanggil inside a component**: `createSolidRouterAdapter` uses `createSignal` dan `createEffect`, which require a reactive context. **Always call it inside a component function**, not at module level.

2. **Exported dari package**: The `createSolidRouterAdapter` is now exported directly dari `@i18n-micro/solid`, so you don't need to create a separate router adapter file untuk basic use cases. However, you can still create custom adapters if needed (see "Creating Custom Router Adapters" section below).

3. **Reactivity**: The built-in adapter automatically menyediakan `getCurrentPathAccessor()` untuk proper reactivity in SolidJS components.
...
- Support any routing strategy (@solidjs/router, custom router, atau no router)
...
| `getCurrentPathAccessor()` | ⚠️ **Highly Recommended** | Returns reactive accessor untuk current path (SolidJS-specific). **Required untuk proper reactivity** - tanpa this, active link states dan path-dependent UI won't update reactively. |
| `linkComponent`            | ❌                        | Component to use untuk rendering links (e.g., `A` dari @solidjs/router)                                                                                                               |
...
**For SolidJS, you MUST also implement `getCurrentPathAccessor()`** - this is critical untuk proper reactivity. Without it, active link states dan any UI that depends on the current path won't update when navigation occurs. The accessor should return a signal that tracks pathname changes.
...
### Contoh 1: Custom Router dengan Signals

Contoh ini shows how to create an adapter untuk a custom router that uses signals:
...
1. **Always provide `getCurrentPathAccessor` untuk SolidJS** (REQUIRED untuk reactivity): Ini memastikan proper reactivity in components. Without it, active link states dan path-dependent UI won't update:
...
2. **Use signals untuk reactive path tracking**: Create signals inside the adapter to track pathname changes:
...
3. **Preserve query parameters dan hash**: When switching locales, maintain query params dan hash fragments:
...
4. **Handle edge cases**: Consider empty paths, root paths, dan invalid locales:
...
- **Solusi**: **You MUST provide `getCurrentPathAccessor`** in your router adapter. Verify it's implemented dan mengembalikan a signal accessor. The accessor harus dipanggil inside `createMemo` untuk proper reactivity. Without `getCurrentPathAccessor`, active states won't update reactively.

**Masalah**: Components don't update when path changes

- **Solusi**: **You MUST implement `getCurrentPathAccessor`** in your router adapter. It must return a signal accessor that tracks pathname changes. Components should call it inside reactive contexts (`createMemo`, `createEffect`, JSX). If you're using `createSolidRouterAdapter`, ensure it's dipanggil inside a component (not at module level) so `createSignal` dan `createEffect` work correctly.
...
- `localeAccessor: Accessor<string>` - Reactive accessor untuk current locale
- `fallbackLocale: string` - Fallback locale (getter)
- `currentRoute: string` - Current route name (getter)
- `routeAccessor: Accessor<string>` - Reactive accessor untuk current route
...
Destroys the reactive root (useful untuk cleanup in tests).

## Composable

### `useI18n(options?: UseI18nOptions)`

The `useI18n` hook menyediakan access to i18n functionality in SolidJS components.
...
| `customActiveLinkStyle` | `JSX.CSSProperties`                                                                 | ❌       | `\{\}`    | Custom styles untuk the current locale link           |
...
Lihat playground implementation in `packages/solid/playground/src/` untuk a complete working example.
...
### Locale Synchronization dengan Router
