...
description: "Gunakan @i18n-micro/react untuk terjemahan React, pengalih lokal, dan komponen perutean di aplikasi Vite/Next.js/CRA."
---

# React Package (`@i18n-micro/react`)

The `@i18n-micro/react` package menyediakan a lightweight, high-performance internationalization solution untuk React applications. It shares the same core logic as Nuxt I18n Micro, offering reactive translations, route-specific support, dan full TypeScript support.

## Ikhtisar

`@i18n-micro/react` is designed untuk React applications that need internationalization tanpa the Nuxt framework. It provides:
...
The package memerlukan React dan optionally React Router:
...
- Use any router library (React Router, custom router, atau no router)
...
Creates a new i18n instance untuk your React application.
...
The `useI18n` hook menyediakan access to i18n functionality in React components. It uses `useSyncExternalStore` internally untuk reactivity, ensuring components re-render when translations atau locale change. The hook automatically uses the routing strategy dari `I18nProvider` if available.
...
| ----------------- | --------------------- | -------- | ------------------------------------------- |
| `i18n`            | `ReactI18n`           | ✅       | The i18n instance created dengan `createI18n` |
...
| `activeStyle`  | `React.CSSProperties`                                                               | ❌       | -       | Styles to apply when link adalahctive                                  |
...
- Uses the routing strategy's `linkComponent` if available (e.g., `Link` dari `react-router-dom`)
...
The package exports `createReactRouterAdapter` untuk React Router integration:
...
| `linkComponent`           | ❌       | Component to use untuk rendering links (e.g., `Link` dari react-router-dom) |
...
### Contoh 1: Next.js Router Adapter

Contoh ini shows how to create an adapter untuk Next.js App Router:
...
### Contoh 2: TanStack Router Adapter

Contoh ini shows how to create an adapter untuk TanStack Router:
...
- **Solusi**: Check that `getCurrentPath` correctly mengembalikan the current path, dan `resolvePath` properly menangani locale prefixes

**Masalah**: Links don't switch locale

- **Solusi**: Ensure `push` dan `replace` methods correctly update the URL using your router's navigation API
...
Lihat "Full Application Setup" example above untuk a complete router integration example using the adapter pattern.
