...
description: "Gunakan @i18n-micro/node di layanan Node.js: memuat terjemahan, memformat pesan, dan berbagi cache dengan runtime Nuxt."
---

# 🌐 Node.js Runtime

Use `@i18n-micro/node` to add i18n translations to any Node.js application, CLI tool, atau backend service. This package menyediakan the same translation logic as the Nuxt module, tetapi untuk pure Node.js environments.
...
The package mendukung the same file structure as the Nuxt module:
...
- `missingWarn?: boolean` - Show warnings untuk missing translations
...
Load translations dari directory (recursive, mendukung pages structure).
...
Get translation untuk a key. Uses `currentRoute` secara default if `routeName` is not provided.
...
Clear cache dan reload translations dari disk. Use this when translation files change.
...
### Hot Module Replacement (HMR) dengan File Watcher
...
The `currentRoute` feature memungkinkan Anda set a route context that akan used secara default in all `t()` calls:
...
- 🔄 **Hot Reload** - Built-in support untuk file watching dan cache reloading

## 🌍 Using in Nuxt Server Routes

Anda dapat use `@i18n-micro/node` in Nuxt server API routes to provide server-side translations:
...
Anda dapat implement custom locale detection logic based on your needs:
...
### Integration dengan Route-Specific Translations
...
3. **Cache I18n instance**: Create a singleton instance dan reuse it across requests untuk better performance
...
6. **Reload on file changes**: Use `reload()` method dengan file watchers in development untuk hot-reloading

## 🚀 Performance Considerations

- **Singleton pattern**: Create one I18n instance dan reuse it across requests
- **Lazy loading**: Load translations once at startup, not on every request
- **Efficient caching**: Translations are cached in memory untuk fast access
- **Route context**: Use `setRoute()` to avoid passing route name to every `t()` call
- **No external calls**: All translations are loaded dari local files, no network requests

## 📚 Related Documentation

- **[Folder Structure](/guides/folder-structure)** - Learn about translation file organization
- **[Server Side Translations](/guides/server-side-translations)** - Nuxt server-side usage
