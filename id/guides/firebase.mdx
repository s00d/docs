---
title: "Penerapan Firebase"
description: "Terapkan Nuxt I18n Micro ke Firebase Hosting dengan Cloud Functions, App Hosting, atau ekspor statis dan tangani payload terjemahan dengan benar."
---
...
Firebase AppHosting adalah powerful platform untuk deploying Nuxt applications dengan serverless infrastructure. However, when deploying applications dengan `nuxt-i18n-micro`, you may encounter specific issues related to how translation JSON files are served. Panduan ini menyediakan comprehensive solutions to ensure your translations work correctly in production.
...
- ❌ After deployment to Firebase AppHosting, JSON files load tetapi translations don't apply
- ❌ No errors appear di console
- ❌ Translation files are visible di Network tab tetapi have incorrect `Content-Type: text/html` instead of `application/json`

**Root Cause:**

Firebase AppHosting (and other platforms using Nitro prerendering) may serve prerendered JSON files dengan incorrect MIME types. Saat browser receives JSON files dengan `Content-Type: text/html`, it cannot properly parse them, causing translations to fail silently.

## ✅ Solutions

### Solution 1: Route Rules Configuration (Recommended)

The most straightforward solution is to add explicit headers untuk translation routes in your `nuxt.config.ts`:
...
- 🚀 Improves performance dengan proper caching
...
Create atau update your `firebase.json` file di project root:
...
Ini akan output detailed logs about:
...
Anda harus see `data.json` untuk each locale under each page.
...
Konfigurasikan prerendering untuk optimal performance:
...
Berikut complete working configuration untuk Firebase AppHosting:
...
- [ ] Network tab shows `Content-Type: application/json` untuk translation files
...
### Issue: 404 Errors untuk Translation Files

**Solution:** Ensure translation files are being copied ke output directory:
...
If page-specific translations are missing, ensure `disablePageLocales` tidak diatur to `true`:
...
**Solution:** Optimize caching dan preloading:
...
4. **Keep translation files small** untuk faster loading
...
Firebase automatically menangani Gzip/Brotli compression, tetapi ensure your build is optimized:
...
- Use route-specific translation files untuk page content

<Tip>
The module automatically sets proper `Content-Type` headers starting dari version **v3.0.0+**. If you're experiencing issues, update ke latest version:
