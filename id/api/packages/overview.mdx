---
title: "API Paket"
description: "API yang diekspos setiap paket workspace, dibaca saat build dari snapshot yang diperiksa CI terhadap sumbernya."
---

# API Paket

`nuxt-i18n-micro` dibangun dari sekumpulan paket kecil, sebagian besar dapat digunakan
sendiri — `@i18n-micro/vue` di aplikasi Vue biasa, `@i18n-micro/core` di mana saja.

Setiap halaman di bawah dihasilkan dari snapshot API yang diperiksa
[`pnpm run api:surface`](https://github.com/s00d/nuxt-i18n-micro/blob/main/docs/guide/maintenance-commands.md#api-surface) terhadap
sumber TypeScript, jadi sebuah ekspor tidak dapat hilang tanpa perubahan referensi ini
atau CI gagal.
...
- [Integrasi](/integrations/overview) — panduan penggunaan paket ini pada setiap framework
- [Opsi Modul](/api/module-options) — opsi yang diterima modul Nuxt
- [Method](/api/methods) — helper runtime yang tersedia di dalam komponen
