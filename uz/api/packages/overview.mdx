...
description: "Har bir workspace paketining eksport qilingan API, build vaqtida CI manbaga qarshi tekshiradigan snapshot'lardan o'qiladi."
---

# Paket API'lari

`nuxt-i18n-micro` bir qator kichik paketlardan qurilgan bo'lib, ularning ko'pchiligi mustaqil ravishda ishlatiladi — oddiy Vue ilovada `@i18n-micro/vue`, har joyda `@i18n-micro/core`.

Quyidagi har bir sahifa [`pnpm run api:surface`](https://github.com/s00d/nuxt-i18n-micro/blob/main/docs/guide/maintenance-commands.md#api-surface) TypeScript manbalariga qarshi tekshiradigan API snapshot'idan generatsiya qilinadi, shuning uchun eksport ushbu ma'lumotnoma o'zgarmasdan yoki CI muvaffaqiyatsiz bo'lmasdan yo'qolishi mumkin emas.
...
- [Integratsiyalar](/integrations/overview) — har bir freymvorkda ushbu paketlardan foydalanish uchun qo'llanmalar
- [Modul opsiyalari](/api/module-options) — Nuxt moduli qabul qiladigan opsiyalar
