---
title: "Sozlash havolasi"
description: "Har bir Nuxt I18n Micro modul parametri standart qiymatlari, turlari va misollari bilan: locale’lar, strategiya, meta, tarjima katalogi va boshqalar."
---

# Sozlash havolasi

Toʻliq havola every option under the `i18n` key in `nuxt.config`. Types and defaults match [`ModuleOptions`](https://github.com/s00d/nuxt-i18n-micro/blob/main/packages/types/src/index.ts) in `@i18n-micro/types`.
...
{/* generated:options-index — tahrirlamang; run `pnpm run docs:generate` */}

| Parametr | Tur | Standart | Vazifasi |
| --- | --- | --- | --- |
...
| [`strategy`](/api/module-options) | `Strategies` | `'prefix_except_default'` | URL routing strategy for locale prefixes. - `'no_prefix'` — no locale in URL; locale stored in cookie. - `'prefix_except_default'` — prefix all locales except the default. - `'prefix'` — always prefix, including the default locale. - `'prefix_and_default'` — like `prefix`, but the default locale bulso accessible without prefix. |
| [`metaBaseUrl`](/api/module-options) | `string` | `undefined` | Base URL for SEO meta tags (canonical, og:url, hreflang). - A concrete URL string (masalan `'https://example.com'`) — used as-is (highest priority). - `undefined` — falls back to `site.url` from `nuxt-site-config` when that module is   present, otherwise the current request origin   (`useRequestURL().origin` on server, `window.location.origin` on client). |
...
| [`hreflangBaseLanguage`](/api/module-options) | `boolean` | `false` | Also emit a bare-language `hreflang` derived from each locale's `iso` (masalan `es-ES` → also `es`). |
...
| [`autoDetectPath`](/api/module-options) | `string` | `'/'` | Where cookie / Accept-Language preference redirects may run (when `redirects` is enabled). - `'/'` — only `/` (deep links in the default locale stay reachable; default) - `'no_prefix'` — only paths without a locale prefix - `'*'` — every path, including rewriting an explicit locale prefix   (masalan `/fr/about` → `/de/about` when the cookie prefers `de`) - any other string — exact path match (masalan `'/welcome'`) Prefixed strategy cleanup (masalan `/en` → `/` under `prefix_except_default`) is not gated. |
...
| [`noPrefixRedirect`](/api/module-options) | `boolean` | `false` | For `no_prefix` strategy: enable redirect from a locale-prefixed URL (masalan `/en/about`) to the unprefixed version (`/about`). |
...
| [`localizedRouteNamePrefix`](/api/module-options) | `string` | `'localized-'` | Prefix prepended to localized route names (masalan `'localized-index'`). |
...
{/* generated:option:locales — tahrirlamang; run `pnpm run docs:generate` */}

**Tur** `Locale[]` · **Standart** `[]`

List of supported locales.
Each entry defines a locale code plus optional metadata (ISO, direction, display name, va h.k.).
...
| ---------------- | --------- | -------- | ----------------------------------------------------- |
| `code`           | `string`  | ✅       | Unique identifier (masalan, `'en'`)                      |
| `displayName`    | `string`  | ❌       | Human-readable name for switchers (masalan, `'English'`) |
| `iso`            | `string`  | ❌       | ISO code (masalan, `'en-US'`)                            |
...
By default, custom properties are typed as `unknown`. To get full TypeScript support, use **module augmentation**. Create a declaration file (masalan, `app/i18n.d.ts` or any `.d.ts` included in your `tsconfig`):
...
{/* generated:option:defaultLocale — tahrirlamang; run `pnpm run docs:generate` */}

**Tur** `string` · **Standart** `'en'`
...
{/* generated:option:strategy — tahrirlamang; run `pnpm run docs:generate` */}
...
- `'prefix_and_default'` — like `prefix`, but the default locale bulso accessible without prefix.
...
{/* generated:option:translationDir — tahrirlamang; run `pnpm run docs:generate` */}

**Tur** `string` · **Standart** `'locales'`
...
{/* generated:option:disablePageLocales — tahrirlamang; run `pnpm run docs:generate` */}

**Tur** `boolean` · **Standart** `false`
...
{/* generated:option:fallbackLocale — tahrirlamang; run `pnpm run docs:generate` */}

**Tur** `string` · **Standart** `undefined (no fallback; returns the raw key)`
...
{/* generated:option:types — tahrirlamang; run `pnpm run docs:generate` */}
...
{/* generated:option:routeLocales — tahrirlamang; run `pnpm run docs:generate` */}

**Tur** `Record<string, string[]>` · **Standart** —

Per-route locale restrictions, extracted from `defineI18nRoute()` calls.
Maps a route path (masalan `'/about'`) to an array of allowed locale codes.
Routes not listed have no restrictions (all locales allowed).

{/* /generated:option:routeLocales */}

Maps a route path (masalan `'/about'`) to allowed locale codes. Routes not listed allow all configured locales.
...
{/* generated:option:routeDisableMeta — tahrirlamang; run `pnpm run docs:generate` */}

**Tur** `Record<string, boolean \| string[]>` · **Standart** —
...
{/* generated:option:experimental — tahrirlamang; run `pnpm run docs:generate` */}

**Tur** `Record<string, unknown>` · **Standart** —
...
Most former `experimental.*` flags were promoted to top-level options in v3 (masalan `hmr`). Prefer documented top-level options. Use `experimental` only when following a release note or migration guide that references a specific key.

### 🔍 SEO & Meta Tags
#### `meta`

{/* generated:option:meta — tahrirlamang; run `pnpm run docs:generate` */}
...
```typescript
meta: true // Generate alternate links, canonical URLs, va h.k.
```
#### `metaBaseUrl`

{/* generated:option:metaBaseUrl — tahrirlamang; run `pnpm run docs:generate` */}

**Tur** `string` · **Standart** `undefined`

Base URL for SEO meta tags (canonical, og:url, hreflang).

- A concrete URL string (masalan `'https://example.com'`) — used as-is (highest priority).
...
{/* generated:option:canonicalQueryWhitelist — tahrirlamang; run `pnpm run docs:generate` */}

**Tur** `string[]` · **Standart** `['page', 'sort', 'filter', 'search', 'q', 'query', 'tag']`
...
{/* generated:option:globalLocaleRoutes — tahrirlamang; run `pnpm run docs:generate` */}

**Tur** `GlobalLocaleRoutes` · **Standart** `\{\}`
...
{/* generated:option:routesLocaleLinks — tahrirlamang; run `pnpm run docs:generate` */}

**Tur** `\{ [key: string]: string \}` · **Standart** `\{\}`

Map route names to other route names to share the same translation files.
Masalan, `\{ 'about-us': 'about' \}` means the `about-us` page will use
...
{/* generated:option:customRegexMatcher — tahrirlamang; run `pnpm run docs:generate` */}

**Tur** `string \| RegExp` · **Standart** `undefined (uses built-in pattern based on locale codes)`
...
{/* generated:option:debug — tahrirlamang; run `pnpm run docs:generate` */}
...
{/* generated:option:disableWatcher — tahrirlamang; run `pnpm run docs:generate` */}
...
{/* generated:option:missingWarn — tahrirlamang; run `pnpm run docs:generate` */}
...
<Tip>
You can set a custom handler for missing translations using `setMissingHandler` method. Thbullows you to send missing translation errors to error tracking services like Sentry.
</Tip>

### 🔧 Plugin Control
#### `define`

{/* generated:option:define — tahrirlamang; run `pnpm run docs:generate` */}
...
{/* generated:option:redirects — tahrirlamang; run `pnpm run docs:generate` */}
...
- **Mijoz**: the `i18n-redirect` global route middleware is not registered — no SPA auto-redirects
...
{/* generated:option:plugin — tahrirlamang; run `pnpm run docs:generate` */}
...
{/* generated:option:hooks — tahrirlamang; run `pnpm run docs:generate` */}
...
{/* generated:option:components — tahrirlamang; run `pnpm run docs:generate` */}

**Tur** `boolean` · **Standart** `true`

Register built-in i18n components (`<i18n-link>`, `<i18n-switcher>`, `<i18n-t>`, `<i18n-group>`).
Set to `false` to disable automatic component registration (masalan if you don't use them
...
{/* generated:option:autoDetectLanguage — tahrirlamang; run `pnpm run docs:generate` */}
...
{/* generated:option:autoDetectPath — tahrirlamang; run `pnpm run docs:generate` */}
...
- `'*'` — every path, including rewriting an explicit locale prefix
  (masalan `/fr/about` → `/de/about` when the cookie prefers `de`)

- any other string — exact path match (masalan `'/welcome'`)

Prefixed strategy cleanup (masalan `/en` → `/` under `prefix_except_default`) is not gated.
...
{/* generated:option:plural — tahrirlamang; run `pnpm run docs:generate` */}

**Tur** `PluralFunc` · **Standart** `built-in pluralization (form index by count)`
...
1. Calls `t(key)` to get the raw translation string (masalan `"no apples | one apple | \{count\} apples"`)
...
For languages with complex pluralization rules (masalan, Russian, Arabic, Polish), override the `plural` option.
...
{/* generated:option:localeCookie — tahrirlamang; run `pnpm run docs:generate` */}

**Tur** `string \| null` · **Standart** `null`
...
- **Talab qilinadi** for redirect behavior in prefix strategies (when `redirects: true`)
#### `apiBaseUrl`

{/* generated:option:apiBaseUrl — tahrirlamang; run `pnpm run docs:generate` */}

**Tur** `string` · **Standart** `'_locales'`
...
**Muhit oʻzgaruvchisi**: `NUXT_I18N_APP_BASE_URL`

```typescript
apiBaseUrl: 'api/_locales'
```

The translations will be fetched from `/\{apiBaseUrl\}/\{routeName\}/\{locale\}/data.json` (masalan, `/api/_locales/index/en/data.json`).
#### `apiBaseClientHost`

{/* generated:option:apiBaseClientHost — tahrirlamang; run `pnpm run docs:generate` */}
...
**Muhit oʻzgaruvchisi**: `NUXT_I18N_APP_BASE_CLIENT_HOST`

```typescript
apiBaseClientHost: 'https://cdn.example.com'
```

When `apiBaseClientHost` is set, client-side translations will be fetched from `\{apiBaseClientHost\}/\{apiBaseUrl\}/\{routeName\}/\{locale\}/data.json` (masalan, `https://cdn.example.com/_locales/index/en/data.json`).
#### `apiBaseServerHost`

{/* generated:option:apiBaseServerHost — tahrirlamang; run `pnpm run docs:generate` */}
...
**Muhit oʻzgaruvchisi**: `NUXT_I18N_APP_BASE_SERVER_HOST`

```typescript
apiBaseServerHost: 'https://internal-cdn.example.com'
```

When `apiBaseServerHost` is set, server-side translations will be fetched from `\{apiBaseServerHost\}/\{apiBaseUrl\}/\{routeName\}/\{locale\}/data.json` (masalan, `https://internal-cdn.example.com/_locales/index/en/data.json`).

#### `translationPayloads`

{/* generated:option:translationPayloads — tahrirlamang; run `pnpm run docs:generate` */}

**Tur** `TranslationPayloadOptions` · **Standart** —
...
<Warning>
With `mode: 'source'`, `publicAssets` defaults to `false` (`prerenderRoutes` bulways off unless you opt in). Pure static hosting without a Nitro/edge runtime therefore cannot load translations on the client unless you enable one of these outputs, keep `serverHandler` available at runtime, or host payloads externally.
...
Use `publicDir` to change the public output folder when payloads are copied. It defaults to `apiBaseUrl` (`_locales`). In premerged mode `publicAssets` writes `\{page\}/\{locale\}/data.json` there (same paths the client fetches). `prerenderRoutes` bun optional Nitro prerender of the handler routes — usually redundant when that tree was already copied.
...
{/* generated:option:metaTrustForwardedHost — tahrirlamang; run `pnpm run docs:generate` */}

**Tur** `boolean` · **Standart** `true`

Trust the `X-Forwarded-Host` header when resolving the base URL for meta tags.
Enable when the app runs behind a reverse proxy (nginx, Cloudflare, AWS ALB, va h.k.)
...
{/* generated:option:metaTrustForwardedProto — tahrirlamang; run `pnpm run docs:generate` */}
...
{/* generated:option:hreflangBaseLanguage — tahrirlamang; run `pnpm run docs:generate` */}

**Tur** `boolean` · **Standart** `false`

Also emit a bare-language `hreflang` derived from each locale's `iso`
(masalan `es-ES` → also `es`). The first regional locale in `locales` claims
...
{/* generated:option:noPrefixRedirect — tahrirlamang; run `pnpm run docs:generate` */}

**Tur** `boolean` · **Standart** `false`

For `no_prefix` strategy: enable redirect from a locale-prefixed URL
(masalan `/en/about`) to the unprefixed version (`/about`).
...
{/* generated:option:excludePatterns — tahrirlamang; run `pnpm run docs:generate` */}

**Tur** `(string \| RegExp)[]` · **Standart** `undefined`
...
Internal Nuxt paths (`/__nuxt_error`, va h.k.) are always excluded automatically.
...
{/* generated:option:localizedRouteNamePrefix — tahrirlamang; run `pnpm run docs:generate` */}

**Tur** `string` · **Standart** `'localized-'`

Prefix prepended to localized route names (masalan `'localized-index'`).
...
{/* generated:option:dateBuild — tahrirlamang; run `pnpm run docs:generate` */}

**Tur** `string \| number` · **Standart** —
...
(masalan a git SHA or build number).
...
Set it explicitly to take control (masalan to tie payload URLs to a release rather than to content):

```ts
export default defineNuxtConfig({
  i18n: {
    // Any stable string/number: git SHA, CI build number, release tag, va h.k.
...
{/* generated:option:httpCacheDuration — tahrirlamang; run `pnpm run docs:generate` */}

**Tur** `number` · **Standart** `31536000`
...
{/* generated:option:numberFormats — tahrirlamang; run `pnpm run docs:generate` */}

**Tur** `Record<string, Record<string, Intl.NumberFormatOptions>>` · **Standart** —
...
{/* generated:option:datetimeFormats — tahrirlamang; run `pnpm run docs:generate` */}

**Tur** `Record<string, Record<string, Intl.DateTimeFormatOptions>>` · **Standart** —
...
{/* generated:option:hmr — tahrirlamang; run `pnpm run docs:generate` */}
...
{/* generated:option:cacheMaxSize — tahrirlamang; run `pnpm run docs:generate` */}

**Tur** `number` · **Standart** `0`
...
{/* generated:option:cacheTtl — tahrirlamang; run `pnpm run docs:generate` */}
...
**Maksimal yozuvlarni baholash formulasi**: `number_of_locales × (number_of_pages + 1)`. Masalan, 10 locales × 500 pages = ~5010 entries.
</Tip>
