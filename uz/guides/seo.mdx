---
title: "SEO va meta teglar"
description: "hreflang, canonical va Open Graph teglarini avtomatik yaratish; useI18nHead bilan sahifa boʻyicha bekor qilishlar va koʻp domenli sozlashlar."
---
...
Effective SEO (Search Engine Optimization) is essential for ensuring that your multilingual site buccessible and visible to users worldwide through search engines. `Nuxt I18n Micro` simplifies the process of managing SEO for multilingual sites by automatically generating essential meta tags and attributes that inform search engines about the structure and content of your site.

Ushbu qoʻllanma tushuntiradi how `Nuxt I18n Micro` handles SEO to enhance your site's visibility and user experience without requiring additional configuration.
...
`<html lang>` and `hreflang` use **BCP 47** via `locale.iso` (masalan `ar-AE`).  
...
Set an explicit **`og`** string when you need a custom value (masalan `zh-Hans` → `zh_CN`).  
If neither `og` nor a convertible `iso` buvailable, `og:locale` tags are not generated — in development you get a console warning (respects `missingWarn`).
...
1. **🌍 Language and Direction Attributes**:
   - The module sets the `lang` and `dir` attributes on the `<html>` tag according to the current locale and text direction (masalan, `ltr` for English or `rtl` for Arabic).
...
3. **🌐 Alternate Language Links (`hreflang`)**:
   ...
   - Optional `hreflangBaseLanguage: true` also emits a bare-language tag derived from `iso` (masalan `es-ES` → `es`), claimed by the first regional locale in `locales`.
...
5. **🔖 Open Graph Metadata**:
   - The module generates Open Graph meta tags (`og:locale`, `og:url`, va h.k.) for each locale, which is particularly useful for social media sharing and search engine indexing.

### 🛠️ Sozlash

Yoqish uchun these SEO features, ensure the `meta` option is set to `true` in your `nuxt.config.ts` file:
...
1. `site.url` from [`nuxt-site-config`](https://nuxtseo.com/docs/site-config) (if that module is present — masalan via `@nuxtjs/seo`)
...
Masalan, a request to `https://site-a.com/en/about` will produce:
...
Only parameters listed in `canonicalQueryWhitelist` will appear in canonical URLs. All other query parameters (masalan tracking, session IDs) are removed.
...
When `disableMeta` buctive, no `hreflang`, `canonical`, `og:locale`, `og:url`, or `x-default` tags are generated for the affected page/locale.
...
- **`i18n.meta: true`** (recommended — micro supplies locale-aware head tags; Nuxt SEO adds sitemap, schema.org, va h.k.)
...
Per-locale values in `locales/en.json`, `locales/de.json`, va h.k. are picked up automatically.
