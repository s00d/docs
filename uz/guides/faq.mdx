...
description: "Nuxt I18n Micro haqida umumiy savollar: locale almashtirish, sahifa oʻtishlari, xotira sarfi va koʻchirish nozikliklari."
---

# ❓ FAQ: Umumiy muammolar & Solutions

Nuxt I18n Micro bilan bogʻliq koʻp beriladigan savollarga javoblar va umumiy muammolar yechimlarini toping.

## 🚀 Ishga kirishishda muammolar

### ❓ Agar a route doesn't load?

Foydalanganda `Nuxt I18n Micro`, certain routes might not load kutilganidek, especially if the router doesn't automatically assign a name to a route in subfolders.

**Yechim:**
Buni hal qilish uchun, manually define the route name for the page by adding the following to the corresponding Vue file:

```javascript
definePageMeta({ name: 'pageName' })
```

Bu taʼminlaydi the route is properly registered, enabling seamless navigation within the application.

### ❓ Nima uchun `$t` or other i18n composables not work in Nuxt plugins?

Nuxt I18n composables (`$t`, `$getLocale`, `$localePath`, va h.k.) may not work kutilganidek within Nuxt plugins or utility functions, resulting in runtime errors.

**Sabab va yechim:**
Nuxt composables require specific contexts (masalan, Nuxt hooks or Vue setup functions) to access the Nuxt instance. If used outside of these contexts (masalan, in utility functions or plugins), the following error might appear in the console:
...
In services or utility functions, pass the translation keys instead of using `$t` directly. Keyin, fetch the translation in the component.
...
If you only need the target URL (masalan, a language switcher link), use `$switchLocalePath()` instead:
...
### ❓ Nima uchun translations break during page transitions, especially with `defineAsyncComponent`?

Foydalanganda `nuxt-i18n-micro` with page transitions, translations may briefly stop working during the transition. This issue occurs because the route changes before the transition completes, causing translations for the new page to load while the old page is still visible.
...
If translations are hosted externally on a CDN or different domain, use `apiBaseClientHost` for client-side requests and `apiBaseServerHost` for server-side requests. The `apiBaseUrl` should only contain the path prefix (masalan, `_locales`).
...
Thbullows dynamic links within translations while preserving proper localization structure.

## 🏗️ Build & Deployment Issues

### ❓ Nima uchun translation payloads end up under `public/_locales/` (or in the server bundle on Edge)?
...
### ❓ Nima uchun I get a build error referring to `@unhead/vue` or an undefined object, especially on Cloudflare Pages?
...
- Certain modules (masalan, **nuxthub**, **nitro-cloudflare-dev**) can introduce conflicts unless configured with correct flags or dependencies.
...
Bu taʼminlaydi it's available during the Nitro prerender phase.

**Solution 2: Add Cloudflare compatibility flags**
Foydalanganda Cloudflare Pages or Cloudflare Workers, Node.js compatibility is often disabled by default. Enable it in your `wrangler.toml`:
...
See the [SEO guide — Nuxt SEO section](/guides/seo#nuxt-seo-nuxtjs-seo) for a full example.
...
Thbupproach is flexible, so releasing modifiers is currently unnecessary. However, modifiers may be added in future releases if there is demand.
