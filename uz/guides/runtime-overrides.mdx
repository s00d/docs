---
title: "Runtime bekor qilishlari"
description: "Ishlash paytida tarjimalar va locale konfiguratsiyasini bekor qilish: ,  va DevTools tahrirlari."
---

# Runtime i18n Overrides

Use this guide when you need one build artifact to run with different locale defaults per environment (masalan: different tenant instances).
...
`strategy` (`no_prefix`, `prefix`, `prefix_except_default`, `prefix_and_default`) bu build-time concern because it affects generated route behavior.
...
- `NUXT_I18N_DISABLED_LOCALES` (comma-separated, masalan `fr,it`)
