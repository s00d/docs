---
title: "Sinov"
description: "Vitest, Playwright va @i18n-micro/test-utils bilan Nuxt I18n Micro loyihalarini sinovdan oʻtkazish."
---
...
Testing the `Nuxt I18n Micro` module is crucial to ensure that your application's localization features work kutilganidek. This documentation will guide you through setting up the testing environment, creating mock configurations for vitest, and writing tests for your components. For a practical example, you can refer to the [example project on GitHub](https://github.com/s00d/nuxt-i18n-micro/tree/main/packages/test-utils/example).

See [Testing routing strategies](https://github.com/s00d/nuxt-i18n-micro/blob/main/docs/guide/testing-strategies.md) for how this repository runs its own suites — the `unit` / `integration` / `e2e` Vitest projects, the per-strategy files (`test/strategies-*.test.ts`), and generate regressions.

## 🛠️ Setting Up the Testing Environment

### 1. Install `@i18n-micro/test-utils`

Birinchidan, you need to install the testing utilities for `Nuxt I18n Micro`. Ushbu paket provides the necessary tools to mock and test your i18n configurations.
...
Keyingi, create a file to set up the mock i18n configuration. This file will be used to mock the i18n functions and utilities during your tests.
...
- **⚙️ Use Vitest for Unit Tests:** Vitest bu powerful testing framework for Vue applications. Use it to write unit tests for your components.
...
Below bu table describing all the utility methods provided by `@i18n-micro/test-utils`.
...
By following these steps, you can effectively test the `Nuxt I18n Micro` module and ensure that your application's localization features work kutilganidek.
