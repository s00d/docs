---
title: "Samaradorlik qoʻllanmasi"
description: "Nuxt I18n Micro’ning katta katalog va yuqori yuklamalarda tez ishlashi: keshlash, mode: 'source', HTTP kesh boshqaruvi va CDN maslahatlari."
---
...
`Nuxt I18n Micro` is designed with performance in mind, offering a significant improvement over traditional internationalization (i18n) modules like `nuxt-i18n`. This guide taqdim etadin in-depth look at the performance benefits of using `Nuxt I18n Micro`, and how it compares to other solutions.
...
- **`@i18n-micro/path-strategy`** — Runtime path resolution, redirects, and link generation. Uses pure functions and pre-computed context flags to minimize allocations on hot paths. Subpath exports (`/prefix`, `/no-prefix`, va h.k.) ensure only the chosen implementation is bundled.

Thbupproach keeps the routing configuration lightweight and ensures fast route resolution regardless of the number of locales.

### 📂 Streamlined Translation Loading

The module supports only JSON files for translations, with a clear separation between global and page-specific files. Bu taʼminlaydi that only the necessary translation data is loaded at any given time, further enhancing performance.
...
{/* generated:payload-budget — tahrirlamang; run `pnpm run docs:generate` */}
...
| --- | --- | --- | --- |
| Browser / CDN | `Cache-Control` on `/\{apiBaseUrl\}/**` | `httpCacheDuration`, `immutable` | a new `?v=` — yaʼni a deploy that changed translations |
