---
title: "Server tarafidagi tarjimalar"
description: "Nuxt server marshrutlari va middleware’lardan tarjimalardan foydalaning: useTranslationServerMiddleware va useLocaleServerMiddleware."
---
...
Nuxt I18n Micro qoʻllab-quvvatlaydi server-side translations and locale information, allowing you to translate content and access locale details on the server. This is particularly useful for APIs or server-rendered applications where localization is needed before reaching the client.
...
Yoqish uchun server-side translations and locale information, use the provided middleware functions:
...
If you need to specify a locale manually (masalan, for testing or certain requests), you can pass it to the translation middleware:
...
> **Eslatma**: The locale detection logic is shared between `useLocaleServerMiddleware` and `useTranslationServerMiddleware` to ensure consistency across the module.
