---
title: "@i18n-micro/node integratsiyasi"
description: "Node.js serverlarida tarjimalarni yuklash va mahalliylashtirish uchun @i18n-micro/node paketidan foydalaning."
---

# 🌐 Node.js Runtime

Use `@i18n-micro/node` to add i18n translations to any Node.js application, CLI tool, or backend service. Ushbu paket provides the same translation logic as the Nuxt module, but for pure Node.js environments.
...
The `currentRoute` feature sizga imkon beradi set a route context that will be used by default in all `t()` calls:
