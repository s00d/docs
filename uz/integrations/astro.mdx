---
title: "@i18n-micro/astro integratsiyasi"
description: "Astro loyihasida @i18n-micro/astro paketidan foydalanish: sozlash, marshrutlash va framework mijoz roʻyxatga olish."
---

# Astro Package (`@i18n-micro/astro`)

The `@i18n-micro/astro` package taqdim etadi lightweight, high-performance internationalization solution for Astro applications. It shares the same core logic as Nuxt I18n Micro, offering SSR support, automatic locale detection, Astro components, and full TypeScript support.
...
**Eslatma:** The `createAstroRouterAdapter` is now exported from the package, so you don't need to create your own router adapter file. However, you can still create a custom adapter if needed.
...
#### Option 3: Edge Runtime Compatible (Cloudflare Workers, Vercel Edge, va h.k.)
...
The package uses a router adapter pattern to decouple i18n functionality from specific routing implementations. Thbullows you to:
...
**Eslatma:** The `createAstroRouterAdapter` is now exported from `@i18n-micro/astro`, so you don't need to create your own adapter file for basic use cases.
...
interface I18nRoutingStrategy {
  ...
  /**
   * Get route name from path (masalan, /en/about -> about)
   ...
   * Checks if first segment bu locale code
   */
  getLocaleFromPath?: (path: string, defaultLocale: string, locales: string[]) => string
...
export function createAstroRouterAdapter(locales: Locale[], defaultLocale: string, getCurrentUrl?: () => URL): I18nRoutingStrategy {
   ...
   * Extracts route name from path (masalan, /en/about -> about)
   */
...
A router adapter bun implementation of the `I18nRoutingStrategy` interface that defines how i18n interacts with your routing system. Thbullows you to:
...
| `getRouteName(path, locales)`                               | ❌       | Extracts route name from path (masalan, `/en/about` → `about`) |
...
Ushbu misol shows how to implement locale routing using subdomains (masalan, `en.example.com`, `fr.example.com`):
...
export function createSubdomainRouterAdapter(locales: Locale[], defaultLocale: string, getCurrentUrl?: () => URL): I18nRoutingStrategy {
  ...
  const getLocaleFromPath = (path: string, defaultLocale: string, locales: string[]): string => {
    ...
    // Extract subdomain (masalan, 'en' from 'en.example.com')
...
Ushbu misol uses query parameters for locale (masalan, `/?locale=fr`):
...
Ushbu misol uses locale suffixes (masalan, `/about.en`, `/about.fr`):
...
export function createSuffixRouterAdapter(locales: Locale[], defaultLocale: string, getCurrentUrl?: () => URL): I18nRoutingStrategy {
  ...
  /**
   * Extract locale from path suffix (masalan, /about.en -> en)
   */
  const getLocaleFromPath = (path: string, defaultLocale: string, locales: string[]): string => {
...
Ushbu misol shows how to integrate with a custom routing library:
...
- **Yechim**: Check that `getLocaleFromPath` correctly identifies locale segments in your URL structure

**Problem**: Route name is incorrect

- **Yechim**: Verify `getRouteName` properly removes locale prefix/suffix and normalizes the path

**Problem**: Links don't switch locale

- **Yechim**: Ensure `switchLocalePath` and `localizePath` correctly modify URLs for your routing strategy

**Problem**: Client-side navigation doesn't work

- **Yechim**: Implement `push` and `replace` methods for client-side islands if needed
...
| -------------------- | ---------------------------------------------------------- | -------- | ---------------- | ---------------------------------------------------------------------------------- |
| `locale`             | `string`                                                   | ✅       | -                | Default locale code (masalan, `'en'`)                                                 |
...
**Eslatma:** By default, files in the `pages/` folder are treated as route-specific translations. The utility automatically maps them to route names based on the directory structure.
...
Formats a relative time (masalan, "2 hours ago").
...
1. **URL Path** - Checks if the first path segment bu locale code (masalan, `/fr/about`)
...
Birinchidan, ensure you have the required framework integrations installed:
...
Prepares translation props for islands. **Muhim:** Always pass specific keys to minimize bundle size.

**Parameters:**

- `astro`: Astro global object
- `keys`: **Recommended** - Array of translation keys to include (masalan, `['islands.vue.title', 'islands.apples']`). If omitted, all route-specific translations are included, which may increase bundle size.
...
Ushbu paket supports DevTools integration via the `@i18n-micro/devtools-ui` Vite plugin. See the [DevTools UI Package documentation](/integrations/devtools-ui) for details.
...
**Muhim:** Types are automatically available when you add the integration to your `astro.config.mjs`. If you're not using the integration (only middleware), you may need to manually add the type reference (see Manual Type Reference below).
...
If for some reason types are not showing up automatically (masalan, with custom TypeScript configuration or older Astro versions), you can manually add the reference to your `src/env.d.ts` file:
...
```json
{
  ...
  "about": {
    "title": "About Us",
    "description": "Batafsil our company"
  }
}
```
