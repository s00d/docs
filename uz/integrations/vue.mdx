---
title: "@i18n-micro/vue integratsiyasi"
description: "Mustaqil Vue 3 ilovalarida @i18n-micro/vue’dan foydalanish: plagin, composable’lar va komponentlar."
---

# Vue Package (`@i18n-micro/vue`)

The `@i18n-micro/vue` package taqdim etadi lightweight, high-performance internationalization solution for Vue 3 applications. It shares the same core logic as Nuxt I18n Micro, offering reactive translations, route-specific support, and full TypeScript support.
...
**Alternative approach**: If you need to provide locales manually (masalan, dynamically), you can use the injection keys:
...
interface I18nRoutingStrategy {
  ...
  /**
   * Component to use for rendering links (masalan, RouterLink)
   */
  linkComponent?: string | Component
...
Replaces the value at `key` in the active dictionary. This bu replace, not a merge; use `mergeTranslations` when siblings should survive.
...
The `useI18n` composable taqdim etadiccess to i18n functionality in Vue components.
...
| `activeStyle` | `CSSProperties`                                                                     | ❌       | `\{\}`    | Styles to apply when link buctive |
