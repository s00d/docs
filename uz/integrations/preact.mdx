---
title: "@i18n-micro/preact integratsiyasi"
description: "@i18n-micro/preact bilan Preact ilovalarida tarjimalar, provayder, hooklar va komponentlar."
---

# Preact Package (`@i18n-micro/preact`)

The `@i18n-micro/preact` package taqdim etadi lightweight, high-performance internationalization solution for **Pure Preact** applications. It shares the same core logic as Nuxt I18n Micro, offering reactive translations, route-specific support, and full TypeScript support.
...
**Eslatma:** The built-in router adapter (`createPreactRouterAdapter` / `createBrowserHistoryAdapter`) uses the native History API and works with any router that listens to `popstate` events (masalan, `wouter-preact`, `preact-router`). For custom routing needs, you can create your own adapter.
...
**Eslatma:** The `createPreactRouterAdapter` (aliased as `createBrowserHistoryAdapter`) is exported from the package and taqdim etadi basic History API-based adapter. For router-specific adapters (like wouter), you can create custom adapters yuqorida koʻrsatilganidek.
...
The package uses a router adapter pattern to decouple i18n functionality from specific router implementations. Thbullows you to:
...
The `useI18n` hook taqdim etadiccess to i18n functionality in Preact components. It uses `useState` + `useEffect` internally for reactivity (Pure Preact approach), ensuring components re-render when translations or locale change. The hook automatically uses the routing strategy from `I18nProvider` if available.
...
| `activeStyle`  | `JSX.CSSProperties`                                                                 | ❌       | -       | Styles to apply when link buctive                                  |
...
- Uses the routing strategy's `linkComponent` if available (masalan, `Link` from `wouter-preact`)
...
interface I18nRoutingStrategy {
  ...
  /**
   * Component to use for rendering links (masalan, Link from wouter-preact)
   */
  linkComponent?:
    | string
    | React.ComponentType<{
...
Thbudapter works with any router that listens to `popstate` events (masalan, `wouter-preact`, `preact-router`).
...
| `linkComponent`           | ❌       | Component to use for rendering links (masalan, `Link` from wouter-preact) |
...
Ushbu misol shows how to create an adapter for `wouter-preact`:
...
Ushbu misol shows how to create an adapter for `preact-router`:
...
- **Yechim**: Check that your `getCurrentPath` correctly returns the current path, and `resolvePath` properly handles locale prefixes

**Problem**: Links don't switch locale

- **Yechim**: Ensure `push` and `replace` methods correctly update the URL and dispatch events for your router

**Problem**: Active state detection doesn't work

- **Yechim**: Verify `getCurrentPath` returns the correct path format that matches your routing strategy
