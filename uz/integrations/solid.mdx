---
title: "@i18n-micro/solid integratsiyasi"
description: "@i18n-micro/solid bilan SolidJS uchun tarjimalar, signallar, komponentlar va provayder."
---

# Solid Package (`@i18n-micro/solid`)

The `@i18n-micro/solid` package taqdim etadi lightweight, high-performance internationalization solution for SolidJS applications. It shares the same core logic as Nuxt I18n Micro, offering reactive translations, route-specific support, and full TypeScript support.
...
interface I18nRoutingStrategy {
  ...
  /**
   * Component to use for rendering links (masalan, A from @solidjs/router)
   */
  linkComponent?: string | Component
...
| `linkComponent`            | ❌                        | Component to use for rendering links (masalan, `A` from @solidjs/router)                                                                                                               |
...
Ushbu misol shows how to create an adapter for a custom router that uses signals:
...
1. **Always provide `getCurrentPathAccessor` for SolidJS** (REQUIRED for reactivity): Bu taʼminlaydi proper reactivity in components. Without it, active link states and path-dependent UI won't update:
...
- **Yechim**: Check that `getCurrentPath` correctly returns the current path, and `resolvePath` properly handles locale prefixes
...
- **Yechim**: **You MUST provide `getCurrentPathAccessor`** in your router adapter. Verify it's implemented and returns a signal accessor. The accessor should be called inside `createMemo` for proper reactivity. Without `getCurrentPathAccessor`, active states won't update reactively.

**Problem**: Components don't update when path changes

- **Yechim**: **You MUST implement `getCurrentPathAccessor`** in your router adapter. It must return a signal accessor that tracks pathname changes. Components should call it inside reactive contexts (`createMemo`, `createEffect`, JSX). If you're using `createSolidRouterAdapter`, ensure it's called inside a component (not at module level) so `createSignal` and `createEffect` work correctly.
...
The `useI18n` hook taqdim etadiccess to i18n functionality in SolidJS components.
