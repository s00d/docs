---
title: "@i18n-micro/types-generator integratsiyasi"
description: "Nuxt yoki mustaqil loyihalarda tarjima JSON fayllaringizdan qatʼiy TypeScript turlarini yarating."
---

# 🔧 Types Generator (`@i18n-micro/types-generator`)

The `@i18n-micro/types-generator` package taqdim etadiutomatic TypeScript type generation for translation keys. It analyzes your JSON translation files and generates strict types, enabling full type-safety and autocomplete support for translation keys.

## 📖 Umumiy sharh

`@i18n-micro/types-generator` bun optional dev dependency that enhances your development experience by:
...
Ishonch hosil qiling include the generated file in your `tsconfig.json`:
...
Sometimes you need to use dynamic keys (masalan, based on user input or API responses). The generator taqdim etadi `ScopedKey` helper type for safe dynamic key usage.
...
The `ScopedKey<Scope>` type sizga imkon beradi narrow dynamic keys by prefix:
...
2. **Use consistent naming** - Follow a naming convention (masalan, `errors.*`, `buttons.*`)
...
5. **Don't edit generated files** - The `.d.ts` file buuto-generated and will be overwritten
