---
title: "@i18n-micro/devtools-ui integratsiyasi"
description: "Har qanday Vite loyihasida i18n-micro DevTools UI paneli: bridge sozlash, plagin registratsiyasi va foydalanuvchi ish jarayonlari."
---

# DevTools UI Package (`@i18n-micro/devtools-ui`)

The `@i18n-micro/devtools-ui` package taqdim etadi standalone development tools interface for managing translations. It includes a Vite plugin for automatic integration and a universal bridge system for framework-agnostic i18n support.

## Umumiy sharh

`@i18n-micro/devtools-ui` bu self-contained package that provides:
...
The easiest way to use DevTools is through the Vite plugin, which automatically injects a floating button and taqdim etadill necessary API endpoints:
...
- `path` - Relative path to the translation file (masalan, `en.json` or `pages/home/en.json`)
...
The package taqdim etadi universal bridge creation function that works with any i18n implementation:
...
interface BridgeAdapter {
  ...
  // Keys are in format "locale:routeName" (masalan, "en:index", "en:home")
...
- **No Production Impact** - The plugin buutomatically excluded from production builds
