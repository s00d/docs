---
title: "快速开始"
description: "安装 Nuxt I18n Micro，配置语言与策略，添加翻译文件，并在五分钟内渲染出你的第一个本地化页面。"
---
...
<Tip>
- **根级文件**（`locales/en.json` 等）——在整个应用中共享的翻译（菜单、页脚、通用 UI）。当 `translationPayloads.mode: 'premerged'`（默认）时，这些翻译会在构建时合并进每个页面。
- **页面级文件**（`locales/pages/<route>/<locale>.json`）——特定页面独有的翻译，仅在访问该页面时加载。
- **动态路由**——`pages/articles/[id].vue` 会映射到 `locales/pages/articles-id/`（方括号会被替换为短横线）。
- **自动生成**——在 `nuxt dev` 期间，如果翻译文件缺失，都会自动创建。
</Tip>

<Info>
该模块同时支持两种目录布局：

- **页面**：`pages/**/*.vue`（Nuxt 3）或 `app/pages/**/*.vue`（Nuxt 4）
- **翻译**：默认将 `locales/` 放在 **项目根目录**（推荐约定），而不是放在 `app/` 内
...
可选的同位放置方式：`i18n: \{ translationDir: 'app/locales' \}`（参见 [fixture `n3`](https://github.com/s00d/nuxt-i18n-micro/tree/main/test/fixtures/n3)）。

对于可选的 CLI 辅助工具 `text-to-i18n`，请使用 [`nuxt-i18n-micro-cli` ≥ 2.1.1](/guides/cli#-text-to-i18n-command)，以便从项目根目录扫描 `app/pages` 与 `app/components`。
</Info>
...
现在你已完成基础配置，接下来可以探索这些进阶主题：

- **[路由策略](/guides/routing-strategies)** —— 区域前缀与重定向的工作方式
- **[按组件的翻译](/guides/per-component-translations)** —— 了解 `$defineI18nRoute`
- **[自定义语言检测](/guides/custom-language-detection)** —— 通过 `useI18nLocale()` 以编程方式管理语言
