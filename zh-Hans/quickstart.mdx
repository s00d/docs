...
- **页面级文件**(`locales/pages/<route>/<locale>.json`)——某个页面独有的翻译,只有访问该页面时才会加载。
- **动态路由**——`pages/articles/[id].vue` 对应 `locales/pages/articles-id/`(方括号会被替换为短横线)。
- **自动生成**——在 `nuxt dev` 期间,所有缺失的翻译文件都会自动创建。
</Tip>
...
- **翻译**:默认将 `locales/` 保留在**项目根目录**下(推荐约定),而不是放在 `app/` 内部
...
可选的共置方式:`i18n: \{ translationDir: 'app/locales' \}`(参见 [fixture `n3`](https://github.com/s00d/nuxt-i18n-micro/tree/main/test/fixtures/n3))。

对于可选的 CLI 辅助命令 `text-to-i18n`,请使用 [`nuxt-i18n-micro-cli` ≥ 2.1.1](/guides/cli#-text-to-i18n-command),这样 `app/pages` 与 `app/components` 会从项目根目录开始扫描。
</Info>
...
现在你已完成基础设置,可以继续探索以下进阶主题:

- **[路由策略](/guides/routing-strategies)** —— 语言前缀与重定向的工作方式
- **[按组件的翻译](/guides/per-component-translations)** —— 了解 `$defineI18nRoute`
- **[自定义语言检测](/guides/custom-language-detection)** —— 使用 `useI18nLocale()` 以编程方式管理语言
