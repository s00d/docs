...
将 **Nuxt**、**Storybook** 与 **nuxt-i18n-micro** 集成到你的项目中,可以让你以组件驱动的方式构建健壮的本地化应用。本指南将逐步介绍如何一起配置这些工具,包括基础配置、本地化以及 Storybook 集成。

## 🛠 Nuxt 配置

要在 Nuxt 项目中启用本地化与 Storybook,请按如下方式配置 `nuxt.config.ts`:
...
要让 Storybook 与 Nuxt 和 nuxt-i18n-micro 集成,请配置 `.storybook/main.ts` 文件,添加 `mergeTranslations` 和 `staticDirs`。
...
1. **[GitHub 上的 Nuxt + Storybook + i18n 示例](https://github.com/s00d/nuxtjs-storybook-i18n-micro)**  
   该仓库演示了如何集成 Nuxt、Storybook 与 nuxt-i18n-micro,包括安装说明与配置细节。

2. **[Nuxt Storybook 官方文档](https://storybook.nuxtjs.org/getting-started/setup)**  
   在 Nuxt 中配置 Storybook 的官方文档,提供了详细步骤与最佳实践。

3. **[StackBlitz 交互式示例](https://stackblitz.com/~/github.com/s00d/nuxtjs-storybook-i18n-micro)**  
