---
title: "Solid 包(`@i18n-micro/solid`)"
description: "使用 @i18n-micro/solid 为 SolidJS 提供 i18n。"
---

# Solid 包(`@i18n-micro/solid`)

`@i18n-micro/solid` 包为 SolidJS 应用提供一个轻量、高性能的国际化方案。它与 Nuxt I18n Micro 共享核心逻辑,提供响应式翻译、按路由的支持以及完整的 TypeScript 支持。

## 概述

`@i18n-micro/solid` 面向不使用 Nuxt 框架、但需要国际化的 SolidJS 应用。它提供:

- **轻量** —— 使用来自 `@i18n-micro/core` 的共享核心逻辑
- **响应式** —— 通过 SolidJS 的 signal,翻译变化时组件自动更新
- **按路由的翻译** —— 支持页面级翻译
- **复数处理** —— 内置复数形式支持
- **格式化** —— 数字、日期与相对时间格式化
- **类型安全** —— 完整的 TypeScript 支持
- **与路由无关** —— 通过 `I18nRoutingStrategy` 接口,可与任意路由库配合或完全不使用路由
- **DevTools 集成** —— 内置 DevTools 支持,便于管理翻译
