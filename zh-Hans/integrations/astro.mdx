---
title: "Astro 集成"
description: "在 Astro 项目中使用 @i18n-micro/astro：集成入口点、useI18n 组合式函数以及与框架无关的辅助工具。"
---
...
### 示例 1: Subdomain-Based Locale Routing
...
### 示例 2: Query Parameter-Based Locale Routing
...
### 示例 3: Suffix-Based Locale Routing
...
### 示例 4: Integration with Third-Party Router
...
- **解决方案**： Check that `getLocaleFromPath` correctly identifies locale segments in your URL structure

**Problem**: Route name is incorrect

- **解决方案**： Verify `getRouteName` properly removes locale prefix/suffix and normalizes the path

**Problem**: Links don't switch locale

- **解决方案**： Ensure `switchLocalePath` and `localizePath` correctly modify URLs for your routing strategy

**Problem**: Client-side navigation doesn't work

- **解决方案**： Implement `push` and `replace` methods for client-side islands if needed
...
| 属性             | 类型                                                       | Required | 默认值          | 描述                                                                        |
| -------------------- | ---------------------------------------------------------- | -------- | ---------------- | ---------------------------------------------------------------------------------- |
| `locale`             | `string`                                                   | ✅       | -                | 默认值 locale code (e.g., `'en'`)                                                 |
...
⚠️ **重要**： The `loadTranslationsIntoI18n` function uses Node.js filesystem APIs and will **NOT work** in Edge runtime environments (Cloudflare Workers, Vercel Edge, Deno Deploy). For these environments, use `import.meta.glob` instead.
...
| `defaultLocale`     | `string`              | ✅       | 默认值 locale code                                        |
...
| `defaultValue` | `string`                   | ❌       | `''`     | 默认值 value if key not found   |
...
Use `getI18nProps()` to prepare translations for islands. **重要**： Pass only the keys your islands actually need to minimize bundle size:
