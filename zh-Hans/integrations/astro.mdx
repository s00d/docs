---
title: "Astro 包(`@i18n-micro/astro`)"
description: "使用 @i18n-micro/astro 为 Astro 提供 i18n。"
---

# Astro 包(`@i18n-micro/astro`)

`@i18n-micro/astro` 包为 Astro 应用提供一个轻量、高性能的国际化方案。它与 Nuxt I18n Micro 共享核心逻辑,支持 SSR、自动语言检测、Astro 组件以及完整的 TypeScript 支持。

## 概述

`@i18n-micro/astro` 面向需要服务端渲染的 Astro 应用提供国际化能力。它提供:

- **轻量** —— 使用来自 `@i18n-micro/core` 的共享核心逻辑
- **SSR 支持** —— 完整的服务端渲染兼容
- **自动语言检测** —— 从 URL 路径、Cookie 或请求头中检测语言
- **复数处理** —— 内置复数形式支持
- **格式化** —— 数字、日期与相对时间格式化
- **类型安全** —— 完整的 TypeScript 支持
- **与路由无关** —— 可与任意路由策略配合,或完全不使用路由
- **DevTools 集成** —— 通过 Vite 插件提供可选的开发工具
- **SEO 优化** —— 自动生成 meta 标签与 hreflang
...
1. **始终处理默认语言**:决定默认语言是否出现在 URL 中还是被隐藏。
...
Astro Islands 允许你在 Astro 页面中使用交互式组件(Vue、React、Svelte)。`@i18n-micro/astro` 包为各框架提供了专属适配器,便于在 island 中无缝集成 i18n。

### 概述

每个 island 通过 props 接收翻译,可确保:

- **本地性**:每个 island 只接收其需要的翻译
- **轻量**:没有全局状态,打包体积极小
- **框架专属**:为每个框架使用原生 API(Vue provide/inject、React Context、Svelte stores)
