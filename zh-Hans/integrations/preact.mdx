---
title: "Preact 包(`@i18n-micro/preact`)"
description: "使用 @i18n-micro/preact 为 Preact 提供 i18n。"
---

# Preact 包(`@i18n-micro/preact`)

`@i18n-micro/preact` 包为 **纯 Preact** 应用提供一个轻量、高性能的国际化方案。它与 Nuxt I18n Micro 共享核心逻辑,提供响应式翻译、按路由的支持以及完整的 TypeScript 支持。

## 概述

`@i18n-micro/preact` 面向纯 Preact 应用(不使用 React 兼容层)提供国际化能力。它提供:

- **轻量** —— 使用来自 `@i18n-micro/core` 的共享核心逻辑
- **响应式** —— 通过 `useState` + `useEffect`(纯 Preact 方式),翻译变化时组件自动更新
- **纯 Preact** —— 使用 Preact 原生 JSX 运行时(`h`),打包体积极小(约 3KB)
- **与路由无关** —— 可与任意路由库配合(wouter、preact-router 或自定义),或完全不使用路由
- **按路由的翻译** —— 支持页面级翻译
- **复数处理** —— 内置复数形式支持
- **格式化** —— 数字、日期与相对时间格式化
- **类型安全** —— 完整的 TypeScript 支持
- **DevTools 集成** —— 通过 Vite 插件在开发期管理翻译

**注意:**内置的路由适配器(`createPreactRouterAdapter` / `createBrowserHistoryAdapter`)使用原生 History API,可与任何监听 `popstate` 事件的路由(例如 `wouter-preact`、`preact-router`)一起工作。对于自定义路由需求,你可以创建自己的适配器。
...
`useI18n` hook 提供了在 Preact 组件中访问 i18n 能力的入口。它内部使用 `useState` + `useEffect` 实现响应性(纯 Preact 方式),确保翻译或语言变化时组件会重新渲染。若 `I18nProvider` 提供了路由策略,该 hook 会自动使用它。
