---
title: "React 包(`@i18n-micro/react`)"
description: "使用 @i18n-micro/react 为 React 提供 i18n。"
---

# React 包(`@i18n-micro/react`)

`@i18n-micro/react` 包为 React 应用提供一个轻量、高性能的国际化方案。它与 Nuxt I18n Micro 共享核心逻辑,提供响应式翻译、按路由的支持以及完整的 TypeScript 支持。

## 概述

`@i18n-micro/react` 面向不使用 Nuxt 框架、但需要国际化的 React 应用。它提供:

- **轻量** —— 使用来自 `@i18n-micro/core` 的共享核心逻辑
- **响应式** —— 通过 `useSyncExternalStore`,翻译变化时组件自动更新
- **按路由的翻译** —— 支持页面级翻译
- **复数处理** —— 内置复数形式支持
- **格式化** —— 数字、日期与相对时间格式化
