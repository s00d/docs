---
title: "模块选项参考"
description: "模块接受的每个选项及其类型、默认值与用途 —— 在构建期从类型定义读取。"
---

# 模块选项参考

`nuxt-i18n-micro` 接受的所有选项均在构建期从
...
本页所有内容都不是手工书写,因此不会与代码脱节 —— 类型、默认值与描述均来自声明本身。

关于这些选项*协同*的含义 —— 哪些组合是合理的、切换策略时会发生什么变化、可运行示例
—— 请阅读[配置](/guides/configuration)。本页给出完整清单;那一页负责解释。
...
嵌套选项以点号路径的形式列出,例如 `translationPayloads.mode` 指的是 `translationPayloads`
对象中的 `mode` 键。
...
翻译如何到达浏览器。[性能](/guides/performance)一节解释了它们在运行时会带来哪些变化。
...
- [性能](/guides/performance) —— payload 相关选项在运行时会带来什么变化
- [包 API](/api/packages/overview) —— 每个工作区包对外导出的 API
