---
title: "Preact 集成"
description: "在 Preact 应用中使用 @i18n-micro/preact：hook、组件、provider 与翻译加载。"
---
...
### 配置 With Router Adapter (Wouter Example)
...
### 配置 With Built-in Router Adapter
...
### 示例 3: Query Parameter-Based Locale Routing
...
- **解决方案**： Check that your `getCurrentPath` correctly returns the current path, and `resolvePath` properly handles locale prefixes

**Problem**: Links don't switch locale

- **解决方案**： Ensure `push` and `replace` methods correctly update the URL and dispatch events for your router

**Problem**: Active state detection doesn't work

- **解决方案**： Verify `getCurrentPath` returns the correct path format that matches your routing strategy
