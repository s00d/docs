...
description: "@i18n-micro/path-strategy 包的公开 API：运行时的语言路径解析、重定向与 SEO 辅助函数，从源码生成。"
---
...
<code>NoPrefixPathStrategy</code> — 36 个成员，与 `NoPrefixPathStrategy` above.
<code>Strategy</code> — 36 个成员，与 `NoPrefixPathStrategy` above.
...
<code>PrefixPathStrategy</code> — 36 个成员，与 `PrefixPathStrategy` above.
<code>Strategy</code> — 36 个成员，与 `PrefixPathStrategy` above.
...
<code>PrefixAndDefaultPathStrategy</code> — 37 个成员，与 `PrefixAndDefaultPathStrategy` above.
<code>Strategy</code> — 37 个成员，与 `PrefixAndDefaultPathStrategy` above.
...
<code>PrefixExceptDefaultPathStrategy</code> — 44 个成员，与 `PrefixExceptDefaultPathStrategy` above.
<code>Strategy</code> — 44 个成员，与 `PrefixExceptDefaultPathStrategy` above.
...
<code>PathStrategy</code> — 22 个成员，与 `PathStrategy` above.
<code>PathStrategyContext</code> — 15 个成员，与 `PathStrategyContext` above.
<code>ResolvedRouteLike</code> — 6 个成员，与 `ResolvedRouteLike` above.
<code>RouteLike</code> — 6 个成员，与 `RouteLike` above.
<code>RouterAdapter</code> — 3 个成员，与 `RouterAdapter` above.
<code>SwitchLocaleOptions</code> — 1 个成员，与 `SwitchLocaleOptions` above.
