...
description: "モジュールが受け付けるすべてのオプションを、その型、デフォルト、目的とともに紹介します — ビルド時に型定義から読み取られます。"
---

# モジュールオプションリファレンス

`nuxt-i18n-micro` が受け付けるすべてのオプションで、
[`packages/types/src/index.ts`](https://github.com/s00d/nuxt-i18n-micro/blob/main/packages/types/src/index.ts)
の `ModuleOptions` インターフェースからビルド時に読み取られます。
ここに手書きされたものはないため、コードから遅れることはありません — 型、デフォルト、説明は宣言そのものから来ています。

これらのオプションが組み合わさって*何を意味する*のか、どの組み合わせが理にかなうのか、戦略を切り替えたときに何が変わるのか、実用的な例が必要な場合は、[設定](/guides/configuration) を参照してください。このページは網羅的なリストであり、あちらが説明です。
...
ネストされたオプションはドット表記のパスの下に表示されるので、`translationPayloads.mode` は `translationPayloads` オブジェクト内の `mode` キーです。

{/* generated:module-options — do not edit; run `pnpm run docs:generate` */}

## ロケールとルーティング

どの言語が存在し、それらが URL にどう表示されるか。
...
翻訳がブラウザに届く仕組み。[Performance](/guides/performance) で、これらがランタイムで何を変えるかを説明しています。
...
- [設定](/guides/configuration) — これらのオプションがどう連携するか
- [Performance](/guides/performance) — ペイロードオプションがランタイムで何を変えるか
- [パッケージ API](/api/packages/overview) — 各ワークスペースパッケージのエクスポートされた API
