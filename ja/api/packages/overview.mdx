---
title: "パッケージ API"
description: "各ワークスペースパッケージのエクスポートされた API を、CI がソースに対してチェックするスナップショットからビルド時に読み取ります。"
---

# パッケージ API

`nuxt-i18n-micro` は小さな複数のパッケージから構築されており、そのほとんどが単独で使用可能です — `@i18n-micro/vue` はプレーンな Vue アプリで、`@i18n-micro/core` はどこでも使えます。

以下の各ページは、[`pnpm run api:surface`](https://github.com/s00d/nuxt-i18n-micro/blob/main/docs/guide/maintenance-commands.md#api-surface) が TypeScript ソースに対してチェックする API スナップショットから生成されます。したがって、エクスポートは、このリファレンスの変更または CI の失敗なしに消えることはありません。
