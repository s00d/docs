---
title: "Paket-API:er"
description: "Referensindex för varje @i18n-micro-workspace-paket, med lista över exporterade API:er och typer genererade från källkoppleringar som CI kontrollerar mot."
---

# Paket-API:er

`nuxt-i18n-micro` är byggd från en uppsättning små paket, varav de flesta kan användas var för sig — `@i18n-micro/vue` i en vanlig Vue-app, `@i18n-micro/core` var som helst.

Varje sida nedan genereras från API-koppleringen som [`pnpm run api:surface`](https://github.com/s00d/nuxt-i18n-micro/blob/main/docs/guide/maintenance-commands.md#api-surface) kontrollerar mot TypeScript-källorna, så en export kan inte försvinna utan att antingen denna referens ändras eller att CI misslyckas.

{/* generated:packages-index — do not edit; run `pnpm run docs:generate` */}

| Paket | Ingångspunkter | Exporter |
| --- | --- | --- |
...
- [Integrationer](/integrations/overview) — guider för användning av dessa paket i varje ramverk
- [Modulalternativ](/api/module-options) — alternativ som Nuxt-modulen accepterar
- [Metoder](/api/methods) — runtime-hjälparna som är tillgängliga inuti komponenter
